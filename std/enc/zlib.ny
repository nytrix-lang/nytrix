...
;; Reference: https://www.rfc-editor.org/rfc/rfc1950.html
...
fn _i32(v){
   "Internal: normalizes FFI return values to signed 32-bit ints."
   mut x = v
...
fn _uncompress_zlib_data(s){
   ...
   while(true){
      ...
      def r = _i32(call4(_uncompress, out, out_len_p, s, n))
      if(r == 0){ break }
      if(r == -5){
         out_cap = out_cap * 2
...
fn compress_zlib(s, level=6){
   ...
   def res = _i32(call5(_compress2, out, out_len_p, s, n, level))
   if(res != 0){
      free(out_len_p)
      free(out)
