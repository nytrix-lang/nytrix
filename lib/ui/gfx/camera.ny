...
fn camera_free_move(c, win, dt){
   ...
   mut move = vec3(0,0,0)
   if(window_key_down(win, 87)){ move = add(move, fwd_h) } ; W
   if(window_key_down(win, 83)){ move = sub(move, fwd_h) } ; S
   if(window_key_down(win, 65)){ move = sub(move, rgt_h) } ; A
   if(window_key_down(win, 68)){ move = add(move, rgt_h) } ; D
   if(window_key_down(win, 69)){ move = add(move, up_g) }  ; E
   if(window_key_down(win, 81)){ move = sub(move, up_g) }  ; Q
