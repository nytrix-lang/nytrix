;; Keywords: ui window backend nsgl
;; NSGL Context helper for Cocoa Nytrix backend.

module std.ui.window.platform.opengl.nsgl (
   create_context, destroy_context, make_current, swap_buffers, swap_interval, get_proc_address
)

use std.core *
use std.ui.window.platform.cocoa as cocoa

if(comptime{ __os_name() == "macos" }){
   #include <OpenGL/OpenGL.h>
}

fn create_context(share=0){
   ...
   def attrs = calloc(10, 4) ;; uint32_t attributes[]
