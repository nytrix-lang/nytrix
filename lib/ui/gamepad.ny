...
fn get_joysticks(){
   ...
   mut jid = 0
   while(jid < 16){
      if(glfw.joystick_present(jid)){ out = append(out, jid) }
      jid += 1
   }
   out
}
...
fn add_gamepad_mapping(mapping_string){
   glfw.update_gamepad_mappings(mapping_string)
}
...
fn is_gamepad_connected(jid){
   glfw.joystick_present(jid) && glfw.joystick_is_gamepad(jid)
}

fn get_gamepad_name(jid){
   if(glfw.joystick_is_gamepad(jid)){
      return glfw.get_gamepad_name(jid)
   }
...
fn get_gamepad_axis_count(jid){
   if(!glfw.joystick_present(jid)){ return 0 }
   if(glfw.joystick_is_gamepad(jid)){ return 6 }
   mut count_ptr = malloc(4)
   glfw.get_joystick_axes(jid, count_ptr)
...
fn get_gamepad_button_count(jid){
   if(!glfw.joystick_present(jid)){ return 0 }
   if(glfw.joystick_is_gamepad(jid)){ return 15 }
   mut count_ptr = malloc(4)
   glfw.get_joystick_buttons(jid, count_ptr)
   def count = load32(count_ptr, 0)
   free(count_ptr)
   count
}
...
fn get_gamepad_button(jid, button){
   if(!glfw.joystick_present(jid)){ return false }
   def ptr = _ensure_state_ptr(jid)
   if(glfw.joystick_is_gamepad(jid)){
      if(glfw.get_gamepad_state(jid, ptr)){
         mut idx = -1
      ...
      mut count_ptr = malloc(4)
      def raw_btns = glfw.get_joystick_buttons(jid, count_ptr)
      def count = load32(count_ptr, 0)
      free(count_ptr)
      mut idx = -1
      if(is_int(button)){ idx = button }
...
fn get_gamepad_axis(jid, axis){
   if(!glfw.joystick_present(jid)){ return 0.0 }
   def ptr = _ensure_state_ptr(jid)
   if(glfw.joystick_is_gamepad(jid)){
      if(glfw.get_gamepad_state(jid, ptr)){
         mut idx = -1
      ...
      mut count_ptr = malloc(4)
      def raw_axes = glfw.get_joystick_axes(jid, count_ptr)
      def count = load32(count_ptr, 0)
      free(count_ptr)
      mut idx = -1
      if(is_int(axis)){ idx = axis }
