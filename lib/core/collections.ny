...
fn group_by(any: xs, fnptr: key_fn): dict {
   ...
   while(i < xs.len){
      def v = xs.get(i)
      ; Keep key_fn after v is materialized; grouped def changes fnptr results.
      def k = key_fn(v)
