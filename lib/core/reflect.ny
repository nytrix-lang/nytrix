;; Keywords: reflect reflection introspection
...
@inline
fn _set_obj_tag(any: obj, int: tag): any {
   store64(prim.sub(obj, 8), tag, 0)
   obj
}
...
@inline
fn _raw_set_tag(any: obj, int: tag): any {
   __store64_idx(prim.sub(obj, 8), 0, tag)
   obj
}
...
fn contains(any: container, any: item): bool {
   ...
   if(_is_range(container)){
      ...
      if(step > 0){ if(delta < 0){ return false } } else { if(delta > 0){ return false } }
...
fn add(any: a, any: b): any {
   ...
   if(__is_int(a)){ if(__is_int(b) || _is_float(b)){ return __add(a, b) } } elif(_is_float(a)){ if(__is_int(b) || _is_float(b)){ return __add(a, b) } }
   if(_is_str(a) && _is_str(b)){ return __add(a, b) }
   if(_is_list_or_tuple(a) && _is_list_or_tuple(b)){ return __add(a, b) }
   if(_is_vecdict(a)){ if(_is_vecdict(b)){ return _vec_zip2(a, b, 0) } }
   return __add(a, b)
}
...
fn sub(any: a, any: b): any {
   ...
   if(__is_int(a)){ if(__is_int(b) || _is_float(b)){ return __sub(a, b) } } elif(_is_float(a)){ if(__is_int(b) || _is_float(b)){ return __sub(a, b) } }
   if(_is_list_or_tuple(a) && _is_list_or_tuple(b)){ return _list_zip2(a, b, 1) }
   if(_is_vecdict(a)){ if(_is_vecdict(b)){ return _vec_zip2(a, b, 1) } }
   return __sub(a, b)
}
...
fn mul(any: a, any: b): any {
   ...
   if(__is_int(a)){ if(__is_int(b) || _is_float(b)){ return __mul(a, b) } } elif(_is_float(a)){ if(__is_int(b) || _is_float(b)){ return __mul(a, b) } }
   if(__is_int(b) && b > 0){
      if(_is_str(a)){
         return repeat(a, b)
      }
...
fn _repr_seq(any: xs, str: open, str: close): str {
   def n = xs.len
   ...
   mut pos = 1
   while(i < n){
      _store_item_raw(parts, pos, repr(xs.get(i)))
      pos += 1
      if(i < n - 1){
...
fn _repr_items(any: its, bool: pairs, str: open="{", str: close="}"): str {
   def n = its.len
   if(n == 0){ return open + close }
   mut parts = list(n * 2 + 1)
   _store_item_raw(parts, 0, open)
   ...
   while(i < n){
      if(pairs){
         def p = its.get(i)
         _store_item_raw(parts, pos, repr(p.get(0)) + ": " + repr(p.get(1)))
      } else {
         _store_item_raw(parts, pos, repr(its.get(i)))
      }
      pos += 1
      if(i < n - 1){
         _store_item_raw(parts, pos, ", ")
...
fn repr(any: x): str {
   ...
   if(__eq(kind, _TAG_LIST)){ return _repr_seq(x, "[", "]") }
   if(__eq(kind, _TAG_TUPLE)){ return _repr_seq(x, "(", ")") }
   ...
   if(__eq(kind, _TAG_DICT)){ return _repr_items(items(x), true) }
   if(__eq(kind, _TAG_SET)){ return _repr_items(items(x), false) }
