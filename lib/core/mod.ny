...
fn startswith(any: s, any: prefix): bool {
   ...
   def sn = core_ref.len(s)
   def pn = core_ref.len(prefix)
...
fn endswith(any: s, any: suffix): bool {
   ...
   def sn = core_ref.len(s)
   def suffix_n = core_ref.len(suffix)
   if(suffix_n > sn){ return false }
   def off = sn - suffix_n
   mut i = 0
   while(i < suffix_n){
      if(load8(s, off + i) != load8(suffix, i)){ return false }
      i += 1
   }
   true
}
...
impl range {
   ...
   fn contains(range: self, any: item): bool {
      def n = self.len
      if(n <= 0){ return false }
      def start = load64(self, 0)
      def step = load64(self, 16)
...
fn _clone_list(any: lst): any {
   ...
   mut i = 0
   while(i < n){
      store_item(out, i, __load_item(lst, i))
      i += 1
   }
