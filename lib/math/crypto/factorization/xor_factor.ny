;; Keywords: factorization xor-factor
...
fn _xorfac_contains_pair(list: pairs, any: a, any: b): bool {
   def entry = _xorfac_pair(a, b)
   mut i = 0
   while(i < pairs.len){
      def cur = pairs.get(i)
      if(cur.get(0) == entry.get(0) && cur.get(1) == entry.get(1)){ return true }
      i += 1
   }
   false
}
      ...
      if((shared & xz) == Z(0)){
         ...
         if(sum_pq > Z(0)){
            ...
            if(roots != nil){
               ...
               if(p > Z(0) && q > Z(0) && p * q == nz && (p ^^ q) == xz && !_xorfac_contains_pair(pairs, p, q)){
