...
#main {
   ...
   assert(sq.get("method", "") == "squfof", "squfof report method")
   assert(sq.get("multiplier_policy", "") == "adaptive-squarefree", "squfof defaults merged")
   assert(sq.contains("saved_q_limit"), "squfof default fields survive")
   def batch = batch_gcd_factor_report(8051)
   assert(batch.get("success", false) && 8051 % batch.get("factor", 1) == 0, "batch GCD report factors 8051")
   assert(batch.get("method", "") == "batch-product-gcd", "batch gcd report method")
   def f1 = pollard_rho(8051)
   assert(f1 != nil, "pollard_rho finds factor")
   def f2 = pollard_pm1(8051, 20)
   assert(f2 != nil, "pollard_pm1 finds factor")
   def fb = pollard_brent(8051)
   assert(fb != nil, "pollard_brent finds factor")
   def fl = lehman_factor(8051)
   assert(fl != nil, "lehman_factor finds factor")
   def fsq = squfof(8051)
   assert(fsq != nil, "squfof finds factor")
   def fps = pollard_strassen(8051)
   assert(fps != nil, "pollard_strassen finds factor")
   def fbg = batch_gcd_factor(8051, 2, 128, 8)
   assert(fbg != nil, "batch_gcd_factor finds factor")
   def f3 = pollard_rho_iter(91, 10000)
   assert(f3 != nil, "pollard_rho_iter finds factor of 91")
   def flr = lehman_factor_report(8051)
   assert(flr.get("success", false), "lehman_factor_report success")
   def br = pollard_brent_report(8051)
   assert(br.get("method", "") == "pollard-brent", "brent report method")
   assert(br.get("elapsed_ms", -1.0) >= 0.0, "brent report elapsed_ms")
   assert(br.get("status", "") != "", "brent report status")
   def pm1 = pollard_pm1_report(91, 12)
   assert(pm1.get("method", "") == "pollard-p-minus-1", "pm1 report method")
   assert(pm1.get("stage1_prime_count", 0) > 0, "pm1 stage report fields")
   def n_u62 = Z(1000000007) * Z(2000000011)
   def sq_u62 = squfof_report(n_u62)
   assert(sq_u62.get("success", false), "u62 squfof finds factor")
   assert(sq_u62.get("numeric_kernel", "") == "u62-int-squfof", "u62 kernel selected for composite")
   def n_br = Z(1009) * Z(1013)
   def ps = pollard_strassen_report(n_br)
   assert(ps.get("success", false), "batched pollard-strassen finds factor")
   assert(ps.get("kernel", "") == "batched-product-gcd", "batched kernel selected")
   assert(!ps.get("fallback_used", true), "batched path avoids fallback")
   def brent_r = pollard_brent_report(Z(1048703) * Z(4503599627827351))
   assert(brent_r.get("factor", nil) != nil, "brent precheck strategy finds factor")
   def leh = lehman_factor_report(Z(101) * Z(103), 20000, 512)
   assert(leh.get("search_kernel", "") == "residue-stepped-square-filter", "lehman residue kernel")
   assert(leh.get("sqrt_checks", 0) > 0, "lehman sqrt counters")
