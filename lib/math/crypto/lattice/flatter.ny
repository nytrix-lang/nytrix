...
#main {
   def r = flatter_reduce_report(Matrix([[97, 0, 0, 0], [14, 1, 0, 0], [35, 0, 1, 0], [62, 0, 0, 1]]), 0.99, 2)
   assert(r.get("method", "") == "flatter-profile", "flatter method")
   assert(r.get("basis", []).len == 4, "flatter row count")
   def qr = qr_factor_report(Matrix([[12, -51, 4], [6, 167, -68], [-4, 24, -41]]))
   assert(qr.get("reconstruction_error", 1.0) < 0.000001, "qr reconstruction")
   assert(qr.get("orthogonality_error", 1.0) < 0.000001, "qr orthogonal")
   def fused = fused_qr_size_reduce_report(Matrix([[10, 0], [9, 1]]), 2, 0.51)
   assert(fused.get("op_count", 0) == 1, "fused op count")
   assert(fused.get("transform_verified", false), "fused transform")
   def blocked = blocked_triangular_size_reduce_report(Matrix([[100, 0, 0, 0, 0, 0], [160, 101, 0, 0, 0, 0], [250, -180, 103, 0, 0, 0], [360, 260, -190, 107, 0, 0], [470, -370, 290, -210, 109, 0], [580, 490, -390, 310, -230, 113]]), 2, 2)
   assert(blocked.get("method", "") == "blocked-triangular-size-reduction", "blocked triangular method")
   assert(blocked.get("block_count", 0) == 3, "blocked triangular block count")
   assert(blocked.get("transform_verified", false), "blocked triangular transform")
   def tri = triangular_size_reduce_report(Matrix([[12, 0, 0, 0], [7, 11, 0, 0], [5, -3, 1, 0], [-6, 4, 2, 1]]), 1)
   assert(tri.get("best_norm_after", 0) <= tri.get("best_norm_before"), "triangular size reduce improves best norm")
   def iter_fused = iterated_fused_qr_size_reduce_report(Matrix([[100, 0, 0], [70, 2, 0], [55, 9, 1], [49, 8, 1]]), 2, 0.51, "gso")
   assert(iter_fused.get("transform_verified", false), "iterated fused transform")
   def short = short_row_prepass_report(Matrix([[100, 0], [1, 1], [2, 0]]))
   assert(short.get("method", "") == "short-row-prepass", "short row method")
   assert(short.get("best_row", 0) == 1, "short row best")
   def lagrange = lagrange_pair_reduce_report(Matrix([[10, 0], [9, 1]]), 1, 4)
   assert(lagrange.get("method", "") == "lagrange-row-pair-reduction", "lagrange method")
   assert(lagrange.get("first_norm_after", Z(100)) <= Z(2), "lagrange first norm")
   def rel = relative_size_reduce_report(Matrix([[10, 0], [9, 1]]), 1)
   assert(rel.get("op_count", 0) == 1, "relative size op count")
   def matmul_rep = lattice_matmul_report(Matrix([[1, 2], [3, 4]]), Matrix([[5, 6], [7, 8]]))
   assert(matmul_rep.get("selected_kernel", "") == "sparse-row", "matmul sparse dispatch")
   assert(matrix.mat_get(matmul_rep.get("matrix"), 0, 0) == Z(19), "matmul 0,0")
   def schoenhage = schoenhage_reduce_report(Matrix([[97, 0, 0, 0], [14, 1, 0, 0], [35, 0, 1, 0], [62, 0, 0, 1]]), "proved3", 3, 2, 64)
   assert(schoenhage.get("method", "") == "schoenhage-recursive-sublattice-reduction", "schoenhage method")
   assert(schoenhage.get("transform_verified", false), "schoenhage transform")
   def recursive = recursive_reduce_report(Matrix([[97, 0, 0, 0], [14, 1, 0, 0], [35, 0, 1, 0], [62, 0, 0, 1]]), 2, 0.75, 1)
   assert(recursive.get("level_count", 0) > 0, "recursive levels")
   assert(recursive.get("transform_verified", false), "recursive transform")
   def num = lattice_numeric_backend_report(Matrix([[10, 0], [9, 1]]), "householder", 1)
   assert(num.get("method", "") == "lattice-numeric-backend-report", "numeric backend method")
   assert(num.get("fixed_point_exact", false), "numeric backend fixed exact")
   assert(num.get("size_reduce_transform_verified", false), "numeric backend transform")
   print("✓ std.math.crypto.lattice.flatter self-test passed")
}
