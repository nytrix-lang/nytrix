;; Keywords: cipher autocrack
;; Classical cipher identification and cracking routines.
module std.math.crypto.cipher.autocrack(cipher_auto_crack, cipher_auto_crack_best)
use std.core
...
fn _contains(any: haystack, any: needle): bool {
   haystack != nil && needle != nil && str.find(haystack, needle) >= 0
}
...
fn _shape_penalty(str: text): int {
   ...
   while(i < bs.len){
      ...
      if(_is_alpha_byte(b)){
         ...
         if(current_run >= 8){ bad_runs += current_run - 7 }
         current_run = 0
      }
      if(b == prev){ same_run += 1 } else {
         if(same_run >= 5){ bad_runs += same_run - 4 }
         same_run = 1
      }
      prev = b
      i += 1
...
fn cipher_auto_crack(str: ciphertext, int: limit=12): list {
   ...
   crypto_require_nonempty(ciphertext, "cipher.autocrack", "ciphertext")
   mut candidates = []
   ...
   def has_separators = digit_n > 0 || _contains(ciphertext, "_") || _contains(ciphertext, "-") || _contains(ciphertext, ":") || _contains(ciphertext, "=")
...
fn cipher_auto_crack_best(str: ciphertext, int: limit=12): list {
   "Return the single best auto-crack candidate as [kind, param, plaintext, score]."
   crypto_require_nonempty(ciphertext, "cipher.autocrack_best", "ciphertext")
   def candidates = cipher_auto_crack(ciphertext, limit)
