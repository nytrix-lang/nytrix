...
#main {
   ...
   def st0 = Z(42)
   def p = pcg32_next(st0, inc)
   assert(pcg32_advance(st0, inc, 1) == get(p, 0), "pcg advance")
   assert(pcg32_rewind(get(p, 0), inc) == st0, "pcg rewind")
   def xs = xorshift32_next(1)
   assert(get(xs, 0) != 1, "xorshift32 moves")
   def xr = xoroshiro128plus_next([1, 2])
   assert(len(get(xr, 0)) == 2, "xoroshiro state")
   def xo = xoshiro256plusplus_next([1, 2, 3, 4])
   assert(len(get(xo, 0)) == 4, "xoshiro state")
   assert(prng_output_family_hint("++") != "unknown family", "family hint")
   print("prng/pcg_xoshiro")
   print("✓ std.math.crypto.prng self-test passed")
}
