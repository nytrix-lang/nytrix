;; Keywords: analysis frequency-analysis language-model worldfreq math crypto
...
fn _ngram_packed_ready(list monograms, list bigrams, list trigrams, list quadgrams) bool {
   monograms.len >= _WF_PACK1 && bigrams.len >= _WF_PACK2 &&
   trigrams.len >= _WF_PACK3 && quadgrams.len >= _WF_PACK4
}

fn _ngram_score_dicts(str text, dict monograms, dict bigrams, dict trigrams, dict quadgrams) int {
...
fn _ngram_score_packed(str text, list monograms, list bigrams, list trigrams, list quadgrams) int {
   mut score = 0
   mut i = 0
   mut count = 0
   mut p1 = 0
...
fn worldfreq_ngram_score(str text, any profile_or_lang="english") int {
   ...
   if !_ngram_packed_ready(monograms, bigrams, trigrams, quadgrams) {
      return _ngram_score_dicts(
         ...
         profile.get("bigrams", dict(0)),
         profile.get("trigrams", dict(0)),
         profile.get("quadgrams", dict(0))
      )
   }
   _ngram_score_packed(text, monograms, bigrams, trigrams, quadgrams)
}
...
#main {
   def paths = worldfreq_profile_paths("english")
   assert(worldfreq_profile_exists("english") && paths.get("lang", "") == "english" && paths.get("quadgrams_path", "").len > 0, "worldfreq profile paths")
   assert(worldfreq_alpha_upper("A b-c! 123") == "ABC", "worldfreq alpha upper")
   def words = worldfreq_word_counter("This is this.")
   assert(words.get("THIS", 0) == 2 && worldfreq_rows_to_table([["AB", 2], ["BA", 1]]) == "AB 2\nBA 1\n", "worldfreq word helpers")
   def grams = worldfreq_ngram_counter("ABABA", 2)
   assert(grams.get("AB", 0) == 2 && grams.get("BA", 0) == 2, "worldfreq ngram counter")
   def profile = {
      "lang": "probe",
      "word_set": {"THIS": true, "IS": true, "SECRET": true, "MESSAGE": true},
      "common_words": {"THIS": 4, "IS": 3},
      "monograms": {"A": 3, "B": 2},
      "bigrams": {"AB": 4, "BA": 4},
      "trigrams": {"ABA": 3},
      "quadgrams": {"ABAB": 2}
   }
   def stats = worldfreq_word_stats("THIS IS A SECRET MESSAGE", profile)
   assert(int(stats.get(2, 0)) >= 4 && worldfreq_ngram_score("ABABA", profile) > 0 && worldfreq_score_text("THIS IS ABABA", profile) > 0, "worldfreq scoring")
   assert(worldfreq_detect("THIS IS A SECRET MESSAGE", [], 1).len == 0, "empty detect set")
   print("✓ std.math.crypto.analysis.worldfreq self-test passed")
}
