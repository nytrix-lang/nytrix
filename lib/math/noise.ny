...
fn perlin3_seed(any: x, any: y, any: z, int: seed=0): any {
   ...
   def w = _ease(zf)
   def s = seed & 255
   def r0 = _perm.get((x0 + s) & 255)
   def r1 = _perm.get((x1 + s) & 255)
   def r00 = _perm.get((r0 + y0) & 255)
   def r01 = _perm.get((r0 + y1) & 255)
   def r10 = _perm.get((r1 + y0) & 255)
   def r11 = _perm.get((r1 + y1) & 255)
   def n000 = _grad(_grad_idx.get((r00 + z0) & 255), xf, yf, zf)
   def n001 = _grad(_grad_idx.get((r00 + z1) & 255), xf, yf, zf - 1)
   def n010 = _grad(_grad_idx.get((r01 + z0) & 255), xf, yf - 1, zf)
   def n011 = _grad(_grad_idx.get((r01 + z1) & 255), xf, yf - 1, zf - 1)
   def n100 = _grad(_grad_idx.get((r10 + z0) & 255), xf - 1, yf, zf)
   def n101 = _grad(_grad_idx.get((r10 + z1) & 255), xf - 1, yf, zf - 1)
   def n110 = _grad(_grad_idx.get((r11 + z0) & 255), xf - 1, yf - 1, zf)
   def n111 = _grad(_grad_idx.get((r11 + z1) & 255), xf - 1, yf - 1, zf - 1)
