;; Keywords: backends backend dispatch math
...
fn _cmd_file_exists(str base) bool {
...
fn _cmd_exists(str cmd) bool {
   ...
   if(_cmd_file_exists(cmd)){ return true }
   def p = env("PATH")
   ...
   while(i < dirs.len){
      def d = dirs.get(i, "")
      if(d.len > 0){
         if(_cmd_file_exists(ospath.join(d, cmd))){ return true }
      }
      i += 1
   }
   false
