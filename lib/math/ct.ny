;; Keywords: ct constant-time side-channel
...
fn ct_abs(int: a): int {
   ...
   def mask = a >> 63 ; all 1s if negative, all 0s if positive
   def xored = a ^^ mask
   xored - mask ; if negative: (~a) - (-1) = ~a + 1 = |a|
}
...
fn ct_swap(int: a, int: b, int: condition): list {
   ...
   def mask = 0 - mask_shifted ; all 1s if condition, all 0s if not
   def masked = x & mask
   [a ^^ masked, b ^^ masked]
}
