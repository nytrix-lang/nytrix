...
#main {
   ...
   assert(argv_before != 0, "os facade argv snapshot")
   assert(__fix_fn_ptr(nil) == nil, "os facade fix fn ptr nil")
   assert(__fix_fn_ptr(0) == 0, "os facade fix fn ptr zero")
   def probe_enabled = __index_read_probe_enabled()
   assert(probe_enabled == true || probe_enabled == false, "os facade index probe bool")
   assert(__set_args(0, 0, 0) == nil, "os facade set empty argv")
   assert(__argc() == 0, "os facade empty argc")
   assert(__argvp() != 0, "os facade empty argv snapshot")
   assert(__argv(0) == nil, "os facade argv out of range")
