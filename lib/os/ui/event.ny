;; Keywords: event
;; Event record construction and inspection for std.os.ui input and window events.
module std.os.ui.event(is_event, make_event, event_type, event_window, event_window_id, event_data, queue_push, queue_pop, queue_len)
use std.core
...
fn make_event(any: kind, any: win, any: win_id=0, any: data=0): list {
   "Constructs an event payload."
   ["std.os.ui.event", kind, win, win_id, data]
}

fn _event_get(any: ev, int: slot, any: fallback=0): any {
