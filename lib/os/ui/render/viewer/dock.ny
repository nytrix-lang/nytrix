;; Keywords: ui dock layout panels workspace tiling os render viewer
;; Docking and tiled-panel layout primitives for editor and viewer workspaces.
;; References:
;; - std.os.ui.render.viewer.gui
module std.os.ui.render.viewer.dock(begin_tool, end_tool, show_tool, tool_closed, normalize_tool_ids, merge_tool_ids, visible_tool_ids, tool_snapshot, apply_tool_rect_if_visible, focus_pref_rect, apply_focus_layout, apply_merged_focus_layout)
use std.core
use std.math
...
fn apply_tool_rect_if_visible(any id, bool visible, list r) any { gui.apply_window_rect_if_visible(id, visible, r) }

fn focus_pref_rect(any id, list root) list {
   "Returns preferred focused-window bounds for a tool id."
   def rid = to_str(id)
...
fn apply_focus_layout(list ids, f64 ww, f64 wh, f64 gap=14.0) int {
   ...
   if(n == 3){
      if(to_str(ids.get(0, "")) == "profiler" && to_str(ids.get(1, "")) == "inspector" && to_str(ids.get(2, "")) == "widget_probe"){
         ...
         gui.apply_window_rect("inspector", right_rows3.get(0, root))
         gui.apply_window_rect("widget_probe", right_rows3.get(1, root))
         return 3
      }
      def rows = gui.split_rows(root, [0.56, 0.44], g)
      def cols = gui.split_cols(rows.get(0, root), [1.0, 1.0], g)
      gui.apply_window_rect(to_str(ids.get(0, "")), cols.get(0, root))
      gui.apply_window_rect(to_str(ids.get(1, "")), cols.get(1, root))
   ...
   if(n == 4){
      ...
      gui.apply_window_rect(to_str(ids.get(0, "")), top.get(0, root))
      gui.apply_window_rect(to_str(ids.get(1, "")), top.get(1, root))
      gui.apply_window_rect(to_str(ids.get(2, "")), bot.get(0, root))
      gui.apply_window_rect(to_str(ids.get(3, "")), bot.get(1, root))
      return 4
   }
   def cols = gui.split_cols(root, [1.0, 1.0], g)
...
#main {
   gui.reset_state()
   def merged = merge_tool_ids(["editor_main", "", "0"], {"id": "inspector"}, {"profiler": true, "node_graph": false}, "widget_probe")
   assert(merged == ["editor_main", "inspector", "profiler", "widget_probe"], "dock merge tool ids")
   assert(normalize_tool_ids({"asset_browser": true, "node_graph": false}) == ["asset_browser"], "dock normalize dict ids")
   assert(show_tool("editor_main", true) && !show_tool("inspector", false), "dock show tool")
   assert(visible_tool_ids(merged) == ["editor_main"], "dock visible tool ids")
   assert(begin_tool("hidden_tool", false, "Hidden", 0.0, 0.0, 120.0, 80.0) == [false, false], "dock hidden tool")
   assert(!tool_closed("editor_main"), "dock tool not closed")
   apply_tool_rect_if_visible("editor_main", true, [10.0, 20.0, 320.0, 220.0])
   assert(gui.window_rect("editor_main").get(0, 0.0) == 10.0 && gui.window_rect("editor_main").get(2, 0.0) == 320.0, "dock apply rect")
   def pref = focus_pref_rect("asset_browser", [0.0, 0.0, 1000.0, 800.0])
   assert(pref.get(2, 0.0) == 1000.0 && pref.get(3, 0.0) > 0.0, "dock focus pref rect")
   assert(apply_focus_layout(["editor_main", "inspector"], 1000.0, 700.0, 10.0) == 2, "dock focus layout")
   assert(gui.window_rect("inspector").get(2, 0.0) > 0.0, "dock focus layout writes rect")
   print("✓ std.os.ui.render.viewer.dock self-test passed")
}
