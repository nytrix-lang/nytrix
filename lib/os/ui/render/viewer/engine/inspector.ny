...
fn _prop(any label, any value, any tone=[0.74, 0.79, 0.88, 1.0]) any {
   def c = is_list(tone) ? tone : [0.74, 0.79, 0.88, 1.0]
   gui.property_row("prop_" + _safe_id(label), label, _prop_value(value), c)
}

fn _prop_bool(any label, any value) any {
   _prop(label, _bool_text(value), bool(value) ? [0.72, 0.94, 0.82, 1.0] : [1.0, 0.70, 0.38, 1.0])
}
...
fn _slider3(str idp, str title, f64 x0, f64 y0, f64 z0, f64 lo, f64 hi, f64 cell_w, bool packed) list {
   gui.text_colored(title, [0.70, 0.76, 0.80, 0.94])
   if(packed){
...
fn _material_section(any state) dict {
   ...
   if(mat_count <= 0){
      gui.text_colored("No material records.", [0.86, 0.74, 0.55, 1.0])
      return st
   }
...
fn _object_transform_tab(any state) dict {
   ...
   if(!bool(st.get("has_scene", false))){
      gui.text_colored("No selected scene object.", [0.86, 0.74, 0.55, 1.0])
      return st
   }
...
fn _transform_tab(any state) dict {
   ...
   if(gui.collapsing_header("inspect_view_transform", "Transform Gizmo", true)){
      ...
      if(_wide_panel(st)){
         ...
         gui.text_colored("Axis constraint", [0.70, 0.76, 0.80, 0.94])
...
fn _renderer_tab(any state) dict {
   ...
   if(gui.collapsing_header("inspect_renderer_stats", "Renderer Counters", true)){
      ...
      _prop("Hotspot", _txt(st.get("renderer_hotspot", ""), "steady"), [1.0, 0.78, 0.28, 1.0])
   }
