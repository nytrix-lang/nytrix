...
fn chrome_metrics(any editor_w, any editor_h) dict {
   ...
   def dense = compact || w < 1180.0 || h < 700.0
   {
      ...
      "toolbar_w": compact ? 36.0 : (dense ? 42.0 : 82.0),
      "toolbar_w_console": compact ? 36.0 : (dense ? 42.0 : 96.0)
   }
}
...
fn draw_header(any scene_name, any fps_value, any layout_name, any active_shot, any tab, any editor_w, any editor_h, any card_w, any scene_icon, any hierarchy_icon, any theme_icon, any console_icon, dict renderer_stats, any renderer_hotspot) int {
   ...
   if(_toolbar_button("toolbar_catalog", scene_icon, dense ? "" : "Catalog", dense ? toolbar_w : 92.0, selected == 0)){ selected = 0 }
   gui.same_line()
   if(_toolbar_button("toolbar_hierarchy", hierarchy_icon, dense ? "" : "Hierarchy", dense ? toolbar_w : 108.0, selected == 1)){ selected = 1 }
   gui.same_line()
...
fn draw_style_tab(dict st) dict {
   ...
   def bg0 = colorpicker.rgba(st.get("bg", [0.05, 0.05, 0.05, 1.0]), [0.05, 0.05, 0.05, 1.0])
   def accent0 = colorpicker.rgba(st.get("accent", [0.6, 0.6, 0.6, 1.0]), [0.6, 0.6, 0.6, 1.0])
   out["bg"] = colorpicker.edit4("clear_color", "Clear Color", bg0, [0.05, 0.05, 0.05, 1.0])
   out["accent"] = colorpicker.edit4("accent_color", "UI Accent", accent0, [0.6, 0.6, 0.6, 1.0])
...
fn draw_layout_tab(dict st) list {
   ...
   if(gui.button("apply_shot", compact ? "Shot" : "Apply", compact ? 80.0 : 88.0)){ action = "apply_shot" }
   gui.same_line()
   if(gui.button("layout_tile", "Tile", compact ? 72.0 : 78.0)){ action = "retile" }
   gui.same_line()
   if(gui.button("layout_save", "Save", compact ? 72.0 : 78.0)){ action = "save_slot" }
   gui.same_line()
   if(gui.button("layout_load", "Load", compact ? 72.0 : 78.0)){ action = "load_slot" }
   gui.separator()
   if(gui.button("dump_shot", "Shot", 78.0)){ action = "dump_shot" }
   gui.same_line()
   if(gui.button("frame_dump", "Frame", 82.0)){ action = "frame_dump" }
   gui.same_line()
   if(gui.button("graph_reset_shell", compact ? "Graph" : "Reset Graph", compact ? 84.0 : 112.0)){ action = "reset_graph" }
   if(gui.button("probe_now", "Probe", 82.0)){ action = "probe" }
   gui.same_line()
   if(gui.button("focus_probe", compact ? "Print" : "Print Probe", compact ? 82.0 : 108.0)){ action = "print_probe" }
