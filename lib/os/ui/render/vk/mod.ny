...
use std.os.ui.render.vk.vulkan (destroy_buffer, free_memory)
...
fn gltf_pipeline_key(str: alpha_mode, bool: double_sided=false, bool: unlit=false, int: feature_mask=0): str { to_str(alpha_mode) + "|ds=" + to_str(double_sided ? 1 : 0) + "|unlit=" + to_str(unlit ? 1 : 0) + "|f=" + to_str(int(feature_mask)) }

fn gltf_variant_key(int: mesh_idx, int: prim_idx, int: variant_idx): str { to_str(int(mesh_idx)) + ":" + to_str(int(prim_idx)) + ":" + to_str(int(variant_idx)) }

fn gltf_alpha_sort_key(f64: depth, int: material_idx, int: part_idx): dict { {"depth": float(depth), "material": int(material_idx), "part": int(part_idx)} }
...
fn indirect_make_key(any: index_buf, int: index_off, any: vertex_buf, int: vertex_off, int: material, any: pipeline, int: index_count): str { to_str(index_buf) + "|" + to_str(index_off) + "|" + to_str(vertex_buf) + "|" + to_str(vertex_off) + "|" + to_str(material) + "|" + to_str(pipeline) + "|" + to_str(index_count) }
...
fn indirect_group_command_bytes(bool: indexed=true): int { indexed ? INDIRECT_CMD_BYTES : INDIRECT_DRAW_BYTES }
...
fn cull_stats(int: total, int: visible): dict { {"total": total, "visible": visible, "culled": total - visible} }
