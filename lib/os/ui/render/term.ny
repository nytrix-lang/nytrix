;; Keywords: render term terminal ansi tty
;; Terminal Emulator for Nytrix
module std.os.ui.render.term(init, toggle, draw, handle_event, log, exec, clear, is_open, set_font, set_colors, get_history, get_input, font_cell_size, framebuffer_size, sync_framebuffer_size, resize_term)
use std.core
use std.os
use std.math
...
fn init(any: font, int: bg_color=0, int: text_color=0): any {
   ...
   _font       = font
   _bg_color   = (bg_color != 0) ? bg_color : 0x00000000 ;; Fully transparent by default
   _white_pack = 0xffffffff
   _cyan_color = _white_pack
   _sep_color  = 0x22ffffff ;; Very subtle white line
...
fn font_cell_size(any: font, any: font_size): list {
   ...
   if(cw <= 1.0){
      def aw = measure_text(font, "A")
      cw = float(aw.get(0, 0.0))
   }
   if(cw <= 1.0){
      def iw = measure_text(font, "i")
...
fn _mark_input_dirty(): any { _mark_cursor_dirty() }
...
fn _wrap_history_line(any: line, f64: max_w): list {
   ...
   while(rest.len > 0){
      ...
      mut i = 0
      while(i < total){
         def char_len = lib_str._utf8_seq_len(rest, i, total)
         if(char_len <= 0){
            i += 1
            continue
         }
         def cp = lib_str._utf8_decode_at(rest, i, char_len)
         mut adv = 8.0
      ...
      if(final_cut == 0 && total > 0){
         def char_len = lib_str._utf8_seq_len(rest, 0, total)
   ...
   if(typ == EVENT_KEY_CHAR){
      ...
      if((mods & MOD_CONTROL) != 0){
         if(k == uin.KEY_A){ _cursor = 0 _mark_cursor_dirty() return true }
            ...
            _cursor = 0 _mark_input_dirty() return true
         }
         elif(k == uin.KEY_K){ _input = str.str_slice(_input, 0, _cursor) _mark_input_dirty() return true }
         elif(k == uin.KEY_W){
            if(_cursor > 0){
               mut i = _cursor - 1
         ...
         elif(k == uin.KEY_C){ _input = "" _cursor = 0 _mark_input_dirty() return true }
         elif(k == uin.KEY_R){
