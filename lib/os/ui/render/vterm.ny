;; Keywords: render vterm
;; Virtual Terminal Emulator for Nytrix
module std.os.ui.render.vterm(new, open, close, update, draw, handle_event, write, send_input, resize, set_viewport, is_running, get_title, cursor_blink_phase, needs_visual_refresh, idle_sleep_ms, default_shell_path, default_shell_args, env_bg_color, abgr_to_color)
use std.core
...
use std.os.ui.render (
   ...
   get_active_backend, measure_text, set_unlit, texture_destroy
)
...
def MODE_UTF8        = 4
def MODE_MOUSE       = 8 ;; any mouse reporting enabled (?1000/?1002/?1006)
def ESC_START      = 1
def ESC_CSI        = 2
def ESC_STR        = 4
...
def SYS_BASE = 0xff000000 ;; opaque background fallback
def SYS_TEXT = 0xffe5e5e5 ;; Theme-like fallback foreground
...
def OFF_SEL_SCROLL_ACC = 152 ;; 4 bytes (f32): accumulated scroll for selection drag
def OFF_LAST_MX        = 156 ;; 4 bytes (f32): last known mouse X in pixels
def OFF_LAST_MY        = 160 ;; 4 bytes (f32): last known mouse Y in pixels
def OFF_SCROLL_F       = 164 ;; 4 bytes (f32): fractional scroll offset in lines
def OFF_BRACKET_PASTE  = 168 ;; 1 byte: bracketed paste mode (?2004)
def OFF_FOCUS_REPORT   = 169 ;; 1 byte: focus in/out reporting (?1004)
def OFF_MOUSE_FLAGS    = 170 ;; 1 byte: mouse flags (1000/1002/1003/1006)
def OFF_MOUSE_BTNDOWN  = 171 ;; 1 byte: whether a mouse button is down
def OFF_MOUSE_BTN      = 172 ;; 1 byte: last pressed mouse button (0/1/2)
def OFF_KBD_PROTO      = 173 ;; 1 byte: kitty keyboard protocol enabled (CSI > 1 u)
def OFF_ESC_STR_KIND   = 174 ;; 1 byte: kind of active string escape (']','P','^','_')
def OFF_LAST_KEY_T     = 176 ;; 8 bytes: last key event timestamp (ns)
def OFF_LAST_KEY_CODE  = 184 ;; 4 bytes: last key code
def OFF_CHAR_CB_ACTIVE = 220 ;; 1 byte: whether char callbacks are working
def OFF_INSERT         = 221 ;; 1 byte: IRM insert mode (CSI 4 h / CSI 4 l)
def OFF_CURSOR_STYLE_S = 222 ;; 1 byte: cursor shape from DECSCUSR (0=default/block,1=block,2=beam,3=underline)
def OFF_PENDING_PRINT  = 223 ;; 1 byte: printable key waiting for a following char event
def OFF_PENDING_CHAR   = 224 ;; 4 bytes: fallback printable codepoint
def OFF_PENDING_MOD    = 228 ;; 4 bytes: fallback modifiers
def OFF_PENDING_ACTION = 232 ;; 4 bytes: fallback key action
def OFF_PENDING_T      = 236 ;; 8 bytes: fallback timestamp
def OFF_LAST_KEY_MOD   = 188 ;; 4 bytes: last key mod
def OFF_KBD_FLAGS      = 192 ;; 4 bytes: kitty keyboard progressive enhancement flags
def OFF_BACKGROUND_ERASE = 196 ;; 1 byte: erase uses current background when enabled
...
fn _vterm_clamp_cursor(dict: vt, any: st, int: co, int: ro): any{
   ...
   if(cx != old_cx || cy != old_cy){
      if(vt.get("pty_trace", false)){
         print("[vterm:pty] clamp cursor cx=" + to_str(old_cx) + " cy=" + to_str(old_cy) + " -> cx=" + to_str(cx) + " cy=" + to_str(cy))
      }
...
fn _vterm_trace_cursor_state(dict: vt, any: st, str: label): any{
   if(!vt.get("pty_trace", false) || !st){ return 0 }
   print("[vterm:pty] " + label + " cx=" + to_str(load32(st, OFF_CX)) + " cy=" + to_str(load32(st, OFF_CY)))
   0
}
...
fn _mouse_xterm_button(int: native_btn): int{
   if(native_btn == 0){ return 0 } ;; left
   if(native_btn == 2){ return 1 } ;; middle
   if(native_btn == 1){ return 2 } ;; right
   native_btn
}
...
fn new(int: cols, int: rows, dict: fonts, int: bg_color=0, int: text_color=0): any{
   ...
   .set("input_trace", _env_bool_cached("NY_TERM_INPUT_TRACE", false))
   .set("pty_trace", _env_bool_cached("NY_TERM_PTY_TRACE", false))
   .set("draw_trace", _env_bool_cached("NY_TERM_DRAW_TRACE", false))
   .set("draw_trace_frames", 0)
   ...
   mut cursor_style = 2 ;; beam by default
   ...
   def st_tmp = 0 ;; resolved below after state alloc; stored in _cursor_style_default for open()
...
fn _vterm_reader_thread(any: sh): any{
   ...
   while(load64(sh, 32) != 0){
      ...
      if(_env_bool_cached("NY_TERM_PTY_TRACE", false)){
         mut head = ""
         mut hi = 0
         def hlim = min(r, 64)
         while(hi < hlim){
            if(hi > 0){ head += " " }
            def hb = load8(t, hi) & 255
            if(hb < 16){ head += "0" }
            head += to_hex(hb)
            hi += 1
         }
         print("[vterm:pty] read r=" + to_str(r) + " head=" + head)
      }
      mutex_lock(lk)
...
fn open(dict: vt, str: cmd="/bin/sh", list: args=[]): any{
   ...
   if(pid == 0){
      ...
      if(!ft){ ne = ne.append("TERM=" + term_val) }
      ne = ne.append("COLORTERM=truecolor")
      ne = ne.append("TERM_PROGRAM=nytrix")
      ne = ne.append("CLICOLOR=1")
      ne = ne.append("CLICOLOR_FORCE=1")
      ne = ne.append("FORCE_COLOR=1")
      if(window_id != 0){
...
fn _poll_child_exit(dict: vt): bool{
   ...
   def wr = __wait4(pid, status_ptr, 1) ;; WNOHANG
   free(status_ptr)
   if(wr == pid){
...
fn idle_sleep_ms(dict: vt, int: now_ticks=-1): int{
   ...
   if(_cursor_blink_enabled(vt)){
      def phase_ns = now_ticks % 500000000
      def remain_ns = 500000000 - phase_ns
...
fn update(dict: vt): dict{
   ...
   _vterm_pty_trace(nvt, "drain end=" + to_str(end) + " off=" + to_str(read_off) + " sz=" + to_str(sz))
   _vterm_pty_trace_bytes(nvt, t_buf, sz)
   ...
   def pty_trace = nvt.get("pty_trace", false)
   _vterm_clamp_cursor(nvt, st, co, ro)
   while(p < sz){
      def b = load8(t_buf, p) & 255
      if(pty_trace && p < 64){
         _vterm_pty_trace(nvt, "parse p=" + to_str(p) + " b=0x" + to_hex(b) + " esc=" + to_str(load32(st, OFF_ESC_STATE)) + " cx=" + to_str(load32(st, OFF_CX)) + " cy=" + to_str(load32(st, OFF_CY)) + " ulen=" + to_str(u_len))
      }
      if(u_len > 0){
         if((b & 0xC0) == 0x80){
         ...
         if(!pty_trace && load32(st, OFF_ESC_STATE) == 0 && load8(st, OFF_INSERT) == 0 && b >= 32 && b < 127){
            mut q = p + 1
            while(q < sz){
...
fn _kg_draw_visible_images(dict: vt,
   ...
   while(r <= ro){
      ...
      if(ry + ch < 0.0 || ry > wh){ r += 1 continue } ;; Cull
...
fn _is_space_like(int: u): bool{
   if(u == 0x00A0){ return true } ;; NO-BREAK SPACE
   if(u == 0x1680){ return true } ;; OGHAM SPACE MARK
   if(u >= 0x2000 && u <= 0x200A){ return true } ;; EN/EM/THIN/etc spaces
   if(u == 0x202F){ return true } ;; NARROW NO-BREAK SPACE
   if(u == 0x205F){ return true } ;; MEDIUM MATHEMATICAL SPACE
   if(u == 0x3000){ return true } ;; IDEOGRAPHIC SPACE
   if(u == 0x200B){ return true } ;; ZERO WIDTH SPACE -> render as normal space
   false
}
...
fn _diacritic_to_num(int: code): int{
   if(code == 0x305){ return 1 }
   if(code >= 0x30d && code <= 0x30e){ return code - 0x30d + 2 }
   if(code == 0x310){ return 4 }
   if(code == 0x312){ return 5 }
   if(code >= 0x33d && code <= 0x33f){ return code - 0x33d + 6 }
   if(code == 0x346){ return 9 }
   if(code >= 0x34a && code <= 0x34c){ return code - 0x34a + 10 }
   if(code >= 0x350 && code <= 0x352){ return code - 0x350 + 13 }
   if(code == 0x357){ return 16 }
   if(code == 0x35b){ return 17 }
   if(code >= 0x363 && code <= 0x36f){ return code - 0x363 + 18 }
   if(code >= 0x483 && code <= 0x487){ return code - 0x483 + 31 }
   if(code >= 0x592 && code <= 0x595){ return code - 0x592 + 36 }
   if(code >= 0x597 && code <= 0x599){ return code - 0x597 + 40 }
   if(code >= 0x59c && code <= 0x5a1){ return code - 0x59c + 43 }
   if(code >= 0x5a8 && code <= 0x5a9){ return code - 0x5a8 + 49 }
   if(code >= 0x5ab && code <= 0x5ac){ return code - 0x5ab + 51 }
   if(code == 0x5af){ return 53 }
   if(code == 0x5c4){ return 54 }
   if(code >= 0x610 && code <= 0x617){ return code - 0x610 + 55 }
   if(code >= 0x657 && code <= 0x65b){ return code - 0x657 + 63 }
   if(code >= 0x65d && code <= 0x65e){ return code - 0x65d + 68 }
   if(code >= 0x6d6 && code <= 0x6dc){ return code - 0x6d6 + 70 }
   if(code >= 0x6df && code <= 0x6e2){ return code - 0x6df + 77 }
   if(code == 0x6e4){ return 81 }
   if(code >= 0x6e7 && code <= 0x6e8){ return code - 0x6e7 + 82 }
   if(code >= 0x6eb && code <= 0x6ec){ return code - 0x6eb + 84 }
   if(code == 0x730){ return 86 }
   if(code >= 0x732 && code <= 0x733){ return code - 0x732 + 87 }
   if(code >= 0x735 && code <= 0x736){ return code - 0x735 + 89 }
   if(code == 0x73a){ return 91 }
   if(code == 0x73d){ return 92 }
   if(code >= 0x73f && code <= 0x741){ return code - 0x73f + 93 }
   if(code == 0x743){ return 96 }
   if(code == 0x745){ return 97 }
   if(code == 0x747){ return 98 }
   if(code >= 0x749 && code <= 0x74a){ return code - 0x749 + 99 }
   if(code >= 0x7eb && code <= 0x7f1){ return code - 0x7eb + 101 }
   if(code == 0x7f3){ return 108 }
   if(code >= 0x816 && code <= 0x819){ return code - 0x816 + 109 }
   if(code >= 0x81b && code <= 0x823){ return code - 0x81b + 113 }
   if(code >= 0x825 && code <= 0x827){ return code - 0x825 + 122 }
   if(code >= 0x829 && code <= 0x82d){ return code - 0x829 + 125 }
   if(code == 0x951){ return 130 }
   if(code >= 0x953 && code <= 0x954){ return code - 0x953 + 131 }
   if(code >= 0xf82 && code <= 0xf83){ return code - 0xf82 + 133 }
   if(code >= 0xf86 && code <= 0xf87){ return code - 0xf86 + 135 }
   if(code >= 0x135d && code <= 0x135f){ return code - 0x135d + 137 }
   if(code == 0x17dd){ return 140 }
   if(code == 0x193a){ return 141 }
   if(code == 0x1a17){ return 142 }
   if(code >= 0x1a75 && code <= 0x1a7c){ return code - 0x1a75 + 143 }
   if(code == 0x1b6b){ return 151 }
   if(code >= 0x1b6d && code <= 0x1b73){ return code - 0x1b6d + 152 }
   if(code >= 0x1cd0 && code <= 0x1cd2){ return code - 0x1cd0 + 159 }
   if(code >= 0x1cda && code <= 0x1cdb){ return code - 0x1cda + 162 }
   if(code == 0x1ce0){ return 164 }
   if(code >= 0x1dc0 && code <= 0x1dc1){ return code - 0x1dc0 + 165 }
   if(code >= 0x1dc3 && code <= 0x1dc9){ return code - 0x1dc3 + 167 }
   if(code >= 0x1dcb && code <= 0x1dcc){ return code - 0x1dcb + 174 }
   if(code >= 0x1dd1 && code <= 0x1de6){ return code - 0x1dd1 + 176 }
   if(code == 0x1dfe){ return 198 }
   if(code >= 0x20d0 && code <= 0x20d1){ return code - 0x20d0 + 199 }
   if(code >= 0x20d4 && code <= 0x20d7){ return code - 0x20d4 + 201 }
   if(code >= 0x20db && code <= 0x20dc){ return code - 0x20db + 205 }
   if(code == 0x20e1){ return 207 }
   if(code == 0x20e7){ return 208 }
   if(code == 0x20e9){ return 209 }
   if(code == 0x20f0){ return 210 }
   if(code >= 0x2cef && code <= 0x2cf1){ return code - 0x2cef + 211 }
   if(code >= 0x2de0 && code <= 0x2dff){ return code - 0x2de0 + 214 }
   if(code == 0xa66f){ return 246 }
   if(code >= 0xa67c && code <= 0xa67d){ return code - 0xa67c + 247 }
   if(code >= 0xa6f0 && code <= 0xa6f1){ return code - 0xa6f0 + 249 }
   if(code >= 0xa8e0 && code <= 0xa8f1){ return code - 0xa8e0 + 251 }
   if(code == 0xaab0){ return 269 }
   if(code >= 0xaab2 && code <= 0xaab3){ return code - 0xaab2 + 270 }
   if(code >= 0xaab7 && code <= 0xaab8){ return code - 0xaab7 + 272 }
   if(code >= 0xaabe && code <= 0xaabf){ return code - 0xaabe + 274 }
   if(code == 0xaac1){ return 276 }
   if(code >= 0xfe20 && code <= 0xfe26){ return code - 0xfe20 + 277 }
   if(code == 0x10a0f){ return 284 }
   if(code == 0x10a38){ return 285 }
   if(code >= 0x1d185 && code <= 0x1d189){ return code - 0x1d185 + 286 }
   if(code >= 0x1d1aa && code <= 0x1d1ad){ return code - 0x1d1aa + 291 }
   if(code >= 0x1d242 && code <= 0x1d244){ return code - 0x1d242 + 295 }
   0
}
...
fn _tcsihandle_fast(dict: vt, any: st, int: co, int: ro, any: g, any: pal, int: dfg, int: dbg, int: u, str: buf): any{
   ...
   def ag = parsed.get(0, [])
   def pr = priv == 63 ;; DEC private mode uses '?'
...
fn _tsetattr_fast(any: st, any: pal, int: dfg, int: dbg, list: ag): any{
   ...
   while(i < n){
      ...
      elif(a == 53){ store32(st, md | ATTR_STRIKE, OFF_CUR_MODE) } ;; overline → strikethrough visually
      elif(a >= 30 && a <= 37){
         mut idx = a - 30
...
fn _tsetmode_fast(dict: vt, any: st, int: co, int: ro, any: g, int: dfg, int: dbg, bool: pr, bool: set, list: ag): any{
   ...
   while(i < ag_n){
      ...
      if(pr){
         ...
         if(a == 4){ store8(st, set ? 1 : 0, OFF_INSERT) } ;; IRM: Insert/Replace Mode
         elif(a == 20){ ;; LNM: Linefeed/Newline mode -- treat LF as CR+LF
            mut m = load32(st, OFF_MODE)
            def LNM_BIT = 256
...
fn _kg_store_image_bytes(dict: vt,
   ...
   if(fmt == 24 || fmt == 32){
      ...
      if(fmt == 32){
         ...
         tex = vkr_create_texture_ex(w, h, bytes, 37) ;; RGBA8_UNORM
      } else {
...
fn send_input(dict: vt, str: s): any{
   _vterm_input_trace(vt, "send len=" + to_str(s.len))
...
fn _get_scratch(int: len): any{
   if(_scratch_cap < len + 128){
      ...
      _scratch_buf, _scratch_cap = malloc(len + 128), len + 128
   } _scratch_buf + 64
}

fn _cp_to_str(int: cp, any: acache): any{
   if(cp < 128){ return load64(acache, cp * 8) }
...
fn _vterm_draw_scrollbar(any: st, int: ro, int: hist_len, int: scroll_off, any: vis, any: ww, any: wh, any: px, any: py): any{
   ...
   def sb_w, sb_x = 6.0, float(ww) - sb_w - px
   def track_h = float(wh) - py * 2.0
   ...
   draw_rect(sb_x, 0.0, sb_w, float(wh), 0x22000000)
...
fn _vterm_draw_cpu(dict: vt, any: st, int: co, int: ro, any: g, list: history, int: hist_len, int: scroll_off, any: scroll_frac_r, any: cw, any: ch, any: wh, any: px, any: py, int: f_reg, any: gy_reg, any: db): any{
   ...
   while(r <= ro){
      ...
      if(ry + ch < 0.0 || ry > wh){ r += 1 continue }
      ...
      while(c < co){
         ...
         if(text_visible){
            draw_text(f_reg, _cp_to_str(cp, acache), rx, ry + gy_reg, rfg)
         }
         c += 1
      }
      r += 1
   ...
   if(load8(st, OFF_CURSOR_VISIBLE) != 0 && scroll_off == 0 && !sel_active){
      ...
      def now_t = ticks()
      def recent_key = (now_t - max(load64(st, OFF_LAST_KEY_T), load64(st, OFF_LAST_CHAR_T))) < 650000000
      def visible = recent_key || !do_blink || (fmod(float(now_t) / 500000000.0, 2.0) < 1.0)
      if(visible){
...
fn _vterm_trace_draw_state(dict: vt, any: st, int: co, int: ro, any: g, list: history, int: hist_len, int: scroll_off, int: db): dict{
   if(!vt.get("draw_trace", false)){ return vt }
   mut frames = int(vt.get("draw_trace_frames", 0))
   if(frames >= common.env_int_clamped("NY_TERM_DRAW_TRACE_FRAMES", 80, 1, 10000)){ return vt }
   print(
      "[vterm:draw] frame=" + to_str(frames)
      + " size=" + to_str(co) + "x" + to_str(ro)
      + " hist=" + to_str(hist_len)
      + " scroll=" + to_str(scroll_off)
      + " sel=" + to_str(load8(st, OFF_SEL_ACTIVE)) + "/" + to_str(load8(st, OFF_SEL_DRAGGING)) + "/" + to_str(load8(st, OFF_SEL_MOVED))
   )
   vt.set("draw_trace_frames", frames + 1)
}
...
fn draw(dict: vt, any: ww, any: wh): any{
   ...
   vt = _vterm_trace_draw_state(vt, st, co, ro, g, history, hist_len, scroll_off, db)
   set_unlit(true)
   def backend = get_active_backend()
   if(backend != BACKEND_MOCK){ vkr_bind_default_texture() }
   draw_rect(0, 0, ww, wh, db)
...
fn _vterm_handle_mouse_press(dict: vt, any: st, int: co, int: ro, list: history, int: hist_len, f64: cw, f64: ch, any: da): dict{
   ...
   _vterm_input_trace(vt, "select start x=" + to_str(sx) + " y=" + to_str(sy) + " abs_y=" + to_str(abs_sy) + " count=" + to_str(count))
   if(count == 1){
      store8(st, 0, OFF_SEL_ACTIVE)
...
fn _vterm_handle_mouse_motion(dict: vt, any: st, int: co, int: ro, int: hist_len, f64: cw, f64: ch, any: da): dict{
   ...
   ; Motion button masks can be stale during X11 warps, synthetic input, and
   ; some compositor transitions. Selection cleanup belongs to release events
   ; and the app-level lost-release guard, not to ordinary motion packets.
   def xy = _mouse_xy_clamped(da, cw, ch, co, ro)
   ...
   _vterm_input_trace(vt, "select drag x=" + to_str(ex) + " y=" + to_str(ey) + " abs_y=" + to_str(abs_ey) + " scroll=" + to_str(n_scroll))
   vt
}
   ...
   if(_mouse_is_left(native_btn)){
      if(load8(st, OFF_SEL_ACTIVE) != 0){
         ...
         if(moved || cnt >= 2){
            ...
            if(full_txt.len > 0){
               _vterm_input_trace(vt, "select copy len=" + to_str(full_txt.len))
      ...
      if(txt.len > 0){
         _vterm_input_trace(vt, "middle paste len=" + to_str(txt.len))
         _scroll_set(st, 0, hist_len)
...
fn _vterm_input_trace(dict: vt, str: msg): any{
   if(vt.get("input_trace", false)){ print("[vterm:input] " + msg) }
   0
}

fn _vterm_pty_trace(dict: vt, str: msg): any{
   if(vt.get("pty_trace", false)){ print("[vterm:pty] " + msg) }
   0
}

fn _vterm_pty_trace_bytes(dict: vt, any: buf, int: n): any{
   if(!vt.get("pty_trace", false)){ return 0 }
   mut out = ""
   mut i = 0
   def lim = min(n, 64)
   while(i < lim){
      if(i > 0){ out += " " }
      def b = load8(buf, i) & 255
      if(b < 16){ out += "0" }
      out += to_hex(b)
      i += 1
   }
   _vterm_pty_trace(vt, "bytes n=" + to_str(n) + " head=" + out)
   0
}
...
fn _vterm_printable_code(int: k, int: md): int{
   ...
   if(k == KEY_KP_DECIMAL){ return 46 }
   if(k == KEY_KP_DIVIDE){ return 47 }
   if(k == KEY_KP_MULTIPLY){ return 42 }
   if(k == KEY_KP_SUBTRACT){ return 45 }
   if(k == KEY_KP_ADD){ return 43 }
   if(k == KEY_KP_EQUAL){ return 61 }
   if(k > 126 && k < 256){ return k }
   -1
}

fn _vterm_send_code(dict: vt, any: st, int: hist_len, int: scroll_off, int: code, int: md): dict{
   if(code < 32 || code == 127){ return vt }
   _vterm_trace_cursor_state(vt, st, "send-code enter")
   _vterm_follow_cursor(vt, st, scroll_off, hist_len)
   _vterm_trace_cursor_state(vt, st, "send-code after follow")
   store64(st, ticks(), OFF_LAST_CHAR_T)
   _vterm_trace_cursor_state(vt, st, "send-code after last-char-t")
   store32(st, code, OFF_LAST_CHAR_C)
   _vterm_trace_cursor_state(vt, st, "send-code after last-char-c")
   if((md & MOD_ALT) != 0){ send_input(vt, "\033") }
   def text = _cp_to_str(code, vt.get("ascii_cache"))
   send_input(vt, text)
   if(vt.get("master_fd", -1) < 0){ return write(vt, text) }
   _vterm_trace_cursor_state(vt, st, "send-code after input")
   vt
}
...
fn _vterm_queue_printable(dict: vt, any: st, int: k, int: md, int: action): list{
   ...
   _vterm_trace_cursor_state(vt, st, "queue printable")
   _vterm_input_trace(vt, "queue printable cp=" + to_str(code) + " action=" + to_str(action))
   _vterm_key_hit(vt)
}
...
fn _vterm_flush_pending_printable(dict: vt, any: st): dict{
   ...
   _vterm_input_trace(vt, "flush printable cp=" + to_str(code) + " action=" + to_str(action))
...
fn _vterm_handle_key_release(dict: vt, any: st, any: da): dict{
   ...
   _vterm_input_trace(vt, "release key=" + to_str(k) + " mod=0x" + to_hex(md))
...
fn _vterm_handle_key_char(dict: vt, any: st, int: hist_len, int: scroll_off, any: da): dict{
   def c = da.get("char")
   if(c >= 32 && c != 127){
      _vterm_trace_cursor_state(vt, st, "char enter")
      if(load8(st, OFF_PENDING_PRINT) != 0){
         _vterm_input_trace(vt, "char cleared pending cp=" + to_str(load32(st, OFF_PENDING_CHAR)))
         _vterm_clear_pending_printable(st)
         _vterm_trace_cursor_state(vt, st, "char after clear pending")
      }
      ...
      _vterm_trace_cursor_state(vt, st, "char after active")
      def md = da.get("mod", 0)
      _vterm_input_trace(vt, "char cp=" + to_str(c) + " mod=0x" + to_hex(md))
      def out = _vterm_send_code(vt, st, hist_len, scroll_off, c, md)
      _vterm_trace_cursor_state(vt, st, "char after send-code")
      return out
   }
   vt
}
...
fn _vterm_handle_ctrl_shift_key(dict: vt, any: st, int: co, list: history, int: hist_len, int: scroll_off, int: k): list{
   if(k == KEY_V || k == KEY_INSERT){
      ...
      if(txt.len > 0){
         _vterm_follow_cursor(vt, st, scroll_off, hist_len)
         _vterm_input_trace(vt, "key paste len=" + to_str(txt.len))
         if(load8(st, OFF_BRACKET_PASTE) != 0){ send_input(vt, "\033[200~" + txt + "\033[201~") } else { send_input(vt, txt) }
      }
   ...
   if(k == KEY_C){
      ...
      co)
      if(full_txt.len > 0){
         _vterm_input_trace(vt, "key copy len=" + to_str(full_txt.len))
         vt = _vt_clip_set(vt, full_txt)
      }
      return _vterm_key_hit(vt)
   }
...
fn _vterm_handle_shift_insert(dict: vt, any: st, int: hist_len, int: scroll_off, int: k): list{
   ...
   if(txt.len > 0){
      _vterm_follow_cursor(vt, st, scroll_off, hist_len)
      _vterm_input_trace(vt, "shift-insert paste len=" + to_str(txt.len))
      if(load8(st, OFF_BRACKET_PASTE) != 0){ send_input(vt, "\033[200~" + txt + "\033[201~") } else { send_input(vt, txt) }
   }
   _vterm_key_hit(vt)
}
...
fn _vterm_handle_ctrl_insert(dict: vt, any: st, int: co, list: history, int: hist_len, int: k): list{
   ...
   if(load8(st, OFF_SEL_ACTIVE) != 0){
      ...
      co)
      if(full_txt.len > 0){
         _vterm_input_trace(vt, "ctrl-insert copy len=" + to_str(full_txt.len))
         vt = _vt_clip_set(vt, full_txt)
      }
   }
   _vterm_key_hit(vt)
...
fn _vterm_handle_key_press(dict: vt, any: st, int: co, list: history, int: hist_len, int: scroll_off, any: da): dict{
   ...
   _vterm_input_trace(vt, "press key=" + to_str(k) + " action=" + to_str(action) + " mod=0x" + to_hex(md))
   if(action != 2){
      def now = ticks()
...
fn handle_event(dict: vt, int: ty, any: da): dict{
   ...
   } elif(ty == EVENT_MOUSE_SCROLL){ ;; SCROLL_MARKER
