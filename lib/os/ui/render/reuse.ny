...
fn _warmup(any opts) int {
   if(is_dict(opts) && opts.contains("warmup")){
      return _opt_int(opts, "warmup", 4)
   }
   ui_profile.env_int_cached("NY_UI_GUI_IDLE_REUSE_WARMUP", 4, 2, 128)
}

fn _redraw_interval(any opts) int {
   if(is_dict(opts) && opts.contains("redraw_interval")){
      return _opt_int(opts, "redraw_interval", 120)
   }
   ui_profile.env_int_cached("NY_UI_GUI_IDLE_REUSE_REDRAW_INTERVAL", 120, 0, 1000000)
}
