...
fn _dbg_win(any: win, any: msg): any { if(_is_debug()){ ui_profile.print_text("[ui:window] win=0x" + to_hex(win) + " " + msg) } }
...
fn _map_backend_key(any: raw_key): int { _normalize_key(raw_key) }
...
fn open_window(str: name, int: x, int: y, int: w, int: h, int: flags=0): any {
   ...
   if(common.env_truthy("NY_UI_HEADLESS")){
      ...
      _dbg("open_window: NY_UI_HEADLESS forcing flags=0x" + to_hex(flags) + " pos=(" + to_str(x) + "," + to_str(y) + ")")
   }
   ...
   if(handle){
      _ensure_window_registry()[handle] = win
...
fn push_event(any: win, int: kind, any: data=0): bool {
   ...
   } elif(kind == EVENT_KEY_PRESSED){
      mut raw_key = KEY_NULL
      if(is_dict(event_data)){ raw_key = event_data.get("key", event_data.get("raw_key", KEY_NULL)) }
      elif(is_int(event_data)){ raw_key = event_data }
      elif(is_float(event_data)){ raw_key = int(event_data) }
      mut nk = _normalize_key(raw_key)
      if(nk == KEY_NULL && is_dict(event_data)){
         def sc = _event_scancode(event_data)
         if(sc > 0){
            def fk = _function_key_from_scancode(sc)
            if(fk > 0){ nk = fk }
         }
      }
      if(nk != KEY_NULL && nk == exit_key(real_win)){ real_win["should_close"] = true }
   }
...
fn check_event(any: win): any {
   ...
   if(q.len == 0){
      ...
      if(state_dirty){ _ensure_window_registry()[handle] = cw }
      if(native_events){
         ...
         if(is_list(polled_events) && polled_events.len > 0){
            ...
            _ensure_window_registry()[handle] = cw
         }
      } else {
         update_input(win)
...
fn set_blit_handler(any: h): any { _blit_hook = h }

fn blit_buffer(any: win, any: buf, int: w, int: h): any { win = _get_win(win) if(_is_window(win) && is_ptr(buf) && _blit_hook){ _blit_hook(buf, w, h) } 0 }
...
fn scroll_pos(any: win): list { win = _get_win(win) if(!_is_window(win)){ return [0.0, 0.0] } [win.get("scroll_x", 0.0), win.get("scroll_y", 0.0)] }
...
fn set_window_resizable(any: win, bool: val): any {
...
fn set_window_decorated(any: win, bool: val): any {
...
fn set_window_floating(any: win, bool: val): any {
