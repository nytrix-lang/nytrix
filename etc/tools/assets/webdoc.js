...
function renderNav() {
    ...
    mdDocs.sort((docA, docB) => {
        const priority = ["README", "LANGUAGE", "PROPOSALS", "DOCKER", "TODO"];
    ...
    function renderTree(node, depth = 0, currentPath = "") {
        ...
        const sortedKeys = Object.keys(node).sort((a, b) => {
            const priority = ["Home", "README", "LANGUAGE", "PROPOSALS", "DOCKER", "TODO"];
        ...
        sortedKeys.forEach(key => {
            ...
            if (key === "Home") {
                ...
                nodeHtml += `<div class="gtitle ${mdDocs.length ? "collapsible" : ""} ${isActive ? "active" : ""}" style="padding-left: ${12 + depth * 10}px;" ${mdDocs.length ? `onclick="toggleNavCategory('${categoryId}')"` : ""}>${esc(key)}${mdDocs.length ? `<span class="collapse-icon ${isActive ? "active" : ""}">▶</span>` : ""}</div>`;
                if (mdDocs.length) {
...
function highlightNytrixCode(code) {
    if (!code) return "";
    if (code.includes("sudo ") || code.includes("xdg-open") || code.includes("#!/") || code.includes("make ")) return esc(code);
...
function renderOverviewPage(overviewMod, specificDocName = null) {
    ...
    const docsToRender = specificDocName ? overviewMod.markdown_docs.filter(d => d.name === specificDocName) : overviewMod.markdown_docs;
    docsToRender.forEach(doc => {
        html += `<div class="md-section" id="md-${doc.name}"><div class="md-title">${esc(doc.name)}</div><div class="markdown-content">${marked.parse(doc.html)}</div></div>`;
    });
    $("content").innerHTML = html;
    document.querySelectorAll(".markdown-content pre code").forEach(block => {
        const className = block.className || "";
        if (className.includes("language-ny") || className.includes("language-nytrix") || (!className.includes("language-") && (block.textContent.includes("fn ") || block.textContent.includes("def ")))) {
