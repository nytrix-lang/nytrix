...
function renderNav() {
    ...
    function renderInfoSection(depth) {
        ...
        let html = "";
        html += `<div class="gtitle collapsible ${infoActive ? "active" : ""}" style="padding-left: ${12 + depth * 10}px;" onclick="toggleNavCategory('${infoId}')">INFO <span class="collapse-icon">▸</span></div>`;
    ...
    function renderTree(node, depth = 0, currentPath = "") {
        ...
        sortedKeys.forEach(key => {
            ...
            if (key === "Home") {
                ...
                nodeHtml += `<div class="gtitle ${hasHomeDocs ? "collapsible" : ""} ${isActive ? "active" : ""}" style="padding-left: ${12 + depth * 10}px;" ${hasHomeDocs ? `onclick="toggleNavCategory('${categoryId}')"` : ""}>${esc(key)}${hasHomeDocs ? `<span class="collapse-icon">▸</span>` : ""}</div>`;
                if (hasHomeDocs) {
            ...
            if (isDirectModule) {
                ...
                nodeHtml += `<div class="gtitle collapsible ${moduleActive ? "active" : ""}" style="padding-left: ${12 + depth * 10}px;" onclick="toggleNavModule('${moduleId}')"><span onclick="selectModule('${m.name.replace(/'/g, "\\\\'")}');event.stopPropagation();" style="cursor: pointer; flex: 1;">${esc(key)}</span><span class="collapse-icon">▸</span></div>`;
                ...
                nodeHtml += `<div class="gtitle ${hasChildren ? "collapsible" : ""} ${isActive ? "active" : ""}" style="padding-left: ${12 + depth * 10}px;" ${hasChildren ? `onclick="toggleNavCategory('${categoryId}')"` : ""}><span onclick="selectModule('${navTarget.replace(/'/g, "\\\\'")}');event.stopPropagation();" style="cursor: pointer; flex: 1;">${esc(key)}</span>${hasChildren ? `<span class="collapse-icon">▸</span>` : ""}</div>`;
                ...
                currentCategory._modules.sort((a, b) => a.name.localeCompare(b.name)).forEach(m => {
                    ...
                    if (hasSymbols) {
                        nodeHtml += `<div class="gtitle collapsible ${moduleActive ? "active" : ""}" style="padding-left: ${12 + (depth + 1) * 10}px;" onclick="toggleNavModule('${moduleId}')"><span onclick="selectModule('${m.name.replace(/'/g, "\\\\'")}');event.stopPropagation();" style="cursor: pointer; flex: 1;">${esc(moduleShortName)}</span><span class="collapse-icon">▸</span></div>`;
                        nodeHtml += `<div id="${moduleId}" class="collapsible-content ${moduleActive ? "active" : ""}">`;
                        m.symbols.sort((a, b) => a.name.localeCompare(b.name)).forEach(s => {
                            const isSymbolCurrent = current && current.includes("::") && current.split("::")[0] === m.name && (current.split("::")[1] === s.name || current.split("::")[1] === s.id);
                            const isFunction = s.kind === "function";
