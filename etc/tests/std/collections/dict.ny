...
fn test_resize(){
   ...
   i = 0
   while(i < 50){
      assert(dict_get(d, i, -1) == i * 10, "Get value after resize")
      i = i + 1
   }
   print("Resize passed")
}
...
fn test_copy_update(){
   ...
   dict_set(d1, "a", 1)
   def d2 = dict_copy(d1)
   assert(dict_get(d2, "a", 0) == 1, "Copy has item")
   dict_set(d2, "b", 2)
   assert(!contains(d1, "b"), "Original unmodified by copy modification")
   def d3 = dict()
   dict_set(d3, "c", 3)
   dict_update(d1, d3)
   assert(dict_get(d1, "c", 0) == 3, "Update from dict")
   ; Update from list of pairs
   def pairs = [["d", 4], ["e", 5]]
   dict_update(d1, pairs)
   assert(dict_get(d1, "d", 0) == 4, "Update from list pair 1")
   assert(dict_get(d1, "e", 0) == 5, "Update from list pair 2")
...
fn test_mixed_types(){
   ...
   def d = dict()
   d = dict_set(d, 123, "int")
   d = dict_set(d, "123", "str")
   assert(dict_get(d, 123, 0) == "int", "Int key")
   assert(dict_get(d, "123", 0) == "str", "Str key")
...
fn test_stress_cycle(){
   ...
   while(i < 100){
       if(i % 2 != 0){
          assert(contains(d, i), "Odd key present")
       } else {
          assert(!contains(d, i), "Even key removed")
       }
       i = i + 1
   }
   ; Re-add some deleted
   dict_set(d, 0, 999)
   assert(contains(d, 0), "Re-added key 0")
   assert(dict_get(d, 0, -1) == 999, "Re-added value correct")
...
fn test_clear(){
   ...
   assert(!contains(d, "a"), "Item a gone")
   dict_set(d, "c", 3)
