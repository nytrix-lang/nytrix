...
fn test_defaults_and_surface(){
   ...
   assert(is_list(tail) && len(tail) == 0,
...
fn test_registry_local_ops(){
   ...
   assert(len(macros) == 2, "local macro list should keep explicit registrations")
   assert(get(macros, 0, "") == "double",
               "first local macro should preserve registration order")
   assert(get(macros, 1, "") == "add2",
               "second local macro should preserve registration order")

   def attrs = syntax.list_attributes_in(reg)
   assert(len(attrs) == 1 && get(attrs, 0, "") == "mark",
   ...
   assert(len(macros2) == 1 && get(macros2, 0, "") == "add2",
...
fn test_registry_global_ops(){
   ...
   assert(dict_get(marked, "marked", false),
               "global attribute application should run custom handler")
   assert(dict_get(marked, "arg_count", 0) == 2,
...
fn test_form_expansion_surface(){
   ...
   assert(len(tail) == 1 && get(tail, 0, 0) == 21,
   ...
   assert(get(deep1, 1, 0) == 42, "nested direct form should expand")
   def inner = get(deep1, 2, list(0))
   assert(is_list(inner), "nested list should remain list")
   assert(get(inner, 0, 0) == 6, "first nested form should expand")
   assert(get(inner, 1, 0) == 10, "second nested form should expand")
   ...
   assert(dict_get(limited, "name", "") == "pong",
...
fn test_attribute_surface_and_fallback(){
   ...
   assert(!dict_get(unchanged, "marked", false),
...
fn test_rewriter_surface(){
   ...
   assert(len(names) == 2, "rewriter should expose deterministic pass list")
   assert(get(names, 0, "") == "add1", "first rewrite should preserve order")
   assert(get(names, 1, "") == "mul2", "second rewrite should preserve order")
