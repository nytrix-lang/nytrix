...
assert(Z(4).sqrt_mod(17).powmod(2, 17).str == Z(4).str, "bigint modular sqrt method")
def qroots = Z(1).quadratic_roots_mod(0, -4, 17)
assert(qroots.len == 2 || qroots.len == 0, "bigint quadratic roots method")
...
assert(__bigint_iroot(Z(27), 3).str == "3", "__bigint_iroot direct")
...
assert(int(1.9).long.str == "1", "float .long")
def list_long_bytes = _core_bigint_to_bytes([1, 2, 3].long)
assert(type_shape(list_long_bytes) != nil, ".long.bytes keeps typed byte list")
assert(list_long_bytes.len == 3, ".long bytes len")
assert(list_long_bytes.get(0) == 3 && list_long_bytes.get(1) == 5 && list_long_bytes.get(2) == 7, ".long bytes values")
assert(_core_bigint_to_bytes("010203".unhex.long).get(0) == 3 && _core_bigint_to_bytes("010203".unhex.long).get(1) == 5 && _core_bigint_to_bytes("010203".unhex.long).get(2) == 7, ".unhex.long roundtrip")
assert(_core_bigint_to_bytes(_core_bigint_to_bytes([1, 2, 3].long).long).len == 3, "bigint bytes aliases chain")
