...
def c2 = Circle(radius: 2)
def r = Shape.Rect(width: 3, height: 7)
...
assert_eq(positive_radius(Shape.Circle(radius: 9)), 9, "ADT guard match")
assert_eq(positive_radius(Shape.Circle(radius: -2)), 0, "ADT wildcard payload field")
def xs = [Shape.Circle(radius: 5), Shape.Empty]
assert_eq(area(xs[0]), 25, "ADT in list")
assert_eq(area(xs[1]), 0, "payload-less ADT in list")
assert_eq(Shape.Circle(radius: 5).method_area(), 25, "payload ADT method")
...
assert_eq(unwrap_or_zero(Option.Some(value: 12)), 12, "generic ADT payload")
assert_eq(unwrap_or_zero(Option.None), 0, "generic ADT empty variant")
def Option<int>: typed_option = Option.Some(value: 41)
match typed_option {
   Option.Some(value: v) -> assert_eq(v + 1, 42, "generic ADT match refines payload type")
