...
def UI_DENSITY = _env_float_between("NY_EDITOR_DENSITY", 0.94, 0.62, 1.35)
...
def TOP_H = max(20.0, 32.0 * UI_DENSITY)
def RAIL_MIN_W = 48.0
def EDIT_MIN_W = 56.0
def DOCK_MIN_H = 40.0
def EDIT_MIN_H = 28.0
...
def SECTION_ICON_Y = 6.0
def SECTION_TEXT_Y = 8.0
def ROW_TEXT_Y = 5.0
def EDITOR_TEXT_TOP = 14.0
...
def RAIL_TABS_H = 30.0
def CONTEXT_W = 330.0
def CONTEXT_ROW_H = 38.0
def FIND_BAR_H = 86.0
def f64: PALETTE_HEADER_H = 76.0
def f64: PALETTE_ROW_H = 36.0
def f64: PALETTE_DETAIL_H = 46.0
def int: PALETTE_MAX_ROWS = 14
def f64: WHICH_KEY_ROW_H = 38.0
def int: WHICH_KEY_MAX_ROWS = 36
def TOP_TAB_MIN_W = 86.0
def TOP_TAB_MAX_W = 188.0
def MOUSE_BACK = 3
def MOUSE_FORWARD = 4
def NAV_LIMIT = 96
def NAV_SEP = "\t"
...
def TERM_TAB_H = 26.0
def TERM_TAB_MIN_W = 82.0
def TERM_TAB_MAX_W = 156.0
def PANEL_SPLIT_HIT = 6.0
...
fn _editor_reuse_opts(f64 sw, f64 sh) dict {
   def any: opts = {
      ...
      "warmup": 4,
      "redraw_interval": common.env_int_clamped("NY_EDITOR_IDLE_REUSE_INTERVAL", 1000, 1, 100000)
   }
   opts
}
...
fn _append_right_run(list out, int font, str text, f64 right_x, f64 y, int color) list {
   if(text.len <= 0){ return out }
   def w = float(gfx.measure_text_fast(font, text).get(0, widgets.text_w(font, text, 0, 0, 7.0)))
   _append_text_run(out, text, right_x - w, y, color)
}

fn _append_center_run(list out, int font, str text, f64 cx, f64 y, int color) list {
   if(text.len <= 0){ return out }
   def w = float(gfx.measure_text_fast(font, text).get(0, 0.0))
...
fn _draw_editor(dict lay, bool caret) int {
   ...
   if(_draw_image_preview(lay, ed.current_buffer(st))){ return 0 }
   def text_x = _line_text_x(lay)
   def filename = to_str(ed.current_buffer(st).get("name", ""))
   def scroll = int(st.get("scroll", 0))
   mut line_runs = []
   mut text_runs = []
...
while(!gfx.window_should_close(win)){
   ...
   term_state = termpane.update(term_state)
