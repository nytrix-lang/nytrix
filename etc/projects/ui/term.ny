...
fn _refresh_update_budget(any now_ticks) any {
   ...
   _max_events_per_frame = common.env_int_clamped("NY_TERM_MAX_EVENTS_PER_FRAME", 160, 32, 1024)
   _input_event_budget_ns = common.env_int_clamped("NY_TERM_INPUT_EVENT_BUDGET_MS", 2, 1, 12) * 1000000
...
fn _event_budget_done(any event_start_ticks, int event_count) bool {
   event_count > 0 && (event_count % 32) == 0 && (ticks() - event_start_ticks) >= _input_event_budget_ns
}
