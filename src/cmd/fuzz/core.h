#ifndef NYTRIX_CORE_H
#define NYTRIX_CORE_H
...
#define NYTRIX_PRINTF_FMT(fmt_index, first_arg) \
  __attribute__((format(printf, fmt_index, first_arg)))
#else
#define NYTRIX_PRINTF_FMT(fmt_index, first_arg)
#endif

#define NYTRIX_FUZZ_ALL_QUICK_JQ_EXPR \
...
#define NYTRIX_FUZZ_ALL_QUICK_JQ_STATUS \
  "jq " NYTRIX_FUZZ_ALL_QUICK_JQ_EXPR " build/fuzz/all/status.json"
#define NYTRIX_FUZZ_ALL_STATE_JQ_EXPR \
...
#define NYTRIX_FUZZ_ALL_STATE_JQ_DEFAULT \
  "jq " NYTRIX_FUZZ_ALL_STATE_JQ_EXPR " build/fuzz/all/run-next-state.json"
#define NYTRIX_FUZZ_ALL_OLD_PATH_PROBE_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix fuzz all old-paths --dry-run --probe --nytrix-root ../nytrix --archive-dir build/cache/old-nytrix --json build/fuzz/all/old-paths.json --markdown build/fuzz/all/old-paths.md"
#define NYTRIX_FUZZ_ALL_OLD_PATH_DRY_RUN_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix fuzz all old-paths --dry-run --nytrix-root ../nytrix --archive-dir build/cache/old-nytrix --json build/fuzz/all/old-paths.json --markdown build/fuzz/all/old-paths.md"
#define NYTRIX_FUZZ_ALL_OLD_PATH_APPLY_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix fuzz all old-paths --apply --wait-writers-s 300 --nytrix-root ../nytrix --archive-dir build/cache/old-nytrix --json build/fuzz/all/old-paths.json --markdown build/fuzz/all/old-paths.md"
#define NYTRIX_FUZZ_ALL_KNOWN_BUGS_REPORT \
  "build/fuzz/ultra/compiler-known-bugs.json"
#define NYTRIX_FUZZ_ALL_KNOWN_BUGS_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix compiler known-bugs --timeout-s 15 --json " NYTRIX_FUZZ_ALL_KNOWN_BUGS_REPORT
#define NYTRIX_FUZZ_ALL_KNOWN_BUGS_PROBE \
  "jq {ok,cases,ok_count,failure_count,known_bug_count,known_bug_reproduced,known_bug_fixed_candidates,known_bug_lost_signal,known_bug_baseline_failures} " NYTRIX_FUZZ_ALL_KNOWN_BUGS_REPORT
#define NYTRIX_FUZZ_ALL_PERF_TRIAGE_REPORT \
  "build/fuzz/ultra/perf-triage-current.json"
#define NYTRIX_FUZZ_ALL_PERF_TRIAGE_MARKDOWN \
  "build/fuzz/ultra/perf-triage-current.md"
#define NYTRIX_FUZZ_ALL_PERF_TRIAGE_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix perf triage --fast --limit 5 --threshold 1.50 --json " NYTRIX_FUZZ_ALL_PERF_TRIAGE_REPORT " --markdown " NYTRIX_FUZZ_ALL_PERF_TRIAGE_MARKDOWN
#define NYTRIX_FUZZ_ALL_PERF_TRIAGE_PROBE \
  "jq {ok,cases,ok_count,failure_count,perf_hotspots,perf_worst_ratio,perf_worst_case,perf_worst_slowdown_percent} " NYTRIX_FUZZ_ALL_PERF_TRIAGE_REPORT
#define NYTRIX_FUZZ_ALL_SELFTEST_FOCUSED_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix selftest run --only fuzz_all_help --json build/fuzz/all/selftest-fuzz_all_help.json --markdown build/fuzz/all/selftest-fuzz_all_help.md"
#define NYTRIX_FUZZ_ALL_SELFTEST_TEMPLATE_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix selftest run --only NAME --json build/fuzz/all/selftest-NAME.json --markdown build/fuzz/all/selftest-NAME.md"
#define NYTRIX_FUZZ_ALL_SELFTEST_FULL_COMMAND \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix selftest run --full --json build/fuzz/all/selftest-full.json --markdown build/fuzz/all/selftest-full.md"
#define NYTRIX_FUZZ_ALL_SELFTEST_CATALOG \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix selftest run --list --json build/fuzz/all/selftest-catalog.json --markdown build/fuzz/all/selftest-catalog.md"
#define NYTRIX_FUZZ_ALL_SELFTEST_PROBE \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix selftest run --list --probe --json build/fuzz/all/selftest-catalog.json --markdown build/fuzz/all/selftest-catalog.md"
#define NYTRIX_FUZZ_ALL_SELFTEST_RUN \
  "env NYTRIX_LOW_PRIORITY=1 NYTRIX_RUN_NICE=10 nice -n 10 ./build/nytrix selftest run --only fuzz_all_help --only fuzz_all_audit --only fuzz_all_default_pressure --only fuzz_all_coverage_commands --only fuzz_all_coverage_focus_companions --only fuzz_all_history_commands --only fuzz_all_preflight_isolation --only fuzz_all_status_canonical --only fuzz_all_status_stale_evidence --only fuzz_all_repeat_status_progress --only fuzz_all_reporting --only fuzz_all_fresh_handoff --only fuzz_all_progress_canonical --only fuzz_all_progress --only fuzz_all_progress_stale_evidence --only fuzz_all_progress_refresh_fail --only fuzz_all_full_pressure_remediation --only fuzz_repro_ready_missing_wrapper --only fuzz_repro_ready_missing_command --only fuzz_all_plan_coverage_next --only fuzz_all_old_paths --only fuzz_all_old_paths_dry_run --only fuzz_all_old_paths_empty_dry_run --only fuzz_all_old_writer_classifier --only compiler_findings --only compiler_known_bugs --only compiler_std_audit --only perf_triage_args --json build/fuzz/all/selftest-cockpit.json --markdown build/fuzz/all/selftest-cockpit.md"
#define NYTRIX_FUZZ_ALL_SELFTEST_COCKPIT_PROBE \
...
bool sb_appendf(str_buf_t *b, const char *fmt, ...) NYTRIX_PRINTF_FMT(2, 3);
