...
static const PerfCase k_cases[] = {
    {"etc/tests/bench/binary.ny", "compile"},
    {"etc/tests/bench/dict.ny", "balanced"},
    {"etc/tests/bench/fibonacci.ny", "speed"},
    {"etc/tests/bench/float.ny", "speed"},
    {"etc/tests/bench/intops.ny", "speed"},
    {"etc/tests/bench/iter.ny", "balanced"},
    {"etc/tests/bench/list.ny", "balanced"},
    {"etc/tests/bench/mandelbrot.ny", "speed"},
    {"etc/tests/bench/sieve.ny", "size"},
    {"etc/tests/bench/spectral.ny", "speed"},
    {"etc/tests/bench/vector.ny", "speed"},
};
...
static const PerfCompareVariant k_compare_variants[] = {
    ...
    {"ny-o2-compile-stdops", "-O2", "--std-builtin-ops", "compile"},
    {"ny-o2-balanced-stdops", "-O2", "--std-builtin-ops", "balanced"},
    {"ny-o3-speed-stdops", "-O3", "--std-builtin-ops", "speed"},
    {"ny-default-no-stdops", NULL, "--no-std-builtin-ops", NULL},
    {"ny-o2-compile-no-stdops", "-O2", "--no-std-builtin-ops", "compile"},
    {"ny-o2-balanced-no-stdops", "-O2", "--no-std-builtin-ops", "balanced"},
    {"ny-o3-speed-no-stdops", "-O3", "--no-std-builtin-ops", "speed"},
};
...
static int write_compare_reports(const char *out_root, const PerfCompareRow *rows, int row_count,
  ...
  if (!csv)
    return 1;
  fputs("case,variant,ok,rc,wall_ms,benchmark_ms,total_ms,codegen_ms,opt_ms,jit_compile_ms,jit_run_ms,suspect,stdout,stderr\n", csv);
  ...
  fprintf(md, "| rank | case | variant | wall ms | bench ms | total ms | codegen ms | opt ms | jit compile ms | jit run ms | suspected subsystem |\n");
  fprintf(md, "| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |\n");
...
static int run_compare_mode(const char *bin, const char *out_root,
  ...
  int case_count = PERF_CASE_COUNT;
  ...
  for (int i = 0; i < case_count; i++) {
    for (int v = 0; v < PERF_COMPARE_VARIANT_COUNT; v++) {
      ...
      case_name_from_path(row->case_name, sizeof(row->case_name), k_cases[i].path);
      nyt_path_copy(row->path, sizeof(row->path), k_cases[i].path);
      ...
      for (int s = 0; s < samples; s++) {
        ...
        double elapsed_ms = 0.0;
        int rc = run_one_compare_ny(bin, k_cases[i].path, &k_compare_variants[v],
...
static void usage(void) {
  ...
  printf("  %sny perf profile etc/tests/bench/sieve.ny -- --bench%s\n", nyt_clr(NYT_CYAN), nyt_clr(NYT_RESET));
