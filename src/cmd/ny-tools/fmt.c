...
static int is_expected_error_fixture(const char *path) {
  ...
  if (!normalized_repo_path(path, rel, sizeof(rel)))
    return 0;
  return strncmp(rel, "etc/tests/error/", 16) == 0;
}
    ...
    if (find_outside_string(line, "malloc(")) {
      ...
      if (!guarded) {
        issue_push(issues, path, ln, 1, "NYAUD2005", "warning",
                   ...
                   "check allocation and make ownership visible near the allocation site, preferably with ptr: name = malloc(...) { ... }");
        stats->findings++;
      }
    }
...
static void audit_scan_file(const char *path, FnVec *fns, FilePressureVec *files,
  ...
  if ((audit_wants(mode, "legacy") || audit_wants(mode, "methods")) && receiver_rewrites > 0) {
    ...
    issue_push(issues, path, receiver_rewrite_first ? receiver_rewrite_first : 1, 1,
               ...
               "prefer s.to_bytes, h.unhex, b.hex, b.text, b.long, a.concat(b), and n.bytes over legacy byte helper calls");
    stats->findings++;
    stats->trim_targets++;
  }
  if (audit_wants(mode, "legacy") && fp.legacy_calls > 0) {
    ...
    issue_push(issues, path, 1, 1, "NYAUD8001", fp.legacy_calls >= 20 ? "warning" : "note",
               msg, "prefer receiver methods/properties such as d.get(k), b.hex, x.len, and indexed assignment x[i] = value");
    stats->findings++;
  }
  if (audit_wants(mode, "types") && (fp.untyped_params > 0 || fp.missing_returns > 0)) {
    ...
    issue_push(issues, path, 1, 1, "NYAUD9001",
               ...
               msg, "add type-first params/returns so attached method lookup, property syntax, and codegen specialization stay precise");
    stats->findings++;
  }
...
static void print_audit_json(const AuditStats *s, IssueVec *issues, FilePressureVec *files,
  ...
  for (size_t i = 0; i < nf; i++) {
    ...
    printf("    {\"file\": ");
    json_str(f->file);
    printf(", \"score\": %d, \"lines\": %d, \"bytes\": %d, \"dict_gets\": %d, \"legacy_calls\": %d, \"untyped_params\": %d, \"missing_returns\": %d, \"repeated_lines\": %d}%s\n",
           f->score, f->lines, f->bytes, f->dict_gets, f->legacy_calls, f->untyped_params,
           f->missing_returns, f->repeated_lines, (i + 1 == nf) ? "" : ",");
  }
  printf("  ],\n");
  ...
  for (size_t i = 0; i < nfn; i++) {
    ...
    printf(", \"name\": ");
    json_str(f->name);
    printf(", \"line\": %d, \"score\": %d, \"lines\": %d, \"dict_gets\": %d, \"legacy_calls\": %d, \"untyped_params\": %d, \"missing_return\": %d}%s\n",
           f->line, f->score, f->lines, f->dict_gets, f->legacy_calls, f->untyped_params,
           f->missing_returns, (i + 1 == nfn) ? "" : ",");
  }
  printf("  ],\n");
  printf("  \"issues\": [\n");
  size_t printed = 0;
...
static int parse_args(int argc, char **argv, FmtOpts *o) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (audit_mode) {
      ...
      /* compatibility with the old audit similarities CLI */
    } else if (strcmp(a, "--root") == 0 && i + 1 < argc) {
      i++;
    } else if (strncmp(a, "--root=", 7) == 0) {
      /* ny-fmt already locates and chdirs to the repo root */
