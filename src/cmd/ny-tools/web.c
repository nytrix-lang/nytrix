...
static int path_has_suffix(const char *path, const char *suffix) {
  if (!path || !suffix)
    return 0;
  size_t pn = strlen(path), sn = strlen(suffix);
  return pn >= sn && strcmp(path + pn - sn, suffix) == 0;
}
...
static char *leading_comments(const char *s, size_t start) {
  ...
  while (line_end > 0) {
    ...
    if (ln >= 2 && line[0] == ';' && line[1] == ';') {
      ...
      /* Ignore blank separators between a doc comment block and a declaration.
       */
    } else {
      break;
    }
...
static char *leading_file_comments(const char *s, size_t n) {
  ...
  for (size_t p = 0; p < n;) {
    ...
    if (ln >= 2 && line[0] == ';' && line[1] == ';') {
      ...
      /* Keep scanning across blank separators in the file header. */
    } else {
      break;
    }
    p = e < n ? e + 1 : e;
...
/* Refine texi-generated HTML: remove nav chrome, inline styles, and link
 * annotations that do not belong in the embedded doc view.
 * Each transform frees its input and returns a fresh allocation so callers
 * never hold stale pointers across the chain. */
static char *refine_info_html(char *html) {
  sb_t out = {0};

  /* Strip inline margin overrides added by makeinfo. */
  ...
  /* Drop the navigation panel, boilerplate footer text, and copiable anchors.
   */
...
static void append_imports_json(sb_t *json, const char *body, size_t n) {
  ...
  for (size_t i = 0; i + 4 < n; i++) {
    ...
    if (p < n && (body[p] == '"' || body[p] == '\'')) {
      ...
      target = strndup0(body + start, p - start);
      if (p < n)
        p++;
    } else {
      size_t start = p;
    ...
    if (p < n && body[p] == '(') {
      ...
      while (p < n && body[p] != ')') {
        ...
        if (p == ss)
          break;
        char *sym = strndup0(body + ss, p - ss);
        char *alias = NULL;
        while (p < n && isspace((unsigned char)body[p]))
          p++;
...
static int ny_doc_search_main(int argc, char **argv, int get_one) {
  ...
  int include_docs = 1;
  int include_symbols = 1;
  int limit = get_one ? 1 : 20;
  sb_t query_sb = {0};
  char err[256];
  ...
  for (int i = 2; i < argc; i++) {
    ...
    if (ny_arg_match(a, "--input", "-i")) {
      ...
      if (!ny_arg_take_int(a, &i, argc, argv, 1, 200, &limit, "limit", err,
                           sizeof(err))) {
        nyt_err("ny-doc", "%s", err);
        return 2;
      }
  ...
  if (!build_search_index(&idx, root, input, include_docs, include_symbols,
                          &source_files, &modules, &symbols)) {
  ...
  if (get_one) {
    if (match_n == 0) {
      nyt_err("ny-doc", "no docs or symbols matched '%s'", query);
      free(matches);
      doc_search_index_free(&idx);
      free(query);
      return 1;
    ...
    printf("%sny doc%s search '%s' %s(%zu matches, %zu indexed",
           nyt_clr(NYT_BOLD), nyt_clr(NYT_RESET), query, nyt_clr(NYT_GRAY),
           match_n, idx.len);
    if (include_symbols)
      printf(", %d modules, %d symbols, %d source files", modules, symbols,
             source_files);
    printf(")%s\n", nyt_clr(NYT_RESET));
    size_t shown = match_n < (size_t)limit ? match_n : (size_t)limit;
    for (size_t i = 0; i < shown; i++)
      print_search_result(matches[i], (int)i + 1, query);
    if (shown < match_n)
...
static char *render_site_head(const char *site, const char *root) {
  ...
  const char *doc_url =
      (root_url.data && *root_url.data) ? root_url.data : repo_url;
...
int ny_web_main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (ny_arg_match(a, "--output", "-o")) {
      ...
      if (!ny_arg_take_int(a, &i, argc, argv, 1, 65535, &port, "port", err,
                           sizeof(err))) {
        nyt_err("ny-doc", "%s", err);
        return 2;
      }
  ...
  if (!mkdir_p(out_dir)) {
    free(bundle);
    nyt_err("ny-doc", "failed to create output dir: %s", out_dir);
    return 1;
  }
  ...
  char *html_tpl = ny_read_file_raw(template_path, NULL);
  char *js_tpl = ny_read_file_raw(js_path, NULL);
  char *css_tpl = ny_read_file_raw(css_path, NULL);
  if (!html_tpl) {
    free(bundle);
    free(js_tpl);
    free(css_tpl);
    nyt_err("ny-doc", "failed to read template: %s", template_path);
    return 1;
  }
  if (!js_tpl)
    js_tpl = strdup("");
  ...
  if (!write_file(cache_web_path, html)) {
    free(html);
    free(docs_json);
    nyt_err("ny-doc", "failed writing %s", cache_web_path);
    return 1;
  }
  if (!write_file(index_path, html)) {
    free(html);
    free(docs_json);
    nyt_err("ny-doc", "failed writing %s", index_path);
    return 1;
  }
  free(html);
  ...
  if (ny_path_readable(logo_png_path) &&
      !copy_file_bytes(logo_png_path, out_logo_png_path)) {
    nyt_err("ny-doc", "failed writing %s", out_logo_png_path);
    return 1;
  }
  if (!ny_path_readable(logo_png_path))
  ...
  if (!ny_path_readable(logo_png_path)) {
    ...
    if (join_path(stale_png, sizeof(stale_png), out_website_assets_dir,
                  "logo.png"))
      unlink(stale_png);
  }
  ...
  nyt_msg("OPEN", NYT_GREEN, "%s",
          display_index_path[0] ? display_index_path : index_path);
  ...
  if (serve) {
    ...
    return 1;
#else
    return serve_http_forever(out_dir, port);
#endif
  }
  return 0;
}
