...
static void collect_ny(const char *path, StrVec *out) {
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    else if (nyt_ends_with(child, ".ny"))
      sv_push(out, child);
  }
  closedir(d);
}
...
#ifdef _WIN32
  ny_test_proc_t proc = ny_test_spawn_argv(argv, NULL, 1);
...
static int test_is_error_path(const char *path) {
  return path && strncmp(path, "etc/tests/error/", 16) == 0;
}

static int test_is_ownership_error_path(const char *path) {
  return path && strstr(path, "etc/tests/error/ownership/") != NULL;
}
...
static int build_trace_argv(char **argv, int max, const char *bin, const char *path,
                            const char *std_path, const char *std_bc, char *flags_buf,
  ...
  return append_arg(argv, &argc, max, (char *)path);
}
...
static void debug_replay_failed_tests(StrVec *failed_paths, const char *bin, const char *std_path,
  ...
  for (size_t i = 0; i < failed_paths->len; i++) {
    ...
    char *expect = NULL;
    char *trace_argv[96];
    if (!build_trace_argv(trace_argv, 96, bin, path, std_path, std_bc, flags_buf, &flags,
                          &expect)) {
...
static int run_error_case(const char *bin, const char *path, const char *std_path,
  ...
  bool ownership_case = path && strstr(path, "etc/tests/error/ownership/") != NULL;
  char *flagv[32];
  ...
  if (tmp_len == 0 || tmp_len >= sizeof(tmp_dir))
    snprintf(tmp_dir, sizeof(tmp_dir), ".\\");
  snprintf(tmp, sizeof(tmp), "%sny-error-%lu-%lu.log", tmp_dir,
           (unsigned long)GetCurrentProcessId(), (unsigned long)GetTickCount());
#else
  ...
  for (size_t i = 0; i < selected.len; i++) {
    ...
    if (use_cache) {
      ...
      if (row && row->ok == 1 && row->sig == sig) {
        ...
        printf("%s%3d%%%s [%s✓%s/%s✓%s/%s✓%s] %s cache%s %s\n", nyt_clr(NYT_GRAY),
               pct, nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET),
               nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET),
               nyt_clr(NYT_GRAY), nyt_clr(NYT_RESET), disp_path(p));
        error_meta_free(flags, expect);
        continue;
      }
    }
    ...
    if (ok) {
      ...
      if (stats)
        stats->passed++;
      printf("%s%3d%%%s [%s✓%s/%s✓%s/%s✓%s] %s%6dms%s %s\n", nyt_clr(NYT_GRAY), pct,
             nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN),
             nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET), nyt_clr(NYT_GRAY),
             dur, nyt_clr(NYT_RESET), disp_path(p));
    } else {
      (*failed)++;
      sv_push_unique(failed_paths, p);
      printf("%s%3d%%%s [%s✗%s/%s✗%s/%s✗%s] %s%6dms%s %s (%s)\n", nyt_clr(NYT_GRAY),
             pct, nyt_clr(NYT_RESET), nyt_clr(NYT_RED), nyt_clr(NYT_RESET), nyt_clr(NYT_RED),
             nyt_clr(NYT_RESET), nyt_clr(NYT_RED), nyt_clr(NYT_RESET), nyt_clr(NYT_GRAY),
             dur, nyt_clr(NYT_RESET), disp_path(p), why[0] ? why : "error mismatch");
    }
    if (cache)
  ...
  for (size_t i = 0; i < selected.len; i++) {
    ...
    if (ok == 2) {
      printf("%s%3zu%%%s [%s-%s/%s-%s/%s-%s] %s%6dms%s %s (%s)\n",
             nyt_clr(NYT_GRAY),
             ((i + 1) * 100) / (selected.len ? selected.len : 1),
             nyt_clr(NYT_RESET), nyt_clr(NYT_GRAY), nyt_clr(NYT_RESET),
             nyt_clr(NYT_GRAY), nyt_clr(NYT_RESET), nyt_clr(NYT_GRAY),
             nyt_clr(NYT_RESET), nyt_clr(NYT_GRAY), dur, nyt_clr(NYT_RESET),
             disp_path(selected.items[i]), why[0] ? why : "skipped");
      continue;
    }
    if (stats) {
      stats->tests++;
    ...
    if (ok) {
      ...
      printf("%s%3d%%%s [%s✗%s/%s✗%s/%s✗%s] %s%6dms%s %s (%s)\n",
             nyt_clr(NYT_GRAY), pct, nyt_clr(NYT_RESET), nyt_clr(NYT_RED),
             nyt_clr(NYT_RESET), nyt_clr(NYT_RED), nyt_clr(NYT_RESET),
             nyt_clr(NYT_RED), nyt_clr(NYT_RESET), nyt_clr(NYT_GRAY), dur,
             nyt_clr(NYT_RESET), disp_path(selected.items[i]),
             why[0] ? why : "repl paste failed");
    }
  }
  sv_free(&selected);
  return 0;
...
static int run_suite(const char *suite_name, StrVec *files, const char *bin, const char *std_path,
    ...
    if (use_cache) {
      ...
      if (row && row->ok == 1 && row->sig == sig) {
        ...
        printf("%s%3d%%%s [%s✓%s/%s✓%s/%s✓%s] %s cache%s %s\n",
               nyt_clr(NYT_GRAY), pct, nyt_clr(NYT_RESET),
               nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET),
               nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET), nyt_clr(NYT_GRAY), nyt_clr(NYT_RESET),
               disp_path(p));
  ...
  while (completed < total) {
    for (int i = 0; i < jobs && launched < total; i++) {
      ...
      if (!p)
        continue;
      ny_test_proc_t pid = run_one_start(bin, p, std_path, std_bc);
      if (!ny_test_proc_valid(pid)) {
        (*failed)++;
        sv_push_unique(failed_paths, p);
        completed++;
    ...
    for (int i = 0; i < jobs; i++) {
      ...
      if (rc == 0) {
        ...
        if (retried) {
          ...
          printf("%s%3d%%%s [%s✓%s/%s✓%s/%s✓%s] %s%6dms%s %s\n",
                 nyt_clr(NYT_GRAY), pct, nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET),
                 nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET), nyt_clr(NYT_GREEN), nyt_clr(NYT_RESET),
                 nyt_clr(NYT_GRAY), dur, nyt_clr(NYT_RESET), disp_path(run[i].path));
        }
      } else {
        ...
        if (timed_out || rc == NY_TEST_TIMEOUT_RC) {
          printf("%s%3d%%%s [%s✗%s/%s✗%s/%s✗%s] %s%6dms%s %s (timeout=%ds)\n",
                 nyt_clr(NYT_GRAY), pct, nyt_clr(NYT_RESET), nyt_clr(NYT_RED), nyt_clr(NYT_RESET),
                 nyt_clr(NYT_RED), nyt_clr(NYT_RESET), nyt_clr(NYT_RED), nyt_clr(NYT_RESET),
                 nyt_clr(NYT_GRAY), dur, nyt_clr(NYT_RESET), disp_path(run[i].path), timeout_sec);
        } else {
          printf(
              "%s%3d%%%s [%s✗%s/%s✗%s/%s✗%s] %s%6dms%s %s "
              "(rc=%d)\n",
              nyt_clr(NYT_GRAY), pct, nyt_clr(NYT_RESET), nyt_clr(NYT_RED), nyt_clr(NYT_RESET),
              nyt_clr(NYT_RED), nyt_clr(NYT_RESET), nyt_clr(NYT_RED), nyt_clr(NYT_RESET),
              nyt_clr(NYT_GRAY), dur, nyt_clr(NYT_RESET), disp_path(run[i].path), rc);
        }
      }
      if (row_stats) {
        row_stats->tests++;
...
int ny_test_main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (ny_arg_match(a, "--help", "-h")) {
      ...
      printf("%snotes:%s timeout defaults to 60s and is capped at 300s; error tests live under %setc/tests/error%s\n",
             nyt_clr(NYT_BOLD), nyt_clr(NYT_RESET), nyt_clr(NYT_CYAN), nyt_clr(NYT_RESET));
      return 0;
    }
    int color_mode = -2;
  ...
  if (files.len == 0) {
    ...
    collect_ny("etc/tests/repl", &files);
    collect_ny("etc/tests/bench", &files);
    collect_ny("etc/tests/error", &files);
    if (with_stdlib)
  ...
  for (size_t i = 0; i < limit; i++) {
    ...
    if (strncmp(p, "etc/tests/bench/", 16) == 0)
      sv_push(&benchmark, p);
    else if (strncmp(p, "etc/tests/repl/", 15) == 0)
      sv_push(&repl, p);
    else if (strncmp(p, "etc/tests/rt/", 13) == 0)
      sv_push(&runtime, p);
    else if (path_is_probe_test(p))
      sv_push(&probe, p);
    else if (strncmp(p, "etc/tests/error/", 16) == 0)
  ...
  /* Optional machine-readable timings for perf work. */
  if (pj && *pj) {
    FILE *f = fopen(pj, "wb");
    if (f) {
      fprintf(f, "{\n");
      ...
      for (size_t i = 0; i < timings.len; i++) {
        ...
        /* Minimal JSON escaping: paths are repo-local and should not contain quotes. */
