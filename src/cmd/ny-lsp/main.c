...
static char *json_extract_string_near(const char *json, const char *needle,
                                      const char *key) {
  if (!json)
    return NULL;
...
static void publish_diagnostics(const char *uri, const char *message, int line,
                                int col, bool has_error) {
  if (!uri)
    return;
  char body[4096];
  char escaped[512];
  ...
  snprintf(body, sizeof(body),
           "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/"
           "publishDiagnostics\",\"params\":"
           "{\"uri\":\"%s\",\"diagnostics\":[{\"range\":{\"start\":{\"line\":%"
           "d,\"character\":%d},"
           "\"end\":{\"line\":%d,\"character\":%d}},\"severity\":1,\"message\":"
           "\"%s\"}]}}",
           uri, line, col, line, col, escaped[0] ? escaped : "parse error");
  send_response(body);
}

static bool analyze_text(const char *text, char **out_msg, int *out_line,
                         int *out_col) {
  if (!text)
    return false;
  parser_t parser;
  parser_init(&parser, text, "<lsp>");
  ...
  if (out_line)
    *out_line = parser.last_error_line > 0 ? parser.last_error_line - 1 : 0;
  if (out_col)
