...
int64_t rt_set_globals(int64_t p) {
  g_globals_ptr = p;
  return p;
}
...
int64_t rt_set_args(int64_t argc, int64_t argv_ptr, int64_t envp_ptr) {
  ...
  for (int i = 0; i < g_argc; i++) {
    if (old_argv[i]) {
      ...
      *(int64_t *)((char *)(uintptr_t)p - 16) = ((int64_t)len << 1) | 1;
      rt_copy_mem((void *)(uintptr_t)p, old_argv[i], len + 1);
  ...
  for (int i = 0; i < env_count; i++) {
    ...
    *(int64_t *)((char *)(uintptr_t)p - 8) = TAG_STR;
    *(int64_t *)((char *)(uintptr_t)p - 16) = ((int64_t)len << 1) | 1;
    rt_copy_mem((void *)(uintptr_t)p, old_envp[i], len + 1);
...
int64_t rt_envp(void) { return (int64_t)g_envp; }
...
int64_t rt_argv(int64_t i) {
  ...
  *(int64_t *)((char *)(uintptr_t)res - 16) = ((int64_t)len << 1) | 1;
  rt_copy_mem((void *)(uintptr_t)res, s, len + 1);
  return res;
}

// Misc
int64_t rt_errno(void) { return errno; }
