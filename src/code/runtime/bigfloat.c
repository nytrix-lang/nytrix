...
/*
 * Native wrappers receive raw scalar arguments, but stdlib calls may still
 * carry a tagged integer or an accidental dynamic pointer for the optional
 * precision argument.  Normalize that boundary before clamping.
 */
static int64_t bf_precision_arg(int64_t p) {
  if (is_int(p))
    return rt_untag_v(p);
  if (is_ptr(p) || p <= 0 || p > 1048576)
    return (int64_t)bf_default_precision();
  return p;
}
...
int64_t rt_bigfloat_from_value_raw(int64_t v, int64_t precision) {
  uint32_t p = bf_precision_clamp(bf_precision_arg(precision));
  if (is_int(v))
    v = rt_untag_v(v);
  if (bf_ptr(v) ||
...
int64_t rt_bigfloat_from_f64_bits(double d, int64_t precision) {
  uint32_t p = bf_precision_clamp(bf_precision_arg(precision));
  return bf_from_value_raw(rt_flt_box_double(d), p);
}
...
double rt_bigfloat_to_f64_raw(int64_t av) {
