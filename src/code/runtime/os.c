...
static char **ny_argv_view(intptr_t rargv, bool *needs_free) {
  if (needs_free)
    *needs_free = false;
  if (!rargv)
    return NULL;
  ...
  /*
   * Native lists are counted buffers, not NULL-terminated pointer arrays.
   * Descriptor slots hold raw string pointers; do not apply legacy integer
   * untagging (which also corrupts odd-addressed C strings) to them.
   */
  if ((uintptr_t)rargv >= RT_NATIVE_TBUF_HEADER &&
      rt_header_readable_cached((uintptr_t)rargv - RT_NATIVE_TBUF_HEADER,
                                RT_NATIVE_TBUF_HEADER)) {
    const int64_t *hdr =
        (const int64_t *)((uintptr_t)rargv - RT_NATIVE_TBUF_HEADER);
    if ((uint64_t)hdr[0] == NY_NATIVE_TBUF_MAGIC) {
      if (hdr[1] < 0 || hdr[2] < 8 || hdr[1] > hdr[3] ||
          (uint64_t)hdr[1] >= SIZE_MAX / sizeof(char *) ||
          (uint64_t)hdr[1] > SIZE_MAX / (uint64_t)hdr[2])
        return NULL;
      size_t count = (size_t)hdr[1];
      char **dst = (char **)calloc(count + 1, sizeof(char *));
      if (!dst)
        return NULL;
      for (size_t i = 0; i < count; ++i)
        dst[i] = (char *)(uintptr_t)rt_tbuf_get(rargv, (int64_t)i, 0);
      if (needs_free)
        *needs_free = true;
      return dst;
    }
...
#endif
  return name && *name ? rt_alloc_string(name) : 0;
}
...
int64_t rt_gettimeofday(int64_t tv, int64_t tz) {
  ...
  if (is_int(tz))
    tz = rt_untag_v(tz);
  return gettimeofday((struct timeval *)(uintptr_t)tv,
...
/*
 * Native replacement for the interpreter-ABI std.core.term leaf.
 */
int64_t rt_terminal_size_raw(void) {
  int32_t dims[2] = {0, 0};
  int64_t status = rt_tty_size((int64_t)(uintptr_t)dims);
  int64_t cols = 0;
  int64_t rows = 0;
  if (is_int(status) && (status >> 1) == 0 && dims[0] > 0 && dims[1] > 0) {
    cols = dims[0];
    rows = dims[1];
  } else {
    ...
    if (columns && lines) {
      char *end_columns = NULL;
      char *end_lines = NULL;
      long parsed_columns = strtol(columns, &end_columns, 10);
      long parsed_lines = strtol(lines, &end_lines, 10);
      if (end_columns != columns && *end_columns == '\0' &&
          end_lines != lines && *end_lines == '\0' &&
          parsed_columns > 0 && parsed_lines > 0) {
        cols = parsed_columns;
        rows = parsed_lines;
      }
    }
  ...
  int64_t out = rt_tbuf_new_raw(2, 24);
  if (!out)
    return 0;
  rt_tbuf_set_tagged(out, 0, cols);
  rt_tbuf_set_tagged(out, 1, rows);
  return out;
}

/*
 * Raw-C-string implementation of std.core.term.color().
 */
int64_t rt_color_raw(int64_t text, int64_t color) {
  const char *code = "\033[37m";
  if (rt_native_is_str(color)) {
    const char *name = (const char *)(uintptr_t)color;
    if (strcmp(name, "black") == 0) code = "\033[30m";
    else if (strcmp(name, "red") == 0) code = "\033[31m";
    else if (strcmp(name, "green") == 0) code = "\033[32m";
    else if (strcmp(name, "yellow") == 0) code = "\033[33m";
    else if (strcmp(name, "blue") == 0) code = "\033[34m";
    else if (strcmp(name, "magenta") == 0) code = "\033[35m";
    else if (strcmp(name, "cyan") == 0) code = "\033[36m";
    else if (strcmp(name, "gray") == 0) code = "\033[90m";
  }
  int64_t prefixed = rt_cstr_concat((int64_t)(uintptr_t)code, text);
  return prefixed ? rt_cstr_concat(
                         prefixed, (int64_t)(uintptr_t)"\033[0m")
                  : 0;
}
...
int64_t rt_canvas(int64_t width, int64_t height) {
  ...
  ny_native_canvas_t *canvas = (ny_native_canvas_t *)calloc(1, sizeof(*canvas));
  if (!canvas)
    return 0;
  canvas->width = width;
  canvas->height = height;
  return (int64_t)(uintptr_t)canvas;
}
...
int64_t rt_canvas_set(int64_t canvas, int64_t x, int64_t y,
                             int64_t ch, int64_t color, int64_t bold) {
  (void)x;
  (void)y;
  (void)ch;
  (void)color;
...
/*
 * The callback-driven TUI loop is intentionally inert in compile-only/native
 * smoke tests; interactive rendering remains on the full stdlib path.
 */
...
/*
 * Native sound smoke tests can cleanly report an unavailable backend.
 */
...
int64_t rt_tui_begin(void) { return 0; }
int64_t rt_tui_end(void) { return 0; }
int64_t rt_poll_key(void) { return 0; }
...
int64_t rt_file_exists(int64_t path) {
  intptr_t rpath = (intptr_t)((path & 1) ? (path >> 1) : path);
  return ny_access((const char *)rpath, 0) == 0;
}
int64_t rt_sound_backend_name(void) {
  return (int64_t)(uintptr_t)"none";
}
...
#ifdef _WIN32
  bool av_free = false;
  char **av = ny_argv_view(rargv, &av_free);
...
int64_t rt_execve(int64_t path, int64_t argv, int64_t envp) {
  ...
  bool env_free = false;
  char **av = ny_argv_view(rargv, &av_free);
  char **ev = renvp ? ny_argv_view(renvp, &env_free) : _environ;
  ...
  bool env_free = false;
  char **av = ny_argv_view(rargv, &av_free);
  char **ev = renvp ? ny_argv_view(renvp, &env_free) : environ;
...
/*
 * Native NYIR thread callbacks are raw code addresses.  They must bypass the
 * tagged callable dispatcher because a perfectly valid executable address can
 * coincidentally have the NY_NATIVE tag bits.
 */
static int64_t rt_thread_call_dispatch_raw(int64_t fn, int64_t argc,
                                            const int64_t *argv) {
  switch (argc) {
  case 0: return ((int64_t (*)(void))fn)();
  case 1: return ((int64_t (*)(int64_t))fn)(argv[0]);
  case 2: return ((int64_t (*)(int64_t, int64_t))fn)(argv[0], argv[1]);
  case 3: return ((int64_t (*)(int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2]);
  case 4: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3]);
  case 5: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4]);
  case 6: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5]);
  case 7: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6]);
  case 8: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
  case 9: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8]);
  case 10: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9]);
  case 11: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10]);
  case 12: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11]);
  case 13: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11], argv[12]);
  case 14: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11], argv[12], argv[13]);
  case 15: return ((int64_t (*)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t, int64_t))fn)(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11], argv[12], argv[13], argv[14]);
  default: return 0;
  }
}

#define RT_THREAD_RAW_ARGC_MARK INT64_MIN

static bool rt_thread_prepare_call_args(int64_t argc, int64_t argv_ptr, int64_t *argc_raw_out,
                                        int64_t **argv_copy_out, bool *raw_fn_out) {
  if (!argc_raw_out || !argv_copy_out)
    return false;
  bool raw_fn = ((uint64_t)argc & (uint64_t)RT_THREAD_RAW_ARGC_MARK) != 0;
  argc &= ~RT_THREAD_RAW_ARGC_MARK;
...
static DWORD WINAPI rt_thread_trampoline(LPVOID p) {
  ...
  int64_t ret = 0;
  if (ta->argc >= 0) {
    ret = ta->raw_fn ? rt_thread_call_dispatch_raw(fn, ta->argc, ta->argv)
                     : rt_thread_call_dispatch(fn, ta->argc, ta->argv);
  } else if (ta->argc == -2) {
...
int64_t rt_thread_join(int64_t tid) {
  ...
  if (is_int(tid))
    tid = rt_untag_v(tid);
  else if (NY_NATIVE_IS(tid))
    tid = (int64_t)(uintptr_t)NY_NATIVE_DECODE(tid);
...
int64_t rt_mutex_lock64(int64_t m) {
  if (is_int(m))
    m = rt_untag_v(m);
  else if (NY_NATIVE_IS(m))
    m = (int64_t)(uintptr_t)NY_NATIVE_DECODE(m);
  if (!m)
    return -1;
...
static void *rt_thread_trampoline(void *p) {
  ...
  int64_t res = 0;
  if (ta->argc >= 0) {
    res = ta->raw_fn ? rt_thread_call_dispatch_raw(fn, ta->argc, ta->argv)
                     : rt_thread_call_dispatch(fn, ta->argc, ta->argv);
  } else if (ta->argc == -2) {
