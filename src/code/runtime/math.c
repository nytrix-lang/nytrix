...
extern int64_t rt_bigint_to_i64_raw(int64_t a);
...
int64_t rt_f64_to_i64(double v) { return (int64_t)v; }
...
double rt_sqrt_f64(double value) { return sqrt(value); }
double rt_sin_f64(double value) { return sin(value); }
double rt_cos_f64(double value) { return cos(value); }
double rt_f64_round(double value) { return (double)llround(value); }
...
double rt_fmod_f64(double left, double right) {
...
int64_t rt_shl(int64_t a, int64_t b) {
  if (rt_is_bigint_obj(a) || rt_is_bigint_obj(b)) {
    ...
    if (!rt_is_bigint_obj(a))
      a = rt_bigint_from_i64_raw(is_int(a) ? rt_untag_v(a) : a);
  ...
  if ((a & 1) && (b & 1)) {
    ...
    if (bv < 64) {
      /*
       * Test the mathematical result before narrowing it.  The old unsigned
       * comparison accepted overflowing positive shifts (and could then
       * reinterpret the wrapped value as a tagged integer), which corrupted
       * both the high-bit and negative-boundary cases.
       */
      __int128 shifted = (__int128)av << (unsigned)bv;
      if (shifted >= (__int128)NY_SMALL_INT_MIN &&
          shifted <= (__int128)NY_SMALL_INT_MAX)
        return rt_tag_v((int64_t)shifted);
    }
  }
...
/*
 * Native NYIR passes typed shift operands as raw i64 values.  The public
 * `__shl` helper above is the tagged interpreter ABI, so keep this adapter at
 * the raw/dynamic boundary and promote only when the mathematical result no
 * longer fits the small-integer representation.
 */
int64_t rt_shl_raw(int64_t a, int64_t b) {
  /*
   * Native arithmetic uses raw scalar shifts, but a typed bigint operand is
   * still a heap handle.  Passing that handle through the raw shift path
   * shifts its address and loses the value (the 2^62 boundary exposed this).
   */
  if (rt_is_bigint_obj(a) || rt_is_bigint_obj(b)) {
    /*
     * The bigint shift entry point consumes the tagged dynamic shift ABI.
     */
  ...
  return rt_bigint_shl(rt_bigint_from_i64_raw(a), rt_tag_v(b));
}
int64_t rt_shr(int64_t a, int64_t b) {
  if (rt_is_bigint_obj(a) || rt_is_bigint_obj(b)) {
    int64_t shift = rt_is_bigint_obj(b) ? rt_bigint_to_i64_raw(b)
                                       : (is_int(b) ? rt_untag_v(b) : b);
    if (!rt_is_bigint_obj(a))
      a = rt_bigint_from_i64_raw(is_int(a) ? rt_untag_v(a) : a);
    return rt_bigint_shr(a, shift);
  }
  return (int64_t)((((uint64_t)(a & 1 ? a >> 1 : a) >> (uint64_t)(b & 1 ? b >> 1 : b))) << 1 | 1);
}
