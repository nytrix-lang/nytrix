...
static bool rt_proof_string(int64_t value, const char **text, size_t *len) {
  ...
  /*
   * Native dictionary reads can return a headerless C-string pointer when a
   * dynamic value crosses the NYIR boundary.  Accept that representation at
   * this internal checker boundary and canonicalize it below.
   */
  uintptr_t raw = (uintptr_t)value;
  if (raw < 4096)
    return false;
  size_t n = strnlen((const char *)raw, 1u << 20);
  if (n >= (1u << 20))
    return false;
  *text = (const char *)raw;
  *len = n;
...
int64_t rt_proof_cert_check(int64_t canonical_v, int64_t digest_v,
  ...
  /*
   * The legacy dictionary path can preserve the digest as a raw scalar,
