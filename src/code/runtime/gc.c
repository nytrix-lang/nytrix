...
nyGcState_t gNyGc = {0};
#define NYGC_TEMP_ROOT_MAX 256
static _Thread_local int64_t *gNyGcTempRoots[NYGC_TEMP_ROOT_MAX];
static _Thread_local size_t gNyGcTempRootCount;
...
static void nyGcFinalizerSet(int64_t obj, void (*fn)(int64_t)) {
  if (g_finalizer_count * 10 >= g_finalizer_cap * 7) {
    ...
    nyGcFinalizerEntry_t *new_table =
        calloc(new_cap, sizeof(nyGcFinalizerEntry_t));
...
static void (*nyGcFinalizerGet(int64_t obj))(int64_t) {
  if (!g_finalizer_table || !g_finalizer_cap)
    return NULL;
  size_t idx = (size_t)obj % g_finalizer_cap;
...
static void nyGcLargeIndexMarkDirty(void) { g_large_index_dirty = true; }
...
static void nyGcLargeIndexRebuild(void) {
  ...
  if (count > g_large_index_cap) {
    ...
    free(g_large_index);
    g_large_index =
        (nyGcLargeInterval_t *)malloc(next * sizeof(nyGcLargeInterval_t));
    if (!g_large_index)
      return;
    g_large_index_cap = next;
  }
  size_t i = 0;
  for (nyGcLargeObject_t *n = gNyGc.large_objects; n && i < count;
       n = n->next, ++i) {
...
static void nyGcLock(void) {
  ...
  while (atomic_flag_test_and_set_explicit(&gNyGcLock, memory_order_acquire)) {
    ...
    unsigned int pauses = spins < 10 ? (1u << spins) : 1024u;
    for (unsigned int i = 0; i < pauses; ++i)
      __builtin_ia32_pause();
#elif defined(__aarch64__)
    unsigned int yields = spins < 10 ? (1u << spins) : 1024u;
    for (unsigned int i = 0; i < yields; ++i)
...
static bool nyGcHeaderInSpace(int64_t obj, uint8_t *start, uint8_t *used_end,
  ...
  if (p < start + NYGC_OBJECT_DATA_OFFSET ||
      p >= used_end + NYGC_OBJECT_DATA_OFFSET)
    return false;
...
static bool nyGcHeaderForObject(int64_t obj, nyGcHeader_t **out) {
  ...
  if (large && large->header) {
    ...
    if (p == obj_ptr &&
        nyGcHeaderCandidateValid(large->header, (uint8_t *)large->header +
                                                    large->total_size)) {
      if (out)
        *out = large->header;
      return true;
...
static void nyGcInitObject(nyGcHeader_t *header, size_t size, uint32_t flags,
                           uint32_t age) {
...
static bool nyGcFits(uint8_t *cursor, uint8_t *limit, size_t total) {
  return cursor <= limit && total <= (size_t)(limit - cursor);
}
...
static int64_t nyGcAllocFastUnlocked(size_t size) {
  ...
  if (total == 0)
    return 0;
  if (!nyGcFits(gNyGc.nursery_ptr, gNyGc.nursery_limit, total))
    return 0;
...
static int64_t nyGcAllocTenuredUnlocked(size_t size) {
  ...
  if (total == 0)
    return 0;
  if (!nyGcFits(gNyGc.tenured_free, gNyGc.tenured_limit, total))
    return 0;
...
static bool nyGcForwardPush(nyGcForwardMap_t *map, int64_t from, int64_t to) {
  ...
  if (map->count >= map->cap) {
    ...
    nyGcForward_t *data =
        (nyGcForward_t *)realloc(map->data, next * sizeof(*data));
    if (!data)
      return false;
    map->data = data;
    map->cap = next;
...
static int64_t nyGcForwardFind(const nyGcForwardMap_t *map, int64_t value) {
  ...
  nyGcForward_t *found = (nyGcForward_t *)bsearch(
      &key, map->data, map->count, sizeof(*map->data), gc_forward_cmp);
...
static int64_t nyGcTaggedToInt(int64_t v) { return is_int(v) ? (v >> 1) : v; }
...
static void nyGcUpdateObjectRefs(nyGcHeader_t *header,
                                 const nyGcForwardMap_t *map) {
  ...
  if (tag == TAG_LIST || tag == TAG_TUPLE) {
    ...
    size_t max_items =
        header->size > 16 ? (header->size - 16) / sizeof(int64_t) : 0;
    if (len < 0)
      len = 0;
    ...
    uint8_t *table = NULL;
    size_t max_slots = 0;
    nyGcHeader_t *table_header = NULL;
    if (nyGcHeaderForObject(tbl, &table_header)) {
      table = (uint8_t *)(uintptr_t)tbl;
      max_slots = table_header->size / 24;
    } else if (header->size > 16) {
      table = (uint8_t *)(uintptr_t)obj + 16;
      max_slots = (header->size - 16) / 24;
    }
    if ((uint64_t)cap > max_slots)
      cap = (int64_t)max_slots;
    for (int64_t i = 0; i < cap; i++) {
      uint8_t *slot = table + (size_t)i * 24;
    ...
    size_t max_slots = header->size / 24;
...
static void nyGcApplyForwards(const nyGcForwardMap_t *map) {
  ...
  for (nyGcLargeObject_t *large = gNyGc.large_objects; large;
       large = large->next) {
    if (large->header)
...
static void nyGcRememberObjectRefs(nyGcHeader_t *header) {
  ...
  if (tag == TAG_LIST || tag == TAG_TUPLE) {
    ...
    int64_t cap = nyGcTaggedToInt(*(int64_t *)((uint8_t *)(uintptr_t)obj + 8));
    int64_t *table_slot = (int64_t *)((uint8_t *)(uintptr_t)obj + 16);
    nyGcRememberSlotIfNursery(table_slot);
    int64_t tbl = *table_slot;
    if (cap < 0)
      cap = 0;
    uint8_t *table = NULL;
  ...
  while (p < ptr) {
    ...
    if (magic != NY_MAGIC1 || ((size_slot >> 1) < header->size)) {
      fprintf(
          ...
          phase, name, (void *)header, (void *)obj, (unsigned long long)magic,
          (unsigned long long)size_slot, (unsigned long long)header->size);
      abort();
    }
    p += total;
  }
...
static void nyGcValidateUnlocked(const char *phase) {
  if (!gNyGc.initialized || !rt_env_enabled("NYTRIX_GC_VALIDATE"))
    return;
  nyGcValidateSpace(phase, "nursery", gNyGc.nursery_start, gNyGc.nursery_ptr,
                    gNyGc.nursery_limit);
  nyGcValidateSpace(phase, "tenured", gNyGc.tenured_start, gNyGc.tenured_free,
                    gNyGc.tenured_limit);
  for (size_t i = 0; i < gNyGc.root_count; i++) {
    if (!gNyGc.roots[i]) {
  ...
  for (size_t i = 0; i < gNyGc.remembered_count; i++) {
    ...
    if (!slot || (!slot_in_large && ((uint8_t *)slot < gNyGc.tenured_start ||
                                     (uint8_t *)slot >= gNyGc.tenured_free))) {
...
static size_t nyGcByteSizeFromEnv(const char *name, size_t fallback,
                                  size_t minimum) {
...
void nyGcInit(void) {
  ...
  gNyGc.nursery_capacity = nyGcByteSizeFromEnv("NYTRIX_GC_NURSERY_SIZE",
                                               NYGC_NURSERY_SIZE, 64u * 1024u);
  gNyGc.tenured_capacity = nyGcByteSizeFromEnv("NYTRIX_GC_TENURED_SIZE",
                                               NYGC_TENURED_SIZE, 256u * 1024u);
  ...
  gNyGc.remembered_index =
      (int64_t **)calloc(gNyGc.remembered_index_capacity, sizeof(int64_t *));
...
void nyGcAddRoot(int64_t *slot) {
  ...
  if (gNyGc.root_count >= gNyGc.root_capacity) {
    if (gNyGc.root_capacity > SIZE_MAX / (2 * sizeof(*gNyGc.roots)))
      nyGcFatal("root set capacity overflow");
...
void nyGcPushTempRoot(int64_t *slot) {
  if (!slot)
    return;
  if (gNyGcTempRootCount >= NYGC_TEMP_ROOT_MAX)
    nyGcFatal("temporary GC root stack overflow");
  if (!gNyGc.initialized)
    nyGcInit();
  if (!gNyGc.enable_nursery)
    return;
  gNyGcTempRoots[gNyGcTempRootCount++] = slot;
  nyGcAddRoot(slot);
}

void nyGcPopTempRoot(void) {
  if (gNyGcTempRootCount == 0)
    return;
  int64_t *slot = gNyGcTempRoots[--gNyGcTempRootCount];
  gNyGcTempRoots[gNyGcTempRootCount] = NULL;
  nyGcRemoveRoot(slot);
}
...
void nyGcWriteBarrier(int64_t *slot, int64_t value) {
  ...
  bool value_in_nursery =
      nyGcHeaderInSpace(value, gNyGc.nursery_start, gNyGc.nursery_ptr, NULL);
  if (value_in_nursery) {
...
static void nyGcAddRememberedUnlocked(int64_t *slot) {
  ...
  if (gNyGc.remembered_count >= gNyGc.remembered_capacity) {
    size_t next_capacity =
        gNyGc.remembered_capacity ? gNyGc.remembered_capacity * 2 : 1024;
    int64_t **next = (int64_t **)realloc(gNyGc.remembered_set,
...
int64_t nyGcAllocFast(size_t size) {
  ...
  if (total < gNyGc.large_threshold &&
      nyGcFits(gNyGc.nursery_ptr, gNyGc.nursery_limit, total)) {
    nyGcLock();
    if (nyGcFits(gNyGc.nursery_ptr, gNyGc.nursery_limit, total)) {
  ...
  nyGcLock();
  int64_t obj = (total >= gNyGc.large_threshold) ? nyGcAllocLargeUnlocked(size)
...
static void nyGcMarkUnlocked(int64_t obj) {
  ...
  if (tag == TAG_LIST || tag == TAG_TUPLE) {
    ...
    if (nyGcHeaderForObject(tbl, &table_header)) {
      ...
      /*
       * Small dictionaries store their entries inline after the table word.
       */
      table = (uint8_t *)(uintptr_t)obj + 16;
      max_slots = (header->size - 16) / 24;
    }
...
static void nyGcPromoteSurvivors(void) {
  ...
  while (ptr < old_nursery_ptr) {
    ...
    if (header->flags & NYGC_MARKED) {
      uint32_t next_age =
          header->age < UINT32_MAX ? header->age + 1 : header->age;
      if (next_age < NYGC_PROMOTION_AGE) {
        ...
        if (!nyGcForwardPush(&forwards, from_obj,
        ...
        dst->flags =
            (dst->flags | NYGC_TENURED) & ~(NYGC_MARKED | NYGC_SCANNED);
        ...
        free(forwards.data);
        fprintf(stderr, "GC: FATAL: tenured full during promotion, heap would "
                        "be corrupted\n");
        abort();
      }
    } else {
...
int64_t nyGcMakeWeak(int64_t target) {
  if (!target)
    return 0;
  int64_t protected_target = target;
  int64_t weak_obj = 0;
  NY_GC_PROTECT(protected_target) { weak_obj = nyGcAlloc(sizeof(int64_t)); }
