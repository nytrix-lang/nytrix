...
#define NYGC_DEFAULT_LOS_THRESHOLD                                             \
  (1024 * 1024) /* 1 MB large-object cutoff                                    \
                 */
...
static inline size_t nyGcBodySize(size_t size) {
  if (size > SIZE_MAX - 15u)
    return 0;
  return (size + 15u) & ~15u;
}
static inline size_t nyGcAllocSize(size_t size) {
  size_t body = nyGcBodySize(size);
  if ((size != 0 && body == 0) || body > SIZE_MAX - NYGC_OBJECT_DATA_OFFSET)
    return 0;
  return NYGC_OBJECT_DATA_OFFSET + body;
}
...
/* Scoped temporary roots protect intermediate objects across allocations that
 * may collect. */
void nyGcPushTempRoot(int64_t *slot);
void nyGcPopTempRoot(void);
#define NY_GC_PROTECT(slot)                                                    \
  for (int ny_gc_protect_once = (nyGcPushTempRoot(&(slot)), 1);                \
       ny_gc_protect_once; nyGcPopTempRoot(), ny_gc_protect_once = 0)
