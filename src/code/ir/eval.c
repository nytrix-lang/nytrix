...
static _Thread_local nyir_eval_global_t *nyir_eval_globals;
static _Thread_local size_t nyir_eval_global_len;
static _Thread_local size_t nyir_eval_global_cap;

void nyir_eval_globals_clear(void) {
  for (size_t i = 0; i < nyir_eval_global_len; ++i)
    free(nyir_eval_globals[i].name);
  free(nyir_eval_globals);
  nyir_eval_globals = NULL;
  nyir_eval_global_len = 0;
  nyir_eval_global_cap = 0;
}

static int64_t *nyir_eval_global_cell(const char *name) {
  if (!name || !*name)
    return NULL;
  for (size_t i = 0; i < nyir_eval_global_len; ++i)
    if (strcmp(nyir_eval_globals[i].name, name) == 0)
      return &nyir_eval_globals[i].value;
  if (nyir_eval_global_len == nyir_eval_global_cap) {
    size_t cap = nyir_eval_global_cap ? nyir_eval_global_cap * 2u : 32u;
    if (cap < nyir_eval_global_cap || cap > SIZE_MAX / sizeof(*nyir_eval_globals))
      return NULL;
    nyir_eval_global_t *grown =
        realloc(nyir_eval_globals, cap * sizeof(*nyir_eval_globals));
    if (!grown)
      return NULL;
    nyir_eval_globals = grown;
    nyir_eval_global_cap = cap;
  }
  char *copy = strdup(name);
  if (!copy)
    return NULL;
  nyir_eval_globals[nyir_eval_global_len] =
      (nyir_eval_global_t){.name = copy, .value = 0};
  return &nyir_eval_globals[nyir_eval_global_len++].value;
}
...
bool nyir_eval_with_calls(const nyir_func_t *f, int64_t *locals,
  ...
  while (pc < f->len) {
    ...
    case NYIR_F32_TO_F64:
      ...
      else if (in->op == NYIR_F64_TO_I64)
        values[in->dst] = (int64_t)nyir_bits_to_f64(a);
      else if (in->op == NYIR_F32_TO_I64)
        values[in->dst] = (int64_t)nyir_bits_to_f32(a);
    ...
    case NYIR_ADDR_SYMBOL: {
      ...
      const char *str_bytes =
          in->symbol ? ny_native_strtab_get(in->symbol, &str_len) : NULL;
      if (str_bytes) {
        ...
        if (sym)
          ...
          addr = (uintptr_t)nyir_eval_global_cell(in->symbol);
        else if (strncmp(in->symbol, "ny_fn_", 6) == 0 ||
                 strncmp(in->symbol, "__ny_lambda_", 12) == 0)
          /*
           * Function values are lowered to direct NYIR calls when invoked;
           * the VM only needs a non-null opaque value for the intermediate
           * address used by that lowering.
           */
          addr = 1;
      }
      if (!addr) {
        ...
        snprintf(reason, sizeof(reason),
                 "NYIR VM could not resolve addr.symbol '%s'",
                 in->symbol && in->symbol[0] ? in->symbol : "<empty>");
        nyir_eval_result_free(result);
        nyir_eval_pool_end(eval_depth, &eval_slot);
        return nyir_inst_err(err, err_len, in, inst_index, reason);
      }
      nyir_eval_note_value(result, in->dst);
      values[in->dst] = (int64_t)addr;
      known[in->dst] = true;
      break;
    }
    case NYIR_ALLOCA:
    ...
    case NYIR_SELECT_I64:
      if (!nyir_eval_read_value(values, known, in->a, &a) ||
          !nyir_eval_read_value(values, known, in->b, &b) ||
          !nyir_eval_read_value(values, known, in->c, &out))
        goto missing_value;
      nyir_eval_note_value(result, in->dst);
      values[in->dst] = a != 0 ? b : out;
      known[in->dst] = true;
      break;
    case NYIR_CMP_I64:
    case NYIR_CMP_F64:
...
bool nyir_tv_equiv_straightline(const nyir_func_t *before,
  ...
  for (int t = 0; t < trials; ++t) {
    ...
    if (!ra.returned || !rb.returned) {
      int ret_a = (int)ra.returned;
      int ret_b = (int)rb.returned;
      nyir_eval_result_free(&ra);
      nyir_eval_result_free(&rb);
      free(locals_a);
      free(locals_b);
    ...
    if (ra.result != rb.result) {
      int64_t res_a = ra.result;
      int64_t res_b = rb.result;
      nyir_eval_result_free(&ra);
      nyir_eval_result_free(&rb);
      free(locals_a);
      free(locals_b);
