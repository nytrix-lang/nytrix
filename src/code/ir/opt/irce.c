 ...
 *
 * Two-phase: (1) hoist invariant checks using SCEV-lite IV range
 * facts (iv_min/iv_max), (2) elide affine constant checks proven
 * safe by trip-count derived ranges. irce_affine_offset_range handles
 * composite indices like i*M + j.
 *
 * References:
 *  Bodik, Gupta, Sarkar — "ABCD: Eliminating Array Bounds Checks on
 *   Demand" (PLDI 2000). §3–5: inequality graph, demand-driven BCE.
 *  Rugina & Rinard — "Symbolic Bounds Analysis of Pointers, Array
 *   Indices, and Accessed Memory Regions" (PLDI 2000).
 *  Pop, Cohen, Silber — SCEV (INRIA TR, 2004). §2–3.
 */
...
static bool irce_affine_offset_range(const nyir_func_t *f, const int *defs, const bool *iv_known, const int64_t *iv_min, const int64_t *iv_max, int value, unsigned depth, int64_t *min_out, int64_t *max_out);
...
static bool irce_hoist_one_invariant_check(nyir_func_t *f, bool *moved) {
  ...
  if (!in_loop || !iv_known || !iv_min || !iv_max) {
    free(in_loop); free(iv_known); free(iv_min); free(iv_max);
    free(defs);
    nyir_scev_free(&info);
    nyir_cfg_free(&cfg);
    return false;
  ...
  for (size_t li = 0; li < info.count; ++li) {
    ...
    if (loop->iv < 0 || (size_t)loop->iv >= values || !loop->trip_count_known ||
        loop->trip_count == 0 || loop->step <= 0 || loop->init < 0)
      continue;
    iv_known[loop->iv] = true;
    iv_min[loop->iv] = loop->init;
    iv_max[loop->iv] = loop->init + (loop->trip_count - 1) * loop->step;
  }
  for (size_t li = 0; li < info.count; ++li) {
    ...
    for (size_t block = 0; block < cfg.block_count; ++block) {
      ...
      for (size_t i = cfg.block_start[block]; i < cfg.block_end[block]; ++i) {
        ...
        int operands[] = {check->a, check->c};
        bool invariant = true;
        for (size_t k = 0; k < 2; ++k) {
          int value = operands[k];
          if (value < 0) continue;
          if (value >= f->next_value) { invariant = false; break; }
          int di = defs[value];
          if (di < 0) continue;
        ...
        if (value_b >= 0) {
          if (value_b >= f->next_value) b_invariant = false;
          else {
            int di = defs[value_b];
            if (di >= 0) {
              size_t def_block = cfg.inst_block[(size_t)di];
        ...
        if (!b_invariant) {
           int64_t min_val = 0;
           if (!irce_affine_offset_range(f, defs, iv_known, iv_min, iv_max, check->b, 0, &min_val, &max_val) || min_val < 0)
              continue;
        ...
        size_t check_idx = i;
        if (!b_invariant) {
           nyir_inst_t cinst = {.op = NYIR_CONST_I64, .dst = f->next_value++, .imm = max_val, .a = -1, .b = -1, .c = -1, .d = -1, .e = -1, .f = -1};
           if (!irce_insert_inst(f, at, cinst)) {
              free(in_loop);
              free(defs);
              nyir_scev_free(&info);
              nyir_cfg_free(&cfg);
              free(iv_known); free(iv_min); free(iv_max);
              return false;
           }
           hoisted.b = cinst.dst;
           at++;
           if (check_idx >= at) check_idx++;
        }

        f->data[check_idx] = (nyir_inst_t){.op = NYIR_NOP, .dst = -1, .a = -1, .b = -1, .c = -1, .d = -1, .e = -1, .f = -1};
...
static bool strip_const_check(nyir_func_t *f, const int *defs,
                              nyir_inst_t *check) {
  /*
   * When .c carries a dynamic byte length it GOVERNS; .imm is only the
   * static capacity (lower.c emits .c = len*8, .imm = cap*8, and every
   * consumer treats c>=0 as authoritative). A constant offset proven
   * below the capacity says nothing about the dynamic length, so no
   * check with a live .c may be discarded here.
   */
  if (check->c >= 0)
    return false;
  int64_t offset = 0;
