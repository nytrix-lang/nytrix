...
static nyir_cmp_t iv_cmp_invert(nyir_cmp_t c) {
  switch (c) {
 ...
 * the loop is sound.
 *
 * A loop-member instruction survives deletion only when it has no
 * observable effect: pure scalar computation over SSA values. Stores,
 * calls, raw loads, allocation, division/modulo (implicit zero-divide
 * trap), and returns must abort the rewrite -- the closed-form sum
 * replaces the whole span, so any side effect inside it would vanish.
 */
static bool iv_member_deletable(const nyir_inst_t *in) {
  switch (in->op) {
  case NYIR_LABEL:
  case NYIR_NOP:
  case NYIR_BR:
  ...
  const unsigned forbidden =
      ...
      NYIR_EFFECT_READ_MEMORY | NYIR_EFFECT_READ_LOCAL | NYIR_EFFECT_CALL |
      NYIR_EFFECT_MAY_TRAP | NYIR_EFFECT_ALLOCATION |
      NYIR_EFFECT_UNKNOWN_SIDE_EFFECT | NYIR_EFFECT_IO | NYIR_EFFECT_THREAD |
      NYIR_EFFECT_FFI | NYIR_EFFECT_FENV | NYIR_EFFECT_VOLATILE;
  return (nyir_inst_effects(in) & forbidden) == 0;
}
...
static bool iv_loop_removal_safe(const nyir_func_t *f, const nyir_cfg_t *cfg,
  ...
  for (size_t b = 0; b < cfg->block_count; ++b) {
    ...
    if (member[b]) {
      /*
       * Loop body: removed with the loop -- but only instruction by
       * instruction when nothing inside is observable.
       */
      for (size_t i = bs; i < be; ++i)
        if (!iv_member_deletable(&f->data[i]))
          return false;
      continue;
    }
    /*
...
static bool closed_form_constant_sum(nyir_func_t *f, const nyir_cfg_t *cfg,
  ...
  bool exit_taken_edge = false; /* exit is the BR_IF's TAKEN target */
  ...
  /*
   * Trip-count math assumes `cmp` spells the CONTINUE condition
   * (continue while i < N). When the chosen exit is the BR_IF's taken
   * edge, the branch condition is the EXIT predicate and the continue
   * condition is its inversion. Keeping the raw polarity substituted a
   * finite formula for an inverted-orientation loop (zero-trip or
   * non-terminating).
   */
  nyir_cmp_t cont_cmp =
      exit_taken_edge ? iv_cmp_invert(cmp->cmp) : cmp->cmp;
  ...
  if (!index || index->step <= 0 ||
      (runtime_bound &&
       (!index_on_a || (cont_cmp != NYIR_CMP_LT &&
                        cont_cmp != NYIR_CMP_LE))))
    return false;
  ...
  if (!runtime_bound) {
    ...
    formula[formula_count++] =
        (nyir_inst_t){.op = NYIR_CMP_I64, .dst = in_range,
                      .a = cmp_value, .b = init_value_for_trips, .c = -1, .d = -1,
                      .e = -1, .f = -1, .cmp = cont_cmp == NYIR_CMP_LT
                                                     ? NYIR_CMP_GT
                                                     : NYIR_CMP_GE};
    formula[formula_count++] =
