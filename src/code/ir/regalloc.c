...
/*
 * Fixed-register instructions are home barriers.  The x64 encoder currently
 * maps colors to callee-saved registers, but DIV/MOD still consume RAX/RDX,
 * variable shifts consume RCX, and calls may clobber every allocatable
 * register on targets whose ABI does not preserve it.  Split at the
 * instruction rather than allowing a colored segment to silently straddle
 * the clobber: the encoder then commits the old segment's canonical home and
 * reloads the next segment on demand.  A zero call mask means "unknown", so
 * retain the conservative all-colors contract for hand-built machine IR.
 */
static uint64_t inst_fixed_clobbers(const ny_mach_inst_t *in,
                                    ny_mach_reg_class_t reg_class) {
  if (!in)
    return 0;
  if (in->opcode == NY_MACH_CALL) {
    if (in->call_clobbers)
      return in->call_clobbers;
    return UINT64_MAX;
  }
  if (reg_class != NY_MACH_REGCLASS_GPR)
    return 0;
  if (in->opcode == NY_MACH_DIV || in->opcode == NY_MACH_MOD)
    return UINT64_MAX;
...
static void coalesce_segments(const ny_mach_func_t *mach,
  ...
  for (size_t i = 0; i + 1 < a->segment_len; ++i) {
    ...
    if (!block_has_color_conflict(a, y, x->color, by_block)) {
      ...
      y->reload = false;
      /*
       * This is a unique-successor / unique-predecessor edge.  The source
       * segment therefore supplies the only runtime entry value and the
       * same-color preg is a valid incoming contract for the successor.
       * Homes remain synchronized by the encoder at block exits.
       */
      y->carried = true;
    }
  }
    ...
    for (uint32_t v = 0; v < nv; ++v) {
      ...
      for (size_t s = start; s <= end;) {
        ...
        if (e > end)
          e = end;
        bool clobber = false;
        for (size_t pc = s; pc <= e; ++pc) {
          if (inst_fixed_clobbers(&mach->insts[pc], reg_class) != 0) {
            e = pc;
            clobber = true;
            break;
          }
        ...
        bool more = e < end;
        /*
         * A clobber is itself part of the old segment: its inputs must be
         * available before the fixed-register sequence runs.  The following
         * segment starts with reload=true, making the canonical home the
         * only legal source after the sequence.
         */
