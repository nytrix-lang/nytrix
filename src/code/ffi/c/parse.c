...
static int parse_array_extent_primary(ny_parser_t *p, size_t *out) {
  if (!p || !out)
    return 0;
  if (parse_is(p, "!") || parse_is(p, "~") || parse_is(p, "+")) {
    char op = p->tok.start ? p->tok.start[0] : '\0';
    parse_advance(p);
    size_t value = 0;
  ...
  if (p->tok.kind == NY_CTOK_IDENT) {
    int64_t def_value = 0;
    if (parser_lookup_define(p, p->tok, &def_value)) {
      /*
       * Preserve a known negative enumerator as a parsed, out-of-range
       * extent. parse_array_suffix then marks it invalid instead of treating
       * it as an unsized/flexible array.
       */
      *out = def_value < 0 ? (size_t)-1 : (size_t)def_value;
      parse_advance(p);
      return 1;
    }
  }
...
static void parse_array_suffix(ny_parser_t *p, ny_ctype_t *ty) {
  while (parse_accept(p, "[")) {
    ...
    if (parsed && extent > 0 && extent <= (size_t)INT64_MAX) {
      ...
      /*
       * A named negative constant is not a flexible (unsized) array.  Keep
       * it distinct so an invalid extent cannot be silently accepted when it
       * happens to be the final field of a struct.
       */
      int64_t named_value = 0;
      if (p->tok.kind == NY_CTOK_IDENT &&
          parser_lookup_define(p, p->tok, &named_value) && named_value < 0)
        ty->array_invalid = 1;
      else
        ty->array_unknown = 1;
    }
