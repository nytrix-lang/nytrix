 ...
 * in src/code/native/llvm/legacy/gencall/mono.c:
...
static fun_sig *type_lookup_static_call_sig(codegen_t *cg, scope *scopes,
  ...
  if (callee->kind == NY_E_IDENT) {
    ...
    fun_sig *sig = type_lookup_static_call_sig_by_name(cg, name, argc,
                                                       callee->as.ident.hash);
    if (sig)
      return sig;
  }
  if (callee->kind == NY_E_MEMBER && callee->as.member.target &&
    ...
    if (ny_resolve_module_expr_path(cg, scopes, depth, callee->as.member.target,
      ...
      if (ny_resolve_module_function_path(cg, module_path,
                                          callee->as.member.name, resolved_fun,
                                          sizeof(resolved_fun))) {
        fun_sig *sig =
...
static const char *infer_expr_type_uncached(codegen_t *cg, scope *scopes,
  ...
  case NY_E_CALL:
    if (e->as.call.callee && e->as.call.callee->kind == NY_E_IDENT) {
      ...
      if (!builtin_shadowed && n && strcmp(n, "prove") == 0 &&
        ...
        if (!proof_type)
          return "proof";
        const char *out =
            arena_strndup(cg->arena, proof_type, strlen(proof_type));
        free(proof_type);
    ...
    if (e->as.call.callee && e->as.call.callee->kind != NY_E_IDENT) {
      fun_sig *sig = type_lookup_static_call_sig(
          cg, scopes, depth, e->as.call.callee, e->as.call.args.len);
  ...
  case NY_E_MEMCALL:
    if (e->as.memcall.target && e->as.memcall.name) {
      ...
      if (ny_resolve_module_expr_path(cg, scopes, depth, e->as.memcall.target,
        ...
        if (ny_resolve_module_function_path(cg, module_path, e->as.memcall.name,
          ...
          if (ret) {
            ...
            e->semantic.canonical_callee = sig->name;
            e->semantic.canonical_callee_stmt = sig->stmt_t;
            return ret;
          }
        }
      }
      ...
      } else if (sig && (!target_type || strcmp(target_type, "any") == 0)) {
        any_contract = true;
      }
      if (sig) {
        ...
        e->semantic.ownership =
            sig->returns_owned ? NY_SEM_OWN_OWNED
                               : (sig->returns_borrow ? NY_SEM_OWN_BORROWED
                                                      : NY_SEM_OWN_UNKNOWN);
        if (sig->return_type)
          return sig->return_type;
        if (sig->inferred_return_type)
...
const char *infer_expr_type(codegen_t *cg, scope *scopes, size_t depth,
                            expr_t *e) {
  const char *type_name = infer_expr_type_uncached(cg, scopes, depth, e);
  if (!e)
    return type_name;
  ny_sem_rep_t rep = NY_SEM_REP_UNKNOWN;
  const char *leaf = ny_name_leaf(type_name);
  if (leaf && (strcmp(leaf, "f32") == 0 || strcmp(leaf, "float32") == 0))
    rep = NY_SEM_REP_F32;
  else if (leaf && (strcmp(leaf, "f64") == 0 || strcmp(leaf, "float") == 0 ||
                    strcmp(leaf, "float64") == 0))
    rep = NY_SEM_REP_F64;
  else if (leaf && (strcmp(leaf, "str") == 0 || strcmp(leaf, "string") == 0 ||
                    strcmp(leaf, "bytes") == 0))
    rep = NY_SEM_REP_STRING;
  else if (leaf && strcmp(leaf, "any") == 0)
    rep = NY_SEM_REP_TAGGED_DYNAMIC;
  else if (leaf && (strcmp(leaf, "list") == 0 || strcmp(leaf, "tuple") == 0 ||
                    strcmp(leaf, "set") == 0))
    rep = NY_SEM_REP_TYPED_BUFFER;
  else if (leaf && (strcmp(leaf, "dict") == 0 || strcmp(leaf, "object") == 0))
    rep = NY_SEM_REP_OBJECT;
  else if (leaf && (strcmp(leaf, "ptr") == 0 || strcmp(leaf, "cstr") == 0))
    rep = NY_SEM_REP_POINTER;
  else if (leaf && (strcmp(leaf, "fnptr") == 0 || strcmp(leaf, "closure") == 0))
    rep = NY_SEM_REP_CLOSURE;
  else if (leaf &&
           (strcmp(leaf, "int") == 0 || strcmp(leaf, "integer") == 0 ||
            strcmp(leaf, "bool") == 0 || strncmp(leaf, "i", 1) == 0 ||
            strncmp(leaf, "u", 1) == 0 || strncmp(leaf, "Fin<", 4) == 0))
    rep = NY_SEM_REP_RAW_INT;
  else if (type_name)
    rep = NY_SEM_REP_AGGREGATE;
  e->semantic.rep = rep;
  e->semantic.resolved = type_name != NULL;
  return type_name;
}
...
static bool ny_fin_try_handle_literal(codegen_t *cg, scope *scopes,
  ...
  if (val < 0 || val >= bound) {
    ny_diag_error(
        tok, "Fin<%" PRId64 "> requires 0 <= value < %" PRId64 ", got %" PRId64,
        bound, bound, val);
    return true;
  }
  return true;
...
LLVMTypeRef resolve_abi_type_name(codegen_t *cg, const char *type_name,
  ...
  if (layout) {
    if (layout->llvm_type)
      return layout->llvm_type;
    layout->llvm_type = LLVMStructCreateNamed(cg->ctx, layout->name);
    return layout->llvm_type;
  }
...
static bool type_call_returns_proven_int(codegen_t *cg, scope *scopes,
  ...
  if (call->as.call.args.len > 16)
    return false;
  fun_sig *sig = type_lookup_static_call_sig(
      cg, scopes, depth, call->as.call.callee, call->as.call.args.len);
  ...
  if (!ret)
    goto done;
  proven =
      type_expr_proven_int_with_params(cg, scopes, depth, ret, local_params,
                                       call->as.call.args.len, recursion + 1);
done:
...
static bool type_expr_proven_int_with_params(codegen_t *cg, scope *scopes,
  ...
  case NY_E_CALL: {
    ...
    bool proven =
        type_call_returns_proven_int(cg, scopes, depth, e, params, param_count,
                                     recursion + 1, &static_user_call);
...
static bool type_param_int_add(ny_type_param_int_t *params, size_t *param_count,
...
static bool type_stmt_preserves_int_locals_with_params(
  ...
  if (s->kind == NY_S_BLOCK) {
    for (size_t i = 0; i < s->as.block.body.len; ++i) {
      if (!type_stmt_preserves_int_locals_with_params(
              cg, scopes, depth, s->as.block.body.data[i], params, param_count,
              cap, recursion + 1))
        return false;
    }
    return true;
  ...
  if (s->kind == NY_S_RETURN)
    return type_expr_proven_int_with_params(cg, scopes, depth, s->as.ret.value,
                                            ...
                                            recursion + 1);
  if (s->kind == NY_S_IF) {
    if (s->as.iff.init && !type_stmt_preserves_int_locals_with_params(
                              cg, scopes, depth, s->as.iff.init, params,
    ...
    if (s->as.iff.conseq &&
        !type_stmt_preserves_int_locals_with_params(
            cg, scopes, depth, s->as.iff.conseq, conseq_params, &conseq_count,
            32, recursion + 1))
      return false;
    if (s->as.iff.alt && !type_stmt_preserves_int_locals_with_params(
                             cg, scopes, depth, s->as.iff.alt, alt_params,
                             &alt_count, 32, recursion + 1))
      return false;
    return true;
  }
  return false;
...
static bool type_stmt_returns_proven_int_with_params(
  ...
  for (size_t i = 0; i < s->as.block.body.len; ++i) {
    ...
    if (!type_stmt_preserves_int_locals_with_params(cg, scopes, depth, child,
                                                    local_params, &local_count,
                                                    32, recursion + 1))
      return false;
  }
  return false;
    ...
    if (e->as.call.callee && e->as.call.callee->kind == NY_E_IDENT) {
      ...
      const char *ret_type =
          sig ? (sig->return_type ? sig->return_type
              ...
              : NULL;
      if (ret_type &&
          (strcmp(ret_type, "int") == 0 || strcmp(ret_type, "i64") == 0))
        return true;
      bool static_user_call = false;
