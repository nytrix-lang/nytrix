  ...
  case NY_T_IDENT: {
    ...
    if (assign_op != NY_T_EOF) {
      ...
      if (assign_op != NY_T_ASSIGN) {
        ...
        /*
         * Keep the assignment target and the read operand distinct AST
         * nodes.  Reusing `lhs` here makes semantic analysis annotate the
         * same identifier as both an lvalue and an rvalue; the native and
         * LLVM lowerers then see a partially resolved compound expression.
         * This showed up as `x += list[i]` losing the first/last value in
         * hot benchmark loops while `x = x + list[i]` was correct.
         */
        if (lhs->kind == NY_E_IDENT) {
          expr_t *read = expr_new(p->arena, NY_E_IDENT, lhs->tok);
          read->as.ident.name = arena_strndup(
              p->arena, lhs->as.ident.name, strlen(lhs->as.ident.name));
          read->as.ident.sym_id = lhs->as.ident.sym_id;
          bin->as.binary.left = read;
        } else {
          bin->as.binary.left = lhs;
        }
        bin->as.binary.right = rhs;
        /*
         * Compound assignment is evaluated as `lhs op (rhs)`, but keeping a
         * nested `+` tree here causes the dynamic type pass to classify the
         * whole read as an any value when the right side contains indexed
         * list reads.  Flatten the associative integer form so the common
         * accumulator shape receives the same typing as `x = x + a + b`.
         * This is limited to `+=`; subtraction and the other operators are
         * intentionally left grouped.
         */
        if (assign_op == NY_T_PLUS_EQ && rhs->kind == NY_E_BINARY &&
            rhs->as.binary.op &&
            strcmp(rhs->as.binary.op, "+") == 0) {
          expr_t *inner = rhs;
          expr_t *left_sum = expr_new(p->arena, NY_E_BINARY, op_tok);
          left_sum->as.binary.op = parser_token_name(NY_T_PLUS);
          left_sum->as.binary.left = bin->as.binary.left;
          left_sum->as.binary.right = inner->as.binary.left;
          bin->as.binary.left = left_sum;
          bin->as.binary.right = inner->as.binary.right;
        }
        rhs = bin;
      }
      if (lhs->kind == NY_E_IDENT) {
