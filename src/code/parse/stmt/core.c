...
static stmt_t *make_resource_cleanup(parser_t *p, token_t tok,
  ...
  /*
   * Resource pointers are nullable, but strict mode requires an explicit
   * boolean condition.  Desugar the cleanup guard as `name != 0` so the
   * generated AST remains valid under the same rules as user code.
   */
  expr_t *test = expr_new(p->arena, NY_E_BINARY, tok);
  test->as.binary.op = "!=";
  test->as.binary.left = make_ident_expr(p, tok, name);
  expr_t *zero = expr_new(p->arena, NY_E_LITERAL, tok);
  zero->as.literal.kind = NY_LIT_INT;
  zero->as.literal.as.i = 0;
  test->as.binary.right = zero;
  if_stmt->as.iff.test = test;
