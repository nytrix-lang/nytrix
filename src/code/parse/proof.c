...
static inline bool ny_proof_nonnegative_div_range(int64_t lmin, int64_t lmax,
...
bool ny_proof_canon_range(const char *s, ny_proof_name_range_fn env,
  ...
  if (strncmp(s, "binary:", 7) == 0) {
    ...
    if (strcmp(opbuf, "+") == 0) {
      ...
      ok = ny_proof_nonnegative_div_range(lmin, lmax, rmin, rmax, &lo, &hi);
    } else if (strcmp(opbuf, "%") == 0) {
...
int ny_proof_canon_decide(const char *s, ny_proof_name_range_fn env,
  ...
  if (strncmp(s, "bool:false", 10) == 0)
    return -1;
  if (strncmp(s, "int:", 4) == 0)
    return strtoll(s + 4, NULL, 10) != 0 ? 1 : -1;
  if (strncmp(s, "name:", 5) == 0) {
    char name[256];
    const char *q = s + 5;
    size_t n = 0;
