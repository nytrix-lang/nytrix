...
/*
 * Structured inline assembly is deliberately only syntax sugar.  It is
 * normalized here to NY_E_ASM, the exact node used by asm("...", "...", ...),
 * so constraints and backend emission cannot drift between spellings.
 */
typedef struct ny_structured_asm_operand {
  const char *name;
  const char *constraint;
  expr_t *value;
  bool output;
} ny_structured_asm_operand_t;

static bool ny_asm_token_is(token_t tok, const char *text) {
  size_t n = strlen(text);
  return tok.len == n && memcmp(tok.lexeme, text, n) == 0;
}

static const char *ny_asm_constraint_for_class(parser_t *p, token_t tok) {
  if (ny_asm_token_is(tok, "reg"))
    return "r";
  if (ny_asm_token_is(tok, "freg"))
    return "f";
  if (ny_asm_token_is(tok, "imm"))
    return "I";
  parser_error(p, tok, "asm operand class must be reg, freg, or imm", NULL);
  return "r";
}
...
static bool ny_asm_append_cstr(char *buf, size_t cap, size_t *len,
                               const char *text) {
  return ny_asm_append(buf, cap, len, text, strlen(text));
}

static bool ny_asm_append_template_line(
    parser_t *p, char *dst, size_t cap, size_t *dst_len, const char *start,
    const ny_structured_asm_operand_t *ops, size_t op_count) {
  const char *end = start;
  while (*end && *end != '\n' && *end != ';')
    end++;
  for (const char *at = start; at < end;) {
    if ((unsigned char)*at == '_' || isalpha((unsigned char)*at)) {
      const char *word = at++;
      while (at < end && ((unsigned char)*at == '_' ||
                          isalnum((unsigned char)*at)))
        at++;
      size_t word_len = (size_t)(at - word);
      size_t operand = op_count;
      for (size_t i = 0; i < op_count; ++i) {
        if (ops[i].name && strlen(ops[i].name) == word_len &&
            memcmp(ops[i].name, word, word_len) == 0) {
          /*
           * Backend constraints are normalized as outputs first, then
           * inputs, even when the block spells its input before `out`.
           */
          size_t outputs = 0, prior_inputs = 0;
          for (size_t j = 0; j < op_count; ++j)
            outputs += ops[j].output ? 1u : 0u;
          for (size_t j = 0; j < i; ++j)
            prior_inputs += ops[j].output ? 0u : 1u;
          operand = ops[i].output ? 0 : outputs;
          operand += ops[i].output ? 0 : prior_inputs;
          if (ops[i].output)
            for (size_t j = 0; j < i; ++j)
              if (ops[j].output)
                operand++;
      ...
      if (operand < op_count) {
        char ref[24];
        int n = snprintf(ref, sizeof(ref), "$%zu", operand);
        if (n < 0 || !ny_asm_append(dst, cap, dst_len, ref, (size_t)n)) {
          parser_error(p, p->cur, "structured asm template is too long", NULL);
          return false;
        }
      } else if (!ny_asm_append(dst, cap, dst_len, word, word_len)) {
        parser_error(p, p->cur, "structured asm template is too long", NULL);
        return false;
      }
      continue;
    }
    if (!ny_asm_append(dst, cap, dst_len, at, 1)) {
      parser_error(p, p->cur, "structured asm template is too long", NULL);
      return false;
    }
    at++;
  }
  return ny_asm_append(dst, cap, dst_len, "\n", 1);
}

static expr_t *ny_parse_structured_asm(parser_t *p, token_t asm_tok) {
  enum { NY_ASM_OPERAND_MAX = 32, NY_ASM_TEXT_MAX = 4096 };
  expr_t *e = expr_new(p->arena, NY_E_ASM, asm_tok);
  ny_structured_asm_operand_t ops[NY_ASM_OPERAND_MAX] = {0};
  size_t op_count = 0;
  char code[NY_ASM_TEXT_MAX] = {0};
  size_t code_len = 0;
  char constraints[NY_ASM_TEXT_MAX] = {0};
  size_t constraints_len = 0;
  char clobbers[NY_ASM_TEXT_MAX] = {0};
  size_t clobbers_len = 0;
  bool saw_instruction = false;

  parser_expect(p, NY_T_LBRACE, "'{' after asm", NULL);
  while (p->cur.kind != NY_T_RBRACE && p->cur.kind != NY_T_EOF) {
    if (p->cur.kind == NY_T_IN ||
        (p->cur.kind == NY_T_IDENT && ny_asm_token_is(p->cur, "out"))) {
      bool output = p->cur.kind != NY_T_IN;
      if (saw_instruction) {
        parser_error(p, p->cur,
                     "asm operands must precede assembly instructions", NULL);
        return e;
      }
      if (op_count == NY_ASM_OPERAND_MAX) {
        parser_error(p, p->cur, "too many asm operands", NULL);
        return e;
      }
      parser_advance(p);
      token_t class_tok = p->cur;
      parser_expect(p, NY_T_IDENT, "asm operand class", NULL);
      token_t name_tok = p->cur;
      parser_expect(p, NY_T_IDENT, "asm operand name", NULL);
      if (output) {
        for (size_t i = 0; i < op_count; ++i) {
          if (ops[i].output) {
            parser_error(p, name_tok,
                         "structured asm currently supports one output", NULL);
            return e;
          }
        }
      } else {
        parser_expect(p, NY_T_ASSIGN, "'=' before asm input expression", NULL);
      }
      ops[op_count].name = arena_strndup(p->arena, name_tok.lexeme, name_tok.len);
      ops[op_count].constraint = ny_asm_constraint_for_class(p, class_tok);
      ops[op_count].output = output;
      if (!output) {
        bool previous_stop = p->stop_expr_at_newline;
        p->stop_expr_at_newline = true;
        ops[op_count].value = p_parse_expr(p, 0);
        p->stop_expr_at_newline = previous_stop;
      }
      op_count++;
      continue;
    }
    if (p->cur.kind == NY_T_IDENT && ny_asm_token_is(p->cur, "clobber")) {
      if (saw_instruction) {
        parser_error(p, p->cur,
                     "asm clobbers must precede assembly instructions", NULL);
        return e;
      }
      parser_advance(p);
      token_t clobber_tok = p->cur;
      parser_expect(p, NY_T_IDENT, "asm clobber name", NULL);
      if (clobbers_len && !ny_asm_append_cstr(clobbers, sizeof(clobbers),
                                              &clobbers_len, ","))
        return e;
      if (!ny_asm_append_cstr(clobbers, sizeof(clobbers), &clobbers_len,
                              "~{") ||
          !ny_asm_append(clobbers, sizeof(clobbers), &clobbers_len,
                         clobber_tok.lexeme, clobber_tok.len) ||
          !ny_asm_append_cstr(clobbers, sizeof(clobbers), &clobbers_len, "}")) {
        parser_error(p, clobber_tok, "structured asm constraints are too long", NULL);
        return e;
      }
      continue;
    }
    ...
    int line = p->cur.real_line;
    if (!ny_asm_append_template_line(p, code, sizeof(code), &code_len,
                                     p->cur.lexeme, ops, op_count))
      return e;
    do {
      parser_advance(p);
    } while (p->cur.kind != NY_T_EOF && p->cur.kind != NY_T_RBRACE &&
             p->cur.real_line == line);
  }
  parser_expect(p, NY_T_RBRACE, "'}' after structured asm", NULL);

  /*
   * Reject operand-only blocks here so both backends report the same source
   * diagnostic instead of deferring an empty-template failure to emission.
   */
  if (!saw_instruction)
    parser_error(p, asm_tok, "structured asm requires an instruction", NULL);

  /*
   * LLVM/GCC constraints require outputs first, independent of source order.
   */
  for (int pass = 0; pass < 2; ++pass) {
    for (size_t i = 0; i < op_count; ++i) {
      if ((pass == 0) != ops[i].output)
        continue;
      if (constraints_len && !ny_asm_append_cstr(constraints, sizeof(constraints),
                                                 &constraints_len, ","))
        return e;
      if (ops[i].output && !ny_asm_append_cstr(constraints, sizeof(constraints),
                                               &constraints_len, "="))
        return e;
      if (!ny_asm_append_cstr(constraints, sizeof(constraints), &constraints_len,
                              ops[i].constraint))
        return e;
    }
  }
  if (clobbers_len) {
    if (constraints_len &&
        !ny_asm_append_cstr(constraints, sizeof(constraints), &constraints_len,
                            ","))
      return e;
    if (!ny_asm_append(constraints, sizeof(constraints), &constraints_len,
                       clobbers, clobbers_len))
      return e;
  }
  for (size_t i = 0; i < op_count; ++i) {
    if (!ops[i].output)
      vec_push_arena(p->arena, &e->as.as_asm.args, ops[i].value);
  }
  e->as.as_asm.code = arena_strndup(p->arena, code, code_len);
  e->as.as_asm.constraints = arena_strndup(p->arena, constraints, constraints_len);
  return e;
}
...
static expr_t *parse_if_stmt_as_expr(parser_t *p, stmt_t *s, token_t tok) {
  ...
  expr_t *then_expr = stmt_value_expr(p, s->as.iff.conseq, tok, "then");
  expr_t *else_expr = s->as.iff.alt
                          ? stmt_value_expr(p, s->as.iff.alt, tok, "else")
...
static expr_t *parse_primary(parser_t *p) {
  ...
  case NY_T_ASM: {
    parser_advance(p);
    if (p->cur.kind == NY_T_LBRACE)
      return ny_parse_structured_asm(p, tok);
  ...
  case NY_T_FN: {
    ...
    if (parser_match(p, NY_T_COLON)) {
      ...
      lam->as.lambda.return_type = parser_parse_return_type_suffix(
          p, expr_parse_type_ref, "expected return type");
    stmt_t *body = NULL;
    if (p->cur.kind == NY_T_LBRACE) {
      body = p_parse_block(p);
    } else {
...
static expr_t *parse_unary(parser_t *p) {
  ...
  if (p->cur.kind == NY_T_STAR) {
    ...
    if (parser_token_is_assign_op(p->cur)) {
      ...
      if (!rhs) {
        parser_error(p, assign_tok,
                     "expected expression after dereference assignment", NULL);
        return NULL;
      }
...
static expr_t *parse_expr_impl(parser_t *p, int prec) {
...
#define NY_MAX_PARSE_DEPTH 512

expr_t *p_parse_expr(parser_t *p, int prec) {
  if (!p)
    return NULL;
  if (p->expr_depth >= NY_MAX_PARSE_DEPTH) {
    parser_error(
        p, p->cur, "maximum expression nesting depth exceeded (512)",
        "simplify the expression or split it into intermediate values");
    return NULL;
  }
  p->expr_depth++;
  expr_t *result = parse_expr_impl(p, prec);
  p->expr_depth--;
  return result;
}
