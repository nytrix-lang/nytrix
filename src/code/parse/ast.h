...
/* Immutable semantic facts consumed by backends.  These deliberately describe
 * runtime representation rather than source syntax: native lowering must not
 * rediscover them from literal/call/member shapes. */
typedef enum ny_sem_rep_t {
  NY_SEM_REP_UNKNOWN = 0,
  NY_SEM_REP_RAW_INT,
  NY_SEM_REP_F32,
  NY_SEM_REP_F64,
  NY_SEM_REP_TAGGED_DYNAMIC,
  NY_SEM_REP_STRING,
  NY_SEM_REP_TYPED_BUFFER,
  NY_SEM_REP_OBJECT,
  NY_SEM_REP_POINTER,
  NY_SEM_REP_CLOSURE,
  NY_SEM_REP_AGGREGATE,
} ny_sem_rep_t;

typedef enum ny_sem_ownership_t {
  NY_SEM_OWN_UNKNOWN = 0,
  NY_SEM_OWN_BORROWED,
  NY_SEM_OWN_OWNED,
  NY_SEM_OWN_SHARED,
} ny_sem_ownership_t;

typedef enum ny_sem_call_kind_t {
  NY_SEM_CALL_NONE = 0,
...
typedef struct ny_expr_semantic_t {
  ny_sem_rep_t rep;
  ny_sem_ownership_t ownership;
  uint32_t alias_class; /* zero means conservative/unknown */
  bool mutable_value;
  bool resolved;
  ny_sem_call_kind_t member_call_kind;
  const char *canonical_callee;
  stmt_t *canonical_callee_stmt;
} ny_expr_semantic_t;
...
typedef struct stmt_func_t {
  const char *name;
  const char *return_type;
  ny_expr_semantic_t return_semantic;
  ny_param_list params;
  stmt_t *body;
  const char *doc;
  bool is_variadic;
