...
static bool nir_constfold_parallel_pure(nyir_func_t *f) {
  ...
  int64_t *value = calloc(n, sizeof(*value));
  bool *fknown = calloc(n, sizeof(*fknown));
  double *fvalue = calloc(n, sizeof(*fvalue));
  if (!known || !value || !fknown || !fvalue) {
    free(known); free(value); free(fknown); free(fvalue);
    return false;
  }
...
bool nyir_const_fold(nyir_func_t *f) {
  ...
  bool *fknown = (bool *)calloc((size_t)f->next_value, sizeof(bool));
  double *fvalue = (double *)calloc((size_t)f->next_value, sizeof(double));
  size_t max_local = nyir_max_local(f);
  bool *local_known = NULL;
  bool *local_addr_taken = NULL;
  int64_t *local_value = NULL;
  if (max_local > 0) {
    local_known = (bool *)calloc(max_local, sizeof(bool));
    local_addr_taken = (bool *)calloc(max_local, sizeof(bool));
    local_value = (int64_t *)calloc(max_local, sizeof(int64_t));
  }
  if (!known || !value || !fknown || !fvalue ||
...
bool nyir_sccp(nyir_func_t *f) {
  ...
  for (size_t i = 0; i < f->len; ++i) {
    ...
    if (in->dst >= 0 && (size_t)in->dst < values && known[in->dst] &&
        (in->op == NYIR_PHI || in->op == NYIR_COPY ||
