...
static bool sm_call(ny_sm_ctx_t *c, const nyir_inst_t *in) {
  ...
  const char *fn =
      (ext || strncmp(sym, "ny_fn_", 6) == 0 || strncmp(sym, "rt_", 3) == 0)
          ? ""
          : "ny_fn_";
  switch (c->kind) {
  case NY_SM_ARM:
...
static bool sm_inst(ny_sm_ctx_t *c, const nyir_inst_t *in) {
  ...
  case NYIR_ADDR_SYMBOL:
    if (!in->symbol || !in->symbol[0]) {
      ny_native_set_err(c->err, c->err_len,
                        "%s NYIR emit: addr.symbol missing symbol name",
                        c->pretty);
      return false;
    }
    switch (c->kind) {
    case NY_SM_ARM:
      return ny_native_printf(c->w, "\tldr\tr0, =%s\n", in->symbol) &&
             (in->dst < 0 || sm_stv(c, in->dst, c->t0));
    case NY_SM_A64:
      return ny_native_printf(c->w, "\tadrp\tx0, %s\n\tadd\tx0, x0, :lo12:%s\n",
                              in->symbol, in->symbol) &&
             (in->dst < 0 || sm_stv(c, in->dst, c->t0));
    case NY_SM_I386:
      return ny_native_printf(c->w, "\tmovl\t$%s, %%eax\n", in->symbol) &&
             (in->dst < 0 || sm_stv(c, in->dst, c->t0));
    case NY_SM_RISCV:
      return ny_native_printf(c->w, "\tla\ta0, %s\n", in->symbol) &&
             (in->dst < 0 || sm_stv(c, in->dst, c->t0));
    }
    return false;
