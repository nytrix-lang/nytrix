...
static bool ny_x64_emit_call_raw(ny_x64_ctx_t *ctx, const expr_t *e) {
  ...
  const call_arg_t *ordered[32] = {0};
  call_arg_t defaults[32] = {0};
  bool has_named = false;
  for (size_t i = 0; i < argc; ++i)
    has_named = has_named || e->as.call.args.data[i].name != NULL;
  if (has_named) {
    if (fn->as.fn.params.len > 32) {
      ny_native_set_err(ctx->err, ctx->err_len,
                        "native x86_64 named call has too many parameters");
      return false;
    }
    bool assigned[32] = {0};
    size_t next_pos = 0;
    for (size_t i = 0; i < argc; ++i) {
      const call_arg_t *arg = &e->as.call.args.data[i];
      size_t idx = next_pos;
      if (arg->name) {
        idx = fn->as.fn.params.len;
        for (size_t j = 0; j < fn->as.fn.params.len; ++j) {
          if (fn->as.fn.params.data[j].name &&
              strcmp(fn->as.fn.params.data[j].name, arg->name) == 0) {
            idx = j;
            break;
          }
        }
        if (idx == fn->as.fn.params.len) {
          ny_native_set_err(ctx->err, ctx->err_len,
                            "native x86_64 unknown named argument '%s'",
                            arg->name);
          return false;
        }
      } else {
        while (idx < fn->as.fn.params.len && assigned[idx])
          ++idx;
        if (idx == fn->as.fn.params.len) {
          ny_native_set_err(ctx->err, ctx->err_len,
                            "native x86_64 call has too many arguments");
          return false;
        }
        next_pos = idx + 1;
      }
      if (assigned[idx]) {
        ny_native_set_err(ctx->err, ctx->err_len,
                          "native x86_64 duplicate argument '%s'",
                          fn->as.fn.params.data[idx].name
                              ? fn->as.fn.params.data[idx].name
                              : "<positional>");
        return false;
      }
    ...
    for (size_t i = 0; i < fn->as.fn.params.len; ++i) {
      if (ordered[i])
        continue;
      defaults[i].val = fn->as.fn.params.data[i].def;
      ordered[i] = &defaults[i];
    }
    argc = fn->as.fn.params.len;
  } else {
    for (size_t i = 0; i < argc; ++i)
      ordered[i] = &e->as.call.args.data[i];
  }
  if (argc != fn->as.fn.params.len) {
    ny_native_set_err(ctx->err, ctx->err_len,
                      "native x86_64 call '%s' expects %zu args, got %zu",
                      name, fn->as.fn.params.len, argc);
    return false;
  }
  if (argc > ctx->target->gp_arg_reg_count) {
    ny_native_set_err(ctx->err, ctx->err_len,
  ...
  for (size_t i = 0; i < argc; ++i) {
    if (!ordered[i]->val) {
      if (!ny_native_put(ctx->w, "\txorq\t%rax, %rax\n"))
        return false;
    } else if (!ny_x64_emit_expr_raw(ctx, ordered[i]->val)) {
      return false;
    }
    if (!ny_native_put(ctx->w, "\tpushq\t%rax\n"))
      return false;
  }
...
#define NY_X64_NIR_MAX_SLOTS 16384
...
static int ny_x64_nir_slot(ny_x64_nir_ctx_t *c, int value_id) {
  if (value_id < 0 || value_id >= c->nyir->next_value)
    return -1;
  int s = c->frame_slots + value_id * 2;
  if (s < 0 || s + 1 >= NY_X64_NIR_MAX_SLOTS)
    return -1;
  c->slot_offset[s] = (s + 1) * 8;
  c->slot_offset[s + 1] = (s + 2) * 8;
  return s;
}
...
static void ny_x64_nir_compute_frame(ny_x64_nir_ctx_t *c) {
  ...
  c->is_leaf = true;
  for (size_t i = 0; i < c->nyir->len; ++i) {
    nyir_op_t op = c->nyir->data[i].op;
    if (op == NYIR_CALL) {
      c->is_leaf = false;
      break;
    }
    /*
     * Vector ops and x87 trig hardcode -N(%rbp) slot addressing; force a
     * rbp frame so those references are valid even for otherwise-leaf
     * functions.
     */
    if (op == NYIR_VEC4_LOAD_F64 || op == NYIR_VEC4_STORE_F64 ||
        ...
        op == NYIR_VEC4_SHUFFLE_F64 || op == NYIR_VEC8_LOAD_F32 ||
        op == NYIR_VEC8_STORE_F32 || op == NYIR_VEC8_ADD_F32 ||
        op == NYIR_VEC8_SUB_F32 || op == NYIR_VEC8_MUL_F32 ||
        op == NYIR_VEC8_DIV_F32 || op == NYIR_VEC8_FMA_F32 ||
        op == NYIR_VEC8_SET1_F32) {
      c->is_leaf = false;
      break;
    }
  }
  ...
  /*
   * Pre-compute two consecutive slots for every NYIR value. Vector values
   * use the second slot for their upper half; scalar values simply leave it
   * unused.
   */
  for (int v = 0; v < max_val &&
                   c->frame_slots + v * 2 + 1 < NY_X64_NIR_MAX_SLOTS; ++v)
    ny_x64_nir_slot(c, v);
  int total = c->frame_slots + max_val * 2;
  int raw = total * 8;
  /*
...
/*
 * A rematerialized immediate must not cross an instruction that consumes a
 * fixed register.  The value's stack home is the conservative handoff across
 * those boundaries.
 */
static bool ny_x64_nir_is_fixed_clobber(ny_x64_nir_ctx_t *c,
                                        const nyir_inst_t *in, size_t idx);

/*
 * Resolve a value through explicit COPY definitions to its CONST_I64 root.
 * The complete chain must lie in one fixed-register-safe region.
 */
static bool ny_x64_nir_try_remat_i64(ny_x64_nir_ctx_t *c, int value_id,
                                     size_t use_idx, int64_t *out) {
  if (value_id < 0 || value_id >= NY_X64_NIR_MAX_SLOTS ||
      use_idx > c->nyir->len)
    return false;
  int current = value_id;
  size_t before = use_idx;
  size_t root = 0;
  int64_t imm = 0;
  for (size_t hops = 0; hops < NY_X64_NIR_MAX_SLOTS; ++hops) {
    int di = c->def_index[current];
    if (di < 0 || (size_t)di >= before)
      return false;
    ...
    if (def->op != NYIR_COPY || def->a < 0 ||
        def->a >= NY_X64_NIR_MAX_SLOTS)
      return false;
    current = def->a;
    before = (size_t)di;
    if (hops + 1 == NY_X64_NIR_MAX_SLOTS)
      return false;
  }
  for (size_t i = root + 1; i < use_idx; ++i) {
    if (ny_x64_nir_is_fixed_clobber(c, &c->nyir->data[i], i))
      return false;
  }
  *out = imm;
  return true;
...
static bool ny_x64_nir_is_fixed_clobber(ny_x64_nir_ctx_t *c,
  ...
  case NYIR_SAR_I64: {
    int64_t count = 0;
    return !ny_x64_nir_try_remat_i64(c, in->b, idx, &count) ||
           count < 1 || count > 63;
  }
  case NYIR_ROR32_I64: {
    int64_t count = 0;
...
static bool ny_x64_nir_find_const_val(ny_x64_nir_ctx_t *c, int value_id, int64_t *out) {
  if (!c || !c->nyir || value_id < 0 || value_id >= (int)c->nyir->next_value)
    return false;
  int current = value_id;
  for (size_t hops = 0; hops < 32; ++hops) {
    if (current < 0 || current >= (int)c->nyir->next_value)
      return false;
    int di = c->def_index[current];
...
/*
 * Check whether value_id has a constant/COPY definition that fits an x86-64
 * sign-extended imm32.
 */
static bool ny_x64_nir_try_const_i32(ny_x64_nir_ctx_t *c, int value_id,
  ...
  (void)use_idx;
  int64_t v = 0;
  if (!ny_x64_nir_find_const_val(c, value_id, &v) ||
...
/*
 * Check whether value_id is a constant/COPY shift count.
 * x86-64 immediate shifts use counts 1 through 63.
 */
static bool ny_x64_nir_try_shift_imm(ny_x64_nir_ctx_t *c, int value_id,
                                     size_t use_idx, int *out) {
  (void)use_idx;
  int64_t v = 0;
  if (!ny_x64_nir_find_const_val(c, value_id, &v) ||
      v < 1 || v > 63)
...
static bool ny_x64_nir_try_pow2_divisor(ny_x64_nir_ctx_t *c, int value_id,
                                        size_t use_idx, int *out) {
  int64_t divisor = 0;
  if (!ny_x64_nir_try_const_i32(c, value_id, use_idx, &divisor) ||
...
static bool ny_x64_nir_remat_consumer(const nyir_inst_t *in, int value_id) {
  if (!ny_x64_nir_inst_uses_value(in, value_id))
    return false;
  switch (in->op) {
  case NYIR_ADD_I64: case NYIR_SUB_I64: case NYIR_MUL_I64:
  case NYIR_AND_I64: case NYIR_OR_I64: case NYIR_XOR_I64:
  case NYIR_SHL_I64: case NYIR_SAR_I64: case NYIR_ROR32_I64:
  case NYIR_CALL:
    return true;
  default:
    return false;
...
/*
 * A home may be skipped only when this value has one COPY-only path to a
 * consumer in the same safe region.  Any unknown segment keeps its home.
 */
static bool ny_x64_nir_can_fuse_const(ny_x64_nir_ctx_t *c, int value_id,
                                      size_t def_idx, int64_t *out) {
  int current = value_id;
  size_t current_def = def_idx;
  for (size_t hops = 0; hops < NY_X64_NIR_MAX_SLOTS; ++hops) {
    if (current < 0 || current >= NY_X64_NIR_MAX_SLOTS ||
        c->use_count[current] != 1)
      return false;
    size_t use_idx = current_def + 1;
    while (use_idx < c->nyir->len &&
           !ny_x64_nir_inst_uses_value(&c->nyir->data[use_idx], current))
      use_idx++;
    if (use_idx == c->nyir->len)
      return false;
    const nyir_inst_t *use = &c->nyir->data[use_idx];
    if (use->op == NYIR_COPY && use->a == current && use->dst >= 0 &&
        use->dst < NY_X64_NIR_MAX_SLOTS) {
      current = use->dst;
      current_def = use_idx;
      continue;
    }
    if (!ny_x64_nir_remat_consumer(use, current))
      return false;
    return ny_x64_nir_try_remat_i64(c, value_id, use_idx, out);
  }
  return false;
}
...
static bool ny_x64_nir_materialize_i64(ny_x64_nir_ctx_t *c, int64_t imm) {
  if (imm >= INT32_MIN && imm <= INT32_MAX)
    return ny_native_printf(c->w, "\tmovq\t$%" PRId64 ", %%rax\n", imm);
  return ny_native_printf(c->w, "\tmovabsq\t$%" PRId64 ", %%rax\n", imm);
}

/*
 * Load a value that may be a next-use-fused CONST_I64 whose home store was
 * skipped (see can_fuse_const).  Fused constants are rematerialized with the
 * signed form that preserves their full i64 value.
 */
static bool ny_x64_nir_load_value(ny_x64_nir_ctx_t *c, int value_id) {
  if (value_id >= 0 && value_id < NY_X64_NIR_MAX_SLOTS &&
      c->fused_const[value_id]) {
    return ny_x64_nir_materialize_i64(c, c->fused_imm[value_id]);
  }
...
/*
 * Check whether value_id has a full-width constant/COPY definition in this
 * use's safe region.
 */
static bool ny_x64_nir_try_const_i64(ny_x64_nir_ctx_t *c, int value_id,
                                     size_t use_idx, int64_t *out) {
  (void)use_idx;
  return ny_x64_nir_find_const_val(c, value_id, out);
}
...
static bool ny_x64_nir_emit_inst(ny_x64_nir_ctx_t *c,
  ...
  case NYIR_CONST_I64: {
    ...
    if (ny_x64_nir_can_fuse_const(c, in->dst, idx, &imm)) {
      c->fused_const[in->dst] = true;
      c->fused_imm[in->dst] = imm;
      return true;
    }
    return ny_x64_nir_materialize_i64(c, in->imm) &&
  ...
  case NYIR_LOAD_LOCAL:
    ...
    if (getenv("NYTRIX_NATIVE_TRACE_LOCALS"))
      fprintf(stderr, "[native-local] load slot=%" PRId64 " f64=%d f32=%d dst=%d\\n",
              in->imm, in->imm < NY_X64_NIR_MAX_SLOTS ? c->local_f64[in->imm] : 0,
              in->imm < NY_X64_NIR_MAX_SLOTS ? c->local_f32[in->imm] : 0,
              in->dst);
    if (in->imm < NY_X64_NIR_MAX_SLOTS && c->local_f64[in->imm])
      return ny_x64_nir_load_xmm(c, (int)in->imm, 0) &&
             ny_x64_nir_store_xmm(c, ny_x64_nir_slot(c, in->dst), 0);
    if (in->imm < NY_X64_NIR_MAX_SLOTS && c->local_f32[in->imm])
      return ny_x64_nir_load_xmm_f32(c, (int)in->imm, 0) &&
  ...
  case NYIR_VEC4_SHL_I64: {
    int shift = 0;
    if (ny_x64_nir_try_shift_imm(c, in->b, idx, &shift))
      return ny_x64_nir_load_xmm(c, ny_x64_nir_slot(c, in->a), 0) &&
  ...
  case NYIR_VEC4_SAR_I64:
    /*
     * No producer lowers this opcode today, and there is no single
     * SSE2/AVX2 instruction for packed-qword arithmetic right shift
     * (vpsraq is AVX512-only). Fail loudly if one appears rather than
     * executing garbage; a psrad-per-dword decomposition belongs with
     * whichever feature introduces a real producer.
     */
    ny_native_set_err(c->err, c->err_len,
                      "nyir x86-64: VEC4_SAR_I64 has no producer and no "
                      "SSE2/AVX2 form");
    return false;
  case NYIR_STORE_LOCAL:
    ...
    c->slot_offset[in->imm] = (in->imm + 1) * 8;
    if (getenv("NYTRIX_NATIVE_TRACE_LOCALS"))
      fprintf(stderr, "[native-local] store slot=%" PRId64 " f64=%d f32=%d value=%d vf64=%d\\n",
              in->imm, in->imm < NY_X64_NIR_MAX_SLOTS ? c->local_f64[in->imm] : 0,
              in->imm < NY_X64_NIR_MAX_SLOTS ? c->local_f32[in->imm] : 0,
              in->a, in->a >= 0 && in->a < NY_X64_NIR_MAX_SLOTS ? c->value_f64[in->a] : 0);
    if (in->a >= 0 && in->a < NY_X64_NIR_MAX_SLOTS &&
        c->value_f64[in->a])
      return ny_x64_nir_load_xmm(c, ny_x64_nir_slot(c, in->a), 0) &&
             ny_x64_nir_store_xmm(c, (int)in->imm, 0);
    if (in->a >= 0 && in->a < NY_X64_NIR_MAX_SLOTS &&
        c->value_f32[in->a])
      return ny_x64_nir_load_xmm_f32(c, ny_x64_nir_slot(c, in->a), 0) &&
             ny_x64_nir_store_xmm_f32(c, (int)in->imm, 0);
    return ny_x64_nir_load_value(c, in->a) &&
           ny_x64_nir_store(c, (int)in->imm);
  case NYIR_LOAD_I64:
    return ny_x64_nir_load_value(c, in->a) &&
           ((in->flags & NYIR_INST_F_MEM_BYTE)
                ? ny_native_put(c->w, "\tmovzbl\t(%rax), %eax\n")
                : ny_native_put(c->w, "\tmovq\t(%rax), %rax\n")) &&
           ny_x64_nir_store(c, ny_x64_nir_slot(c, in->dst));
  case NYIR_STORE_I64: {
    const char *st = (in->flags & NYIR_INST_F_MEM_BYTE)
                         ? "\tmovb\t%r10b, (%rax)\n"
                         : "\tmovq\t%r10, (%rax)\n";
    return ny_x64_nir_load_value(c, in->c) &&
           ny_native_put(c->w, "\tmovq\t%rax, %r10\n") &&
           ny_x64_nir_load_value(c, in->a) &&
           ny_native_put(c->w, st);
  }
  case NYIR_ADD_I64: {
    int64_t imm = 0;
    /*
     ...
     */
    if (ny_x64_nir_try_const_i32(c, in->b, idx, &imm))
      return ny_x64_nir_load_value(c, in->a) &&
             ny_native_printf(c->w, "\taddq\t$%" PRId64 ", %%rax\n", imm) &&
             ny_x64_nir_store(c, ny_x64_nir_slot(c, in->dst));
    if (ny_x64_nir_try_const_i32(c, in->a, idx, &imm))
      return ny_x64_nir_load_value(c, in->b) &&
             ny_native_printf(c->w, "\taddq\t$%" PRId64 ", %%rax\n", imm) &&
             ny_x64_nir_store(c, ny_x64_nir_slot(c, in->dst));
    return ny_x64_nir_load_value(c, in->b) &&
  ...
  case NYIR_MUL_I64: {
     ...
     */
    if (ny_x64_nir_try_const_i32(c, in->b, idx, &imm) && imm > 0 && (imm & (imm - 1)) == 0) {
      int shift = 0;
      int64_t v = imm;
      while (v > 1) { v >>= 1; shift++; }
      return ny_x64_nir_load_value(c, in->a) &&
    ...
    if (ny_x64_nir_try_const_i32(c, in->a, idx, &imm) && imm > 0 && (imm & (imm - 1)) == 0) {
      int shift = 0;
      int64_t v = imm;
      while (v > 1) { v >>= 1; shift++; }
      return ny_x64_nir_load_value(c, in->b) &&
  ...
  case NYIR_DIV_I64: {
    int shift = 0;
    if (ny_x64_nir_try_pow2_divisor(c, in->b, idx, &shift)) {
    ...
    int64_t d = 0;
    if (ny_x64_nir_try_const_i64(c, in->b, idx, &d) && d != 0 && d != 1 &&
        d != -1) {
  ...
  case NYIR_ROR32_I64: {
    int64_t shift = 0;
    if (ny_x64_nir_try_const_i32(c, in->b, idx, &shift) &&
        shift >= 0 && shift < 32)
      return ny_x64_nir_load_value(c, in->a) &&
             ny_native_printf(c->w, "\tmovl\t%%eax, %%eax\n\trorl\t$%" PRId64 ", %%eax\n",
                              shift) &&
             ny_x64_nir_store(c, ny_x64_nir_slot(c, in->dst));
    return ny_x64_nir_load_value(c, in->b) &&
           ny_native_put(c->w, "\tmovq\t%rax, %r10\n") &&
  ...
  case NYIR_SELECT_I64: {
    char true_addr[64];
    ny_x64_nir_slot_str(c, ny_x64_nir_slot(c, in->b), true_addr,
                        sizeof(true_addr));
    return ny_x64_nir_load_value(c, in->a) &&
           ny_native_put(c->w, "\ttestq\t%rax, %rax\n") &&
           ny_x64_nir_load_value(c, in->c) &&
           ny_native_printf(c->w, "\tcmovne\t%s, %%rax\n", true_addr) &&
           ny_x64_nir_store(c, ny_x64_nir_slot(c, in->dst));
  }
     ...
     if (in->a >= 0 && in->a < NY_X64_NIR_MAX_SLOTS) {
       ...
       if (di >= 0 && (size_t)di < c->nyir->len) {
         ...
         if (cmp->op == NYIR_CMP_I64 && cmp->dst == in->a) {
           ...
           int64_t bimm = 0;
           bool b_is_imm = ny_x64_nir_try_const_i32(c, cmp->b, idx, &bimm);
           if (b_is_imm)
  ...
  case NYIR_CALL: {
    ...
    if (in->symbol && strcmp(in->symbol, "rt_sqrt_f64") == 0 && argc == 1 && in->a >= 0 && in->dst >= 0) {
    ...
    for (int i = 0; i < argc; ++i) {
      if (agg_in_regs[i]) {
        ...
        int64_t carg_imm = 0;
        if (ny_x64_nir_try_const_i32(c, arg_vals[i], idx, &carg_imm)) {
    ...
    } else if (strncmp(sym_name, "ny_fn_", 6) == 0) {
      snprintf(fn_label, sizeof(fn_label), "%s%s",
               c->target->symbol_prefix, sym_name);
    } else {
...
bool ny_native_x86_64_emit_nir(ny_native_writer_t *w,
  ...
  for (size_t i = 0; i < nyir->len; ++i) {
    if (ny_trace_enabled("NY_TRACE_EMIT"))
      fprintf(stderr, "[emit %s] i=%zu op=%s\n", name, i,
              nyir_op_name(nyir->data[i].op));
...
static bool ny_x64_mach_is_f64_operand(const ny_mach_func_t *mach,
                                const ny_mach_operand_t *op) {
  if (!op) return false;
  if (op->kind == NY_MACH_OPERAND_VREG) {
    return op->as.reg < mach->vreg_len &&
           mach->vreg_types[op->as.reg] == NY_MACH_TYPE_F64;
  }
  if (op->kind == NY_MACH_OPERAND_FRAME) {
    return op->as.frame_index < mach->frame_slot_len &&
           mach->frame_slots[op->as.frame_index].type == NY_MACH_TYPE_F64;
  }
  return false;
}
...
static bool ny_x64_mach_scalar_supported(const ny_mach_func_t *mach,
  ...
  for (size_t i = 0; i < mach->inst_len; ++i) {
    ...
    case NY_MACH_SIN:
    case NY_MACH_COS:
      if (!ny_x64_mach_is_f64_operand(mach, &in->dst) ||
          !ny_x64_mach_is_f64_operand(mach, &in->src0))
        return false;
      break;
    case NY_MACH_LOAD:
    ...
    case NY_MACH_SELECT:
      if (!(ny_x64_mach_is_i64(mach, &in->dst) &&
            ny_x64_mach_is_i64(mach, &in->src0) &&
            ny_x64_mach_is_i64(mach, &in->src1) &&
            ny_x64_mach_is_i64(mach, &in->src2) &&
            in->condition == NY_MACH_COND_NE))
        return false;
      break;
    case NY_MACH_CMP:
      if (!(ny_x64_mach_is_i64(mach, &in->dst) &&
...
static long long ny_x64_mach_offset_ll(const ny_mach_func_t *mach,
                                       const ny_mach_operand_t *op) {
  /*
   * Offsets are printed into "-%d(%%rbp)" strings; compute in 64-bit so
   * large frame/vreg indices cannot overflow int (UBSan caught
   * 452884203*8 here on big functions).
   */
  long long v = op->kind == NY_MACH_OPERAND_FRAME
                    ? (long long)(op->as.frame_index + 1) * 8
                    : ((long long)mach->frame_slot_len + op->as.reg + 1) * 8;
  return v;
}
static int ny_x64_mach_offset(const ny_mach_func_t *mach,
                             const ny_mach_operand_t *op) {
  long long v = ny_x64_mach_offset_ll(mach, op);
...
static const char *ny_x64_mach_setcc(ny_mach_cond_t condition) {
  ...
  case NY_MACH_COND_B: return "setb";
  case NY_MACH_COND_AE: return "setae";
  case NY_MACH_COND_BE: return "setbe";
  case NY_MACH_COND_A: return "seta";
  default: return NULL;
  }
}
...
bool ny_native_x86_64_emit_mach_scalar(
    ny_native_writer_t *w, const ny_native_target_info_t *target,
    const ny_mach_func_t *mach, const char *func_name, bool tag_return,
    char *err, size_t err_len) {
  const char *name = func_name && func_name[0] ? func_name : "rt_main";
  if (!w || !target)
    return false;
  if (!ny_x64_mach_scalar_supported(mach, target, name)) {
    if (getenv("NY_DUMP_MACH"))
      ny_mach_dump(mach, stderr);
    if (err && err_len)
      ny_native_set_err(err, err_len,
                        "x86-64 scalar machine form rejected for %s", name);
    return false;
  }
  if (tag_return) {
    for (size_t i = 0; i < mach->inst_len; ++i)
  ...
  for (size_t block = 0; block < mach->block_len; ++block) {
    ...
    for (size_t n = 0; n < b->inst_count; ++n) {
      ...
      case NY_MACH_STORE:
        if (ny_x64_mach_is_float(mach, &in->src0)) {
          ...
          if (in->dst.kind == NY_MACH_OPERAND_FRAME) {
            ...
            /*
             * Indirect store: MACH_STORE convention is dst=POINTER,
             * src0=VALUE. The previous form loaded the value bits into
             * rax and dereferenced them (wild read + garbage store).
             */
            if (!ny_native_printf(w, "\tmovq\t-%d(%%rbp), %%rax\n", a)) return false;
            if (!ny_native_printf(w, "\tmovq\t-%d(%%rbp), %%rcx\n", dst)) return false;
            if (!ny_native_printf(w, "\t%s\t(%%rcx), %%xmm0\n", move)) return false;
            if (!ny_native_printf(w, "\t%s\t%%xmm0, (%%rax)\n", move)) return false;
          }
          rax_cached_off = -1;
          break;
        }
      ...
      case NY_MACH_SHL: case NY_MACH_SAR: case NY_MACH_ROR:
        ...
        if (!ny_native_printf(w, "\tmovq\t-%d(%%rbp), %%rcx\n\t%s\t%%cl, %%rax\n\tmovq\t%%rax, -%d(%%rbp)\n",
                              b_off,
                              in->opcode == NY_MACH_SHL ? "shlq"
                                  : in->opcode == NY_MACH_SAR ? "sarq"
                                                              : "rorq",
                              dst)) return false;
        rax_cached_off = in->dst.kind == NY_MACH_OPERAND_VREG ? dst : -1;
        break;
      case NY_MACH_ROR32:
        if (!ny_native_printf(w, "\tmovl\t-%d(%%rbp), %%eax\n\tmovq\t-%d(%%rbp), %%rcx\n\trorl\t%%cl, %%eax\n\tmovq\t%%rax, -%d(%%rbp)\n",
                              a, b_off, dst)) return false;
        rax_cached_off = in->dst.kind == NY_MACH_OPERAND_VREG ? dst : -1;
        break;
      case NY_MACH_SELECT: {
        int false_off = ny_x64_mach_offset(mach, &in->src2);
        if (!ny_native_printf(w,
            "\tmovq\t-%d(%%rbp), %%rax\n\ttestq\t%%rax, %%rax\n\tmovq\t-%d(%%rbp), %%rax\n\tcmovne\t-%d(%%rbp), %%rax\n\tmovq\t%%rax, -%d(%%rbp)\n",
            a, false_off, b_off, dst))
          return false;
        rax_cached_off = in->dst.kind == NY_MACH_OPERAND_VREG ? dst : -1;
        break;
      }
      ...
      case NY_MACH_CALL:
        if (in->src0.kind == NY_MACH_OPERAND_SYMBOL &&
            in->src0.as.symbol &&
            strcmp(in->src0.as.symbol, "rt_sqrt_f64") == 0 &&
            in->args_len == 1) {
        ...
        for (size_t arg = 0; arg < in->args_len; ++arg) {
          ...
          if (on_stack) {
            ...
            if (!ny_native_printf(w, "\tmovq\t-%d(%%rbp), %s\n", arg_off,
        ...
        if (!ny_native_printf(w, "\tcall\t%s%s%s\n", target->symbol_prefix,
                              !in->call_is_extern &&
                                      strncmp(in->src0.as.symbol, "ny_fn_", 6) != 0
                                  ? "ny_fn_"
                                  : "",
