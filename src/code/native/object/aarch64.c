...
static bool ny_a64_append(ny_obj_buf_t *code, ny_a64_def_t *defs,
                          ...
                          size_t *reloc_count, const nyir_func_t *nyir,
                          const ny_native_target_info_t *target,
...
static bool ny_a64_parallel_task(size_t i, void *opaque) {
  ...
  ny_a64_def_t def[1];
  ny_a64_reloc_t local_relocs[256];
  size_t dc = 0, rc = 0;
  if (!ny_a64_append(&r->code, def, &dc, local_relocs, &rc,
                     &ctx->funcs[i], ctx->target, r->symbol, true, false,
                     r->error, sizeof(r->error)))
    return false;
  if (rc) {
    r->relocs = malloc(rc * sizeof(*r->relocs));
    if (!r->relocs) {
      ny_obj_free(&r->code);
      return false;
    }
    memcpy(r->relocs, local_relocs, rc * sizeof(*r->relocs));
  }
  r->reloc_count = rc;
  return true;
}
...
static bool ny_a64_append_functions_parallel(
    ...
    ny_a64_reloc_t *relocs, size_t *reloc_count,
  ...
  for (size_t i = 0; i < func_count && ok; ++i) {
    ...
    if (*def_count >= 256 || *reloc_count + r->reloc_count > 256 ||
