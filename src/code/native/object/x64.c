...
/* ------------------------------------------------------------------ */
/* machine form-owned x86-64 byte encoder (scalar stack machine).               */
/*                                                                    */
...
typedef struct {
  ...
  size_t cur_inst;
  bool *color_seeded;
  bool force_stack_reads; /* dual-write colors but load from stack */
...
static bool mach_end_segments(ny_x64_mach_enc_t *e) {
  ...
  for (uint32_t v = 0; v < e->colors_len; ++v) {
    ...
    if (!seg || seg->end != e->cur_inst)
      continue;
    if (seg->spill && seg->color >= 0 && e->color_seeded[v]) {
      ...
      if (!mach_u8(e, rex) || !mach_u8(e, 0x89) || !mach_u8(e, modrm) ||
          !mach_i32(e, off))
        return false;
...
static bool mach_commit_vreg(ny_x64_mach_enc_t *e, const ny_mach_operand_t *dst,
  ...
  if (dst && dst->kind == NY_MACH_OPERAND_VREG) {
    ...
    if (preg >= 0 && !mach_mov_reg_rax(e, preg))
      return false;
  }
  return true;
}
...
/* movdqu xmm0 <-> mem: F3 0F 6F/7F 85 disp32 (unaligned). */
static bool mach_load_xmm0_v128(ny_x64_mach_enc_t *e, int off) {
  return mach_u8(e, 0xf3) && mach_u8(e, 0x0f) && mach_u8(e, 0x6f) &&
         mach_u8(e, 0x85) && mach_i32(e, off);
}
static bool mach_store_xmm0_v128(ny_x64_mach_enc_t *e, int off) {
  return mach_u8(e, 0xf3) && mach_u8(e, 0x0f) && mach_u8(e, 0x7f) &&
         mach_u8(e, 0x85) && mach_i32(e, off);
}
...
static bool mach_load_xmm(ny_x64_mach_enc_t *e, int off, bool f32,
  ...
  return mach_u8(e, f32 ? 0xf3 : 0xf2) && mach_u8(e, 0x0f) &&
         mach_u8(e, 0x10) && mach_u8(e, 0x85 | (reg << 3)) &&
         mach_i32(e, off);
}
...
static bool mach_store_xmm(ny_x64_mach_enc_t *e, int off, bool f32,
  ...
  if (reg >= 8)
    return mach_err(e, "x64 machine form encode: unsupported XMM register");
  return mach_u8(e, f32 ? 0xf3 : 0xf2) && mach_u8(e, 0x0f) &&
         mach_u8(e, 0x11) && mach_u8(e, 0x85 | (reg << 3)) &&
         mach_i32(e, off);
}
    ...
    for (size_t n = 0; n < blk->inst_count; ++n) {
      ...
      case NY_MACH_LOAD:
        ...
        if (in->src0.kind == NY_MACH_OPERAND_FRAME) {
          ...
          if (!mach_load_rax(e, a) || !mach_u8(e, 0x48) || !mach_u8(e, 0x8b) ||
      ...
      case NY_MACH_STORE:
        ...
        if (in->dst.kind == NY_MACH_OPERAND_FRAME) {
          ...
          if (!mach_load_rcx(e, dst) || !mach_u8(e, 0x48) || !mach_u8(e, 0x89) ||
              !mach_u8(e, 0x01))
            return false;
        }
        break;
      ...
      case NY_MACH_DIV:
        ...
        if (mach_is_float(mach, &in->dst)) {
          ...
          if (!mach_load_xmm0(e, a, f32) || !mach_sse_op_mem(e, b, f32, opc) ||
              !mach_store_xmm0(e, dst, f32))
            return false;
          break;
        }
      ...
      case NY_MACH_FMA: {
        ...
        if (mach_is_float(mach, &in->dst)) {
          bool f32 = mach_is_f32(mach, &in->dst);
          if (!mach_load_xmm0(e, a, f32) || !mach_sse_op_mem(e, b, f32, 0x59) ||
      ...
      mach_div_scalar:
        ...
        if (mach_is_float(mach, &in->dst) && in->opcode == NY_MACH_DIV) {
          bool f32 = mach_is_f32(mach, &in->dst);
          if (!mach_load_xmm0(e, a, f32) || !mach_sse_op_mem(e, b, f32, 0x5e) ||
              !mach_store_xmm0(e, dst, f32))
            return false;
          break;
        }
      ...
      case NY_MACH_SHL:
      case NY_MACH_SAR:
        if (!mach_load_rax(e, a) || !mach_load_rcx(e, b))
          return false;
        if (!mach_u8(e, 0x48) || !mach_u8(e, 0xd3) ||
            !mach_u8(e, in->opcode == NY_MACH_SHL ? 0xe0 : 0xf8) ||
            !mach_commit_vreg(e, &in->dst, dst))
          return false;
        break;
      case NY_MACH_CMP: {
        if (mach_is_float(mach, &in->src0)) {
          ...
          if (f32) {
            if (!mach_u8(e, 0x0f) || !mach_u8(e, 0x2e) || !mach_u8(e, 0xc1))
              return false;
          } else if (!mach_u8(e, 0x66) || !mach_u8(e, 0x0f) || !mach_u8(e, 0x2e) ||
                     !mach_u8(e, 0xc1))
            return false;
      ...
      case NY_MACH_CALL: {
        ...
        for (size_t ai = 0; ai < in->args_len; ++ai) {
          ...
          if (in->arg_sizes && in->arg_sizes[ai] > 0) {
            if (agg_reg[ai]) {
              ...
              if (!mach_load_rax(e, aoff) || !mach_u8(e, 0x48) ||
          ...
          if (on_stack) {
            ...
            if (is_f32) {
              if (!mach_load_eax(e, aoff) || !mach_store_eax_rsp(e, stack_off))
                return false;
            } else if (!mach_load_rax(e, aoff) || !mach_u8(e, 0x48) ||
        ...
        if (in->dst.kind == NY_MACH_OPERAND_VREG) {
          if (mach_is_float(mach, &in->dst)) {
            bool f32 = mach_is_f32(mach, &in->dst);
            if (!mach_store_xmm0(e, mach_slot_off(e, &in->dst), f32))
              return false;
      ...
      case NY_MACH_RET:
        if (in->src0.kind != NY_MACH_OPERAND_NONE) {
          if (mach_is_float(mach, &in->src0)) {
            bool f32 = mach_is_f32(mach, &in->src0);
            if (!mach_load_xmm0(e, mach_slot_off(e, &in->src0), f32))
              return false;
