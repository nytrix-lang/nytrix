...
#include "code/typing/ownership.h"
#include <inttypes.h>
...
static const char *ny_stmt_kind_profile_name(int kind) {
  static const char *names[] = {
      ...
      [NY_S_ENUM] = "ENUM",
      [NY_S_MACRO] = "MACRO",
      [NY_S_INCLUDE] = "INCLUDE",
      [NY_S_DEFINE] = "DEFINE",
      [NY_S_OPERATOR] = "OPERATOR",
      [NY_S_IMPL] = "IMPL",
  };
...
/*
 * Ownership functions extracted to src/code/typing/ownership.c
 */
bool stmt_expr_is_mutating_name(const char *name);
bool stmt_expr_is_int_list_literal(codegen_t *cg, scope *scopes, size_t depth,
                                   expr_t *e);
bool stmt_expr_int_range(codegen_t *cg, scope *scopes, size_t depth, expr_t *e,
...
__attribute__((unused)) static bool stmt_call_tail_is(expr_t *e,
                                                      const char *tail) {
...
static bool stmt_params_static_list_only_uses(codegen_t *cg, scope *scopes,
  ...
  if (!params)
    return true;
  for (size_t i = 0; i < params->len; ++i) {
    if (!stmt_expr_static_list_only_uses(cg, scopes, depth, params->data[i].def,
                                         name))
      return false;
  }
  return true;
...
static bool stmt_expr_static_list_only_uses(codegen_t *cg, scope *scopes,
  ...
  case NY_E_FSTRING:
    for (size_t i = 0; i < e->as.fstring.parts.len; ++i) {
      fstring_part_t *part = &e->as.fstring.parts.data[i];
      if (part->kind == NY_FSP_EXPR &&
          !stmt_expr_static_list_only_uses(cg, scopes, depth, part->as.e, name))
        return false;
    }
    return true;
  case NY_E_LIST:
...
static const char *
stmt_params_list_fastpath_bail_reason(codegen_t *cg, scope *scopes,
                                      size_t depth, const ny_param_list *params,
...
static const char *stmt_expr_list_fastpath_bail_reason(codegen_t *cg,
  ...
  case NY_E_ASM:
    ...
    return NULL;
  case NY_E_COMPTIME:
    return stmt_list_fastpath_bail_reason(
        cg, scopes, depth, e->as.comptime_expr.body, NULL, name, allow_set_idx);
  case NY_E_FSTRING:
    for (size_t i = 0; i < e->as.fstring.parts.len; ++i) {
      fstring_part_t *part = &e->as.fstring.parts.data[i];
      if (part->kind != NY_FSP_EXPR)
...
static bool stmt_expr_single_elem_elide_violates(codegen_t *cg, scope *scopes,
  ...
  case NY_E_CALL: {
    ...
    if (stmt_expr_single_elem_elide_violates(cg, scopes, depth,
      ...
      return true;
    for (size_t i = 0; i < e->as.call.args.len; ++i)
      if (stmt_expr_single_elem_elide_violates(
  ...
  case NY_E_TERNARY:
    return stmt_expr_single_elem_elide_violates(cg, scopes, depth,
                                                e->as.ternary.cond, name) ||
           stmt_expr_single_elem_elide_violates(
               cg, scopes, depth, e->as.ternary.true_expr, name) ||
           stmt_expr_single_elem_elide_violates(cg, scopes, depth,
                                                e->as.ternary.false_expr, name);
  case NY_E_LIST:
  case NY_E_TUPLE:
...
static bool stmt_single_elem_elide_violates(codegen_t *cg, scope *scopes,
  ...
  case NY_S_GUARD:
    return stmt_expr_single_elem_elide_violates(cg, scopes, depth,
           ...
           stmt_single_elem_elide_violates(
               cg, scopes, depth, s->as.guard.fallback, decl_stmt, name);
  case NY_S_WHILE:
    return stmt_single_elem_elide_violates(cg, scopes, depth, s->as.whl.init,
           ...
           stmt_single_elem_elide_violates(cg, scopes, depth, s->as.whl.body,
                                           decl_stmt, name) ||
           stmt_single_elem_elide_violates(cg, scopes, depth, s->as.whl.update,
                                           decl_stmt, name);
  case NY_S_FOR:
    if ((s->as.fr.iter_var && strcmp(s->as.fr.iter_var, name) == 0) ||
        (s->as.fr.iter_index_var && strcmp(s->as.fr.iter_index_var, name) == 0))
      return true;
  ...
  case NY_S_MATCH:
    ...
    return stmt_single_elem_elide_violates(
        cg, scopes, depth, s->as.match.default_conseq, decl_stmt, name);
  default:
...
static bool stmt_can_elide_single_elem_list(codegen_t *cg, scope *scopes,
  ...
  for (size_t i = 0; i < cg->prog->body.len; ++i) {
    if (stmt_single_elem_elide_violates(
            cg, scopes, depth, cg->prog->body.data[i], decl_stmt, name))
      return false;
  }
  return true;
}
...
static size_t stmt_count_mut_int_list_literals(codegen_t *cg, scope *scopes,
  ...
  case NY_S_VAR: {
    ...
    for (size_t i = 0; i < lim; ++i) {
      if (stmt_expr_static_int_list_shape(
              cg, scopes, depth, s->as.var.exprs.data[i], NULL, NULL, NULL))
        ++n;
    }
    return n;
  }
...
static bool stmt_raw_f64_set_values_valid(codegen_t *cg, scope *scopes,
  ...
  case NY_S_EXPR:
    return stmt_expr_raw_f64_set_values_valid(cg, scopes, depth,
                                              s->as.expr.expr, name);
  case NY_S_IF:
    return stmt_expr_raw_f64_set_values_valid(cg, scopes, depth, s->as.iff.test,
                                              name) &&
  ...
  case NY_S_WHILE:
    return stmt_raw_f64_set_values_valid(cg, scopes, depth, s->as.whl.init,
                                         name) &&
           stmt_expr_raw_f64_set_values_valid(cg, scopes, depth, s->as.whl.test,
                                              name) &&
  ...
  case NY_S_FOR:
    return stmt_raw_f64_set_values_valid(cg, scopes, depth, s->as.fr.init,
                                         name) &&
           stmt_expr_raw_f64_set_values_valid(cg, scopes, depth, s->as.fr.cond,
                                              name) &&
           stmt_expr_raw_f64_set_values_valid(cg, scopes, depth,
                                              s->as.fr.iterable, name) &&
...
static void stmt_init_raw_f64_list_storage(codegen_t *cg, scope *scopes,
  ...
  for (size_t i = 0; i < seed_len; ++i) {
    seed_vals[i] =
        gen_expr_as_f64(cg, scopes, depth, seed->as.list_like.data[i]);
    if (!seed_vals[i]) {
...
static bool stmt_expr_is_append_to_name(expr_t *e, const char *name,
  ...
  if (e->kind == NY_E_MEMCALL && e->as.memcall.name &&
      ny_name_tail_is(e->as.memcall.name, "append") && e->as.memcall.target &&
...
static bool stmt_expr_is_list_op_to_name(expr_t *e, const char *name) {
  if (!e || !name)
    return false;
  if (e->kind == NY_E_MEMCALL && e->as.memcall.name && e->as.memcall.target &&
      e->as.memcall.target->kind == NY_E_IDENT &&
      e->as.memcall.target->as.ident.name &&
      strcmp(e->as.memcall.target->as.ident.name, name) == 0) {
    const char *m = e->as.memcall.name;
    return ny_name_tail_is(m, "extend") || ny_name_tail_is(m, "slice") ||
           ny_name_tail_is(m, "concat") || ny_name_tail_is(m, "push") ||
           ny_name_tail_is(m, "insert") || ny_name_tail_is(m, "clear") ||
           ny_name_tail_is(m, "clone") || ny_name_tail_is(m, "reverse") ||
           ny_name_tail_is(m, "sort") || ny_name_tail_is(m, "sorted");
  }
  if (e->kind == NY_E_CALL && e->as.call.callee &&
      e->as.call.callee->kind == NY_E_IDENT &&
    ...
    if (target && target->kind == NY_E_IDENT && target->as.ident.name &&
        strcmp(target->as.ident.name, name) == 0) {
      return ny_name_tail_is(callee, "extend") ||
             ny_name_tail_is(callee, "slice") ||
             ny_name_tail_is(callee, "concat") ||
             ny_name_tail_is(callee, "push") ||
             ny_name_tail_is(callee, "insert") ||
             ny_name_tail_is(callee, "clear") ||
             ny_name_tail_is(callee, "clone") ||
             ny_name_tail_is(callee, "reverse") ||
             ny_name_tail_is(callee, "sort") ||
             ny_name_tail_is(callee, "sorted");
    }
  }
  return false;
}
...
static bool stmt_loop_assigns_name_unproven_int(codegen_t *cg, scope *scopes,
  ...
  case NY_S_IF:
    return stmt_loop_assigns_name_unproven_int(cg, scopes, depth,
           ...
           stmt_loop_assigns_name_unproven_int(cg, scopes, depth, s->as.iff.alt,
                                               name);
  case NY_S_GUARD:
    return stmt_loop_assigns_name_unproven_int(cg, scopes, depth,
                                               s->as.guard.fallback, name);
  ...
  case NY_S_FOR:
    return stmt_loop_assigns_name_unproven_int(cg, scopes, depth, s->as.fr.init,
                                               name) ||
           stmt_loop_assigns_name_unproven_int(cg, scopes, depth, s->as.fr.body,
                                               name) ||
           stmt_loop_assigns_name_unproven_int(cg, scopes, depth,
                                               s->as.fr.update, name);
  case NY_S_TRY:
    return stmt_loop_assigns_name_unproven_int(cg, scopes, depth, s->as.tr.body,
                                               name) ||
           stmt_loop_assigns_name_unproven_int(cg, scopes, depth,
                                               s->as.tr.handler, name);
  case NY_S_DEFER:
    return stmt_loop_assigns_name_unproven_int(cg, scopes, depth, s->as.de.body,
                                               name);
  case NY_S_MATCH:
    for (size_t i = 0; i < s->as.match.arms.len; ++i)
...
static void
stmt_var_setup_local_binding(codegen_t *cg, scope *scopes, size_t depth,
                             stmt_t *var_stmt, sema_var_t *sema, size_t idx,
                             const char *name, const char *decl_type,
                             bool decl_type_explicit, bool prefer_direct_locals,
                             bool *bind_direct, LLVMValueRef *slot) {
...
bool stmt_expr_is_mutating_name(const char *name) {
  static const char *const k_names[] = {
      "add",  "append", "extend", "sub", "remove", "sort", "clear",
      "push", "insert", "set",    "put", "delete", "del",  NULL};
  if (!name)
    return false;
...
static bool stmt_strict_condition_type_allowed(const char *type_name) {
  return !type_name || !type_name[0] || strcmp(type_name, "bool") == 0 ||
         strcmp(type_name, "any") == 0;
}

static bool stmt_strict_condition_is_valid(codegen_t *cg, scope *scopes,
                                           size_t depth, expr_t *e) {
  if (!cg || !cg->strict_types || !e || ny_is_stdlib_tok(e->tok))
    return true;
  const char *type_name = infer_expr_type(cg, scopes, depth, e);
  if (stmt_strict_condition_type_allowed(type_name))
    return true;
  ny_diag_error(e->tok, "strict condition must be bool, got %s", type_name);
  cg->had_error = 1;
  return false;
}
...
static LLVMValueRef stmt_gen_cond_i1(codegen_t *cg, scope *scopes, size_t depth,
  ...
  if (!stmt_strict_condition_is_valid(cg, scopes, depth, e))
    return LLVMConstInt(ny_i1_ty(cg), 0, false);
...
static void match_bind_adt_pattern(codegen_t *cg, scope *scopes, size_t depth,
  ...
  for (size_t i = 0; i < argc; i++) {
    ...
    if (arg->name) {
      ...
      if (i >= mem->fields.len) {
        ny_diag_error(pat->tok,
                      "too many positional fields in ADT match pattern '%s.%s'",
                      owner->name, mem->name);
        cg->had_error = 1;
        continue;
      }
...
static LLVMValueRef stmt_try_dynamic_while_trip_count_raw(codegen_t *cg,
  ...
  LLVMValueRef span =
      ny_sub(cg, rhs_raw, LLVMConstInt(cg->type_i64, (uint64_t)start_lo, true),
  ...
  if (step == 1)
    return nonneg_span;
  LLVMValueRef numerator = ny_add(
      cg, nonneg_span, LLVMConstInt(cg->type_i64, (uint64_t)(step - 1), true),
...
static bool stmt_expr_is_self_str_concat(codegen_t *cg, scope *scopes,
                                         size_t depth, expr_t *e,
                                         const char *name, expr_t **piece_out) {
  if (piece_out)
    *piece_out = NULL;
...
static void stmt_collect_str_append_candidate(codegen_t *cg, scope *scopes,
  ...
  case NY_S_BLOCK:
    for (size_t i = 0; i < s->as.block.body.len; ++i)
      stmt_collect_str_append_candidate(
          cg, scopes, depth, s->as.block.body.data[i], name_out, ambiguous);
    return;
  case NY_S_VAR:
    if (!s->as.var.is_decl && s->as.var.names.len == 1 &&
        s->as.var.exprs.len == 1 && s->as.var.names.data[0] &&
        stmt_expr_is_self_str_concat(cg, scopes, depth, s->as.var.exprs.data[0],
  ...
  case NY_S_MATCH:
    for (size_t i = 0; i < s->as.match.arms.len; ++i)
      stmt_collect_str_append_candidate(cg, scopes, depth,
                                        ...
                                        name_out, ambiguous);
    stmt_collect_str_append_candidate(
        cg, scopes, depth, s->as.match.default_conseq, name_out, ambiguous);
    return;
  default:
    return;
  }
...
static bool stmt_str_append_body_only_uses(codegen_t *cg, scope *scopes,
  ...
  case NY_S_IF:
    return !stmt_contains_ident_name(s->as.iff.init, name) &&
           !stmt_expr_contains_ident_name(s->as.iff.test, name) &&
           stmt_str_append_body_only_uses(cg, scopes, depth, s->as.iff.conseq,
                                          name) &&
  ...
  case NY_S_MATCH:
    ...
    for (size_t i = 0; i < s->as.match.arms.len; ++i) {
      ...
      if (stmt_expr_contains_ident_name(arm->guard, name))
        return false;
      if (!stmt_str_append_body_only_uses(cg, scopes, depth, arm->conseq, name))
        return false;
    }
...
static bool stmt_begin_str_append_loop(codegen_t *cg,
  ...
  LLVMValueRef builder = LLVMBuildCall2(
      cg->builder, new_sig->type, new_sig->value, (LLVMValueRef[]){cap}, 1,
...
static void stmt_finish_str_append_loop(codegen_t *cg,
  ...
  LLVMValueRef out = LLVMBuildCall2(
      cg->builder, to_str_sig->type, to_str_sig->value,
      (LLVMValueRef[]){loop->builder}, 1, NY_LLVM_NAME(cg, "str_loop_result"));
...
static bool stmt_try_emit_active_str_append_assignment(
    codegen_t *cg, scope *scopes, size_t depth, const char *name, expr_t *rhs) {
...
static bool stmt_expr_is_dict_set_to_name(expr_t *e, const char *name) {
  if (!e || !name)
    return false;
  if (e->kind == NY_E_MEMCALL && e->as.memcall.name &&
      ny_name_tail_is(e->as.memcall.name, "set") && e->as.memcall.args.len == 2)
...
stmt_find_dict_set_snapshot(stmt_loop_dict_set_snapshot_t *snaps, size_t count,
                            binding *b) {
  if (!snaps || !b)
    return NULL;
  for (size_t i = 0; i < count; ++i) {
...
stmt_record_loop_dict_set_snapshot(stmt_loop_dict_set_snapshot_t *snaps,
                                   size_t *count, size_t cap, binding *b) {
  const char *bt =
      b ? (b->decl_type_name ? b->decl_type_name : b->type_name) : NULL;
...
static void stmt_collect_loop_dict_set_snapshots(
  ...
  case NY_S_VAR:
    ...
    for (size_t i = 0;
      ...
      const char *name = s->as.var.names.data[i];
      if (!name ||
          !stmt_expr_is_dict_set_to_name(s->as.var.exprs.data[i], name))
        continue;
      binding *b =
          stmt_lookup_binding(cg, scopes, depth, name, strlen(name), 0);
...
static void stmt_reserve_loop_dict_sets(codegen_t *cg,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (!ny_mul_range_ok(snap->sets_per_iter, trip_count, &additional) ||
      ...
      continue;
    LLVMValueRef cur =
        ny_load(cg, b->value, NY_LLVM_NAME(cg, "dict_reserve_cur"));
    LLVMValueRef add =
        LLVMConstInt(cg->type_i64, ((uint64_t)additional << 1) | 1u, false);
    LLVMValueRef out = LLVMBuildCall2(
        cg->builder, reserve_sig->type, reserve_sig->value,
        (LLVMValueRef[]){cur, add}, 2, NY_LLVM_NAME(cg, "dict_reserve"));
    ny_store(cg, b->value, out);
  }
}

static void
stmt_reserve_loop_dict_sets_dynamic(codegen_t *cg,
                                    stmt_loop_dict_set_snapshot_t *snaps,
                                    size_t count, LLVMValueRef trip_count_raw) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (snap->sets_per_iter != 1)
      additional = LLVMBuildMul(
          cg->builder, additional,
          LLVMConstInt(cg->type_i64, (uint64_t)snap->sets_per_iter, false),
          NY_LLVM_NAME(cg, "dict_reserve_additional_raw"));
    LLVMValueRef tagged =
        ny_or(cg,
              ny_shl(cg, additional, ny_c1(cg),
...
static void
stmt_reserve_loop_append_lists(codegen_t *cg,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (!ny_mul_range_ok(snap->appends_per_iter, trip_count, &total_delta) ||
      ...
      continue;
    LLVMValueRef cur =
        ny_load(cg, b->value, NY_LLVM_NAME(cg, "list_reserve_cur"));
    LLVMValueRef cap =
        LLVMConstInt(cg->type_i64, ((uint64_t)desired << 1) | 1u, false);
    LLVMValueRef out = LLVMBuildCall2(
        cg->builder, reserve_sig->type, reserve_sig->value,
        (LLVMValueRef[]){cur, cap}, 2, NY_LLVM_NAME(cg, "list_reserve"));
    ny_store(cg, b->value, out);
  }
}
...
static bool stmt_extract_list_sum_body(codegen_t *cg, scope *scopes,
                                       size_t depth, stmt_t *body,
                                       stmt_t *update, const char *list_name,
                                       const char *index_name,
                                       const char **acc_name_out,
                                       bool *requires_int_list_proof) {
  if (requires_int_list_proof)
...
static bool stmt_match_list_sum_loop(codegen_t *cg, scope *scopes, size_t depth,
                                     stmt_t *s, stmt_list_sum_loop_t *out) {
  if (out)
  ...
  if (!stmt_extract_list_sum_body(
          cg, scopes, depth, s->as.whl.body, s->as.whl.update, list_name,
          lhs->as.ident.name, &acc_name, &requires_int_list_proof) ||
      !acc_name)
    return false;
  ...
  const char *list_type = list_b
                              ? (list_b->decl_type_name ? list_b->decl_type_name
                                                        : list_b->type_name)
                              : NULL;
  bool list_ok =
  ...
  if (!list_b || list_b->static_indexable_object_elided || !idx_b || !acc_b ||
      ...
      !acc_b->value || !list_ok || !idx_b->is_int_slot || !acc_b->is_int_slot)
    return false;
...
static bool stmt_try_emit_list_sum_loop(codegen_t *cg, scope *scopes,
  ...
  ny_dbg_loc(cg, s->tok);
  LLVMValueRef list_v = loop.list_binding->is_slot
  ...
  LLVMValueRef idx_raw = loop.index_binding->raw_int_value
                             ? ny_load(cg, loop.index_binding->raw_int_value,
                                       NY_LLVM_NAME(cg, "list_sum_idx_raw"))
                             : ny_untag_int(cg, idx_v);
  LLVMValueRef list_ptr = LLVMBuildIntToPtr(
      cg->builder, list_v, ny_ptr_i64_ty(cg), NY_LLVM_NAME(cg, "list_sum_ptr"));
  LLVMValueRef len_v = ny_load(cg, list_ptr, NY_LLVM_NAME(cg, "list_sum_len"));
  ...
  LLVMValueRef acc_raw = loop.acc_binding->raw_int_value
                             ? ny_load(cg, loop.acc_binding->raw_int_value,
                                       NY_LLVM_NAME(cg, "list_sum_acc_raw"))
                             : ny_untag_int(cg, acc_v);
  LLVMValueRef new_raw = ny_add(cg, acc_raw, ny_untag_int(cg, sum_v),
...
bool ny_expr_is_pure(codegen_t *cg, scope *scopes, size_t depth, expr_t *e) {
  ...
  case NY_E_TERNARY:
    return ny_expr_is_pure(cg, scopes, depth, e->as.ternary.cond) &&
           ny_expr_is_pure(cg, scopes, depth, e->as.ternary.true_expr) &&
           ny_expr_is_pure(cg, scopes, depth, e->as.ternary.false_expr);
  case NY_E_INDEX: {
  ...
  case NY_E_CALL:
    if (!stmt_auto_simd_call_is_effect_free(cg, &e->as.call))
      return false;
    for (size_t i = 0; i < e->as.call.args.len; ++i) {
      if (!ny_expr_is_pure(cg, scopes, depth, e->as.call.args.data[i].val))
        return false;
    }
    return true;
  case NY_E_LIST:
...
static void gen_stmt_while(codegen_t *cg, scope *scopes, size_t *depth,
  ...
  bool invariant_ok =
      !s->as.whl.invariant ||
      ny_proof_check_loop_invariant(cg, scopes, *depth, s->as.whl.invariant);
  ...
  bool range_update_safe =
      ...
      !stmt_loop_assigns_name_unproven_int(cg, scopes, *depth, s->as.whl.body,
                                           range_lhs_name) &&
      !stmt_loop_assigns_name_unproven_int(cg, scopes, *depth, s->as.whl.update,
                                           range_lhs_name);
  bool has_trip_count_hint = range_control_safe && range_update_safe &&
                             stmt_try_while_trip_upper_bound(
                                 cg, scopes, *depth, s, &trip_count_hint) &&
                             trip_count_hint > 0;
  ...
  if ((has_trip_count_hint || dynamic_trip_count_raw) &&
    ...
    stmt_collect_loop_dict_set_snapshots(cg, scopes, *depth, s->as.whl.body,
                                         dict_set_snaps, &dict_set_snap_count,
                                         sizeof(dict_set_snaps) /
                                             sizeof(dict_set_snaps[0]));
    stmt_reserve_loop_append_lists(cg, append_len_snaps, append_len_snap_count,
                                   trip_count_hint);
    if (has_trip_count_hint)
      stmt_reserve_loop_dict_sets(cg, dict_set_snaps, dict_set_snap_count,
                                  trip_count_hint);
    else
      stmt_reserve_loop_dict_sets_dynamic(
          cg, dict_set_snaps, dict_set_snap_count, dynamic_trip_count_raw);
  }
  LLVMValueRef f = ny_cur_fn(cg);
  ...
  binding *while_lhs_binding = stmt_while_lhs_binding(cg, scopes, *depth, s);
...
static bool stmt_try_emit_fast_range_for(codegen_t *cg, scope *scopes,
  ...
  if (ny_env_enabled_default_on("NYTRIX_STRING_APPEND_LOOP_BUILDER") &&
    ...
    const char *name = NULL;
    bool ambiguous = false;
    stmt_collect_str_append_candidate(cg, scopes, *depth, s->as.fr.body, &name,
                                      &ambiguous);
    if (name && !ambiguous &&
        stmt_str_append_body_only_uses(cg, scopes, *depth, s->as.fr.body,
                                       name)) {
      bool is_global = false;
      binding *b =
          stmt_var_lookup_existing(cg, scopes, *depth, name, &is_global);
      if (b && b->is_mut && b->is_slot && b->value) {
        const char *bt = b->decl_type_name ? b->decl_type_name : b->type_name;
        if (ny_type_is(bt, "str")) {
          ...
          use_str_append_range =
              stmt_begin_str_append_loop(cg, &str_append_range_loop);
        }
      }
    }
  }
  ...
  ny_pos(cg, cb);
  LLVMValueRef cur_raw =
      ny_phi(cg, cg->type_i64, NY_LLVM_NAME(cg, "for_range_cur"));
  LLVMValueRef idx_raw =
      ny_phi(cg, cg->type_i64, NY_LLVM_NAME(cg, "for_range_idx"));
  ...
  ny_dbg_loc(cg, s->tok);
  ny_cond_br(cg,
             ny_slt(cg, cur_raw, stop_raw, NY_LLVM_NAME(cg, "for_range_cond")),
             bb, eb);
  ...
  LLVMValueRef latch_br = ny_br(cg, cb);
  apply_loop_metadata(
      cg, latch_br, s->as.fr.attr_unroll, s->as.fr.attr_nounroll,
      s->as.fr.attr_vectorize,
      stmt_auto_simd_body_safe(cg, scopes, *depth, s->as.fr.body));
...
static void gen_stmt_for(codegen_t *cg, scope *scopes, size_t *depth, stmt_t *s,
  ...
  LLVMValueRef get_args[3] = {itv, i_val, ny_c1(cg)};
    ...
    if (name && !ambiguous &&
      ...
      if (b && b->is_mut && b->is_slot && b->value) {
        ...
        if (ny_type_is(bt, "str")) {
          ...
          use_str_append_iter =
              stmt_begin_str_append_loop(cg, &str_append_iter_loop);
        }
      }
    }
  }
...
static void gen_stmt_struct(codegen_t *cg, stmt_t *s) {
  ...
  if (s->kind == NY_S_LAYOUT && s->as.layout.deftype_params.len > 0) {
     ...
     * bound (Buf<16>) or the param's `= default` literal
     * (ny_layout_array_extent in func.c).
     */
 ...
 */
static void gen_stmt_var(codegen_t *cg, scope *scopes, size_t *depth,
                         stmt_t *s) {
  ...
  if (dest) {
    NY_COMPILER_ASSERTF(s->as.var.exprs.len == 1,
                        "destructure expects exactly one source expr, got %zu",
                        s->as.var.exprs.len);
  } else if (s->as.var.exprs.len > 1) {
    NY_COMPILER_ASSERTF(s->as.var.exprs.len == s->as.var.names.len,
  ...
  if (parallel && s->as.var.names.len == 1 && s->as.var.exprs.len == 1) {
    ...
    bool first_escapes = sema && sema->escapes.len > 0 && sema->escapes.data[0];
    ...
    if (!first_static_list_object_elided &&
      ...
      parallel = false;
               ...
               stmt_can_elide_single_elem_list(cg, scopes, *depth, s,
                                               first_name, first_escapes, e0)) {
  ...
  if (!parallel && !first_static_list_object_elided &&
    ...
    if (!dest && first_name &&
      ...
      first_val = gen_expr_list_stack_alloc(cg, scopes, *depth, e0);
             ...
             stmt_can_elide_single_elem_list(cg, scopes, *depth, s, first_name,
                                             first_escapes, e0)) {
      first_val = gen_expr(cg, scopes, *depth, e0->as.list_like.data[0]);
      first_single_elem_elided = true;
    } else
  ...
  if (!s->as.var.is_decl && parallel && s->as.var.names.len > 1) {
    ...
    if (!parallel_assign_values) {
      ny_diag_error(s->tok, "out of memory while preparing grouped assignment");
      cg->had_error = 1;
      return;
    }
  ...
  for (size_t i = 0; i < s->as.var.names.len; i++) {
    ...
    if (!decl_type && expr_for_check) {
      ...
      if ((cg->strict_types && (stmt_bindable_inferred_type(inf) ||
    ...
    if (*depth == 0 && top_entry_can_hoist && !s->as.var.is_decl) {
      ...
      binding *existing =
          stmt_var_lookup_existing(cg, scopes, *depth, n, &existing_is_global);
    ...
    if (*depth == 0 && !top_entry_local_path) {
      ...
      if (gb) {
        ...
        bool sema_global_f64 =
            sema && sema->is_f64_proven.len > i && sema->is_f64_proven.data[i];
        ...
        if (s->tok.filename) {
          ...
          ny_diag_fix("declare with %sdef %s = ...%s or %smut %s = ...%s at "
                      "the top level",
                      clr(NY_CLR_BOLD), n, clr(NY_CLR_RESET), clr(NY_CLR_BOLD),
                      n, clr(NY_CLR_RESET));
        }
      }
    }
    ...
    if (*depth > 0 || top_entry_local_path) {
      if (s->as.var.is_decl) {
        stmt_var_setup_local_binding(cg, scopes, *depth, s, sema, i, n,
                                     ...
                                     prefer_direct_locals, &bind_direct, &slot);
      } else {
        bool target_is_global = false;
        binding *eb =
            stmt_var_lookup_existing(cg, scopes, *depth, n, &target_is_global);
        if (eb) {
          ...
          stmt_var_setup_local_binding(
              cg, scopes, *depth, s, sema, i, n, decl_type, decl_type_explicit,
              prefer_direct_locals, &bind_direct, &slot);
        }
      }
    }
    ...
    if (expr_for_check && scopes[*depth].loop_trip_hint > 0) {
      stmt_try_widen_loop_accumulator_binding(
          cg, scopes, *depth, n, expr_for_check, scopes[*depth].loop_trip_hint);
    }

    if ((target_is_f64_slot || target_is_f32_slot) && slot && expr_for_check &&
    ...
    bool is_f64_expr = stmt_type_name_is_f64_value(decl_type) ||
    ...
    if (!rhs_self_dest && slot) {
      size_t nlen = strlen(n);
      rhs_self_dest = stmt_lookup_binding_no_mark(scopes, *depth, n, nlen, 0);
    }
    bool rhs_pre_proven_int =
        expr_for_check &&
        ny_is_proven_int(cg, scopes, *depth, expr_for_check, NULL);
    bool suppress_self_raw_int =
        rhs_self_dest && rhs_self_dest->is_int_slot && expr_for_check &&
        stmt_expr_contains_ident_name(expr_for_check, n) && !rhs_pre_proven_int;
    ...
    if (direct_native_float_candidate) {
      ...
      target_val = LLVMBuildCall2(cg->builder, gs->type, gs->value,
    ...
    if (expr_for_check) {
      ...
      if (ny_expr_is_list_or_tuple_lit(expr_for_check) ||
        ...
        rhs_f64_list_storage =
            stmt_expr_is_list_ctor(expr_for_check) || static_f64_shape;
        ...
        if (rhs_int_list_storage && static_int_seed) {
          rhs_has_list_int_range =
              stmt_int_list_literal_range(cg, scopes, *depth, static_int_seed,
        ...
        if (stmt_expr_is_append_to_name(expr_for_check, n, &append_value)) {
        ...
        } else if (stmt_expr_is_list_op_to_name(expr_for_check, n)) {
          binding *src = resolved_local ? resolved_local : resolved_global;
          if (!src) {
            size_t nlen = strlen(n);
            src = stmt_lookup_binding(cg, scopes, *depth, n, nlen, 0);
          ...
          if (src && src->is_list_storage) {
            rhs_list_storage = true;
            rhs_int_list_storage = src->is_int_list_storage;
            rhs_f64_list_storage = src->is_f64_list_storage;
          }
        }
      }
    }
    if (bind_direct) {
      ...
      if (b) {
        if (!decl_type_explicit)
          b->decl_type_name = NULL;
        stmt_update_numeric_binding_proof(cg, b, is_int_direct, is_f64_direct);
        stmt_update_int_binding_range(b, rhs_has_int_range, rhs_int_min_raw,
                                      rhs_int_max_raw);
        stmt_update_list_binding_proof(
            b, rhs_list_storage, rhs_int_list_storage, rhs_f64_list_storage);
        ...
        if (first_raw_int_list_mutation && i == 0)
          stmt_init_raw_int_list_storage(cg, scopes, *depth, b, expr_for_check);
        else if (first_raw_f64_list_mutation && i == 0)
          stmt_init_raw_f64_list_storage(cg, scopes, *depth, b, expr_for_check);
        else if (i == 0) {
        ...
        stmt_ownership_post_store(cg, scopes, *depth, b, expr_for_check, s->tok,
                                  false);
      }
    ...
    if (resolved_local && !resolved_local->is_slot) {
      ...
      stmt_update_dict_binding_range(resolved_local, rhs_has_dict_int_range,
                                     rhs_dict_min_raw, rhs_dict_max_raw);
      stmt_update_direct_callable_binding(cg, resolved_local, expr_for_check);
      if (first_static_list_object_elided && i == 0)
      ...
      if (first_raw_int_list_mutation && i == 0)
        ...
        if (first_raw_int_list_bail_reason)
          resolved_local->raw_int_list_bail_reason =
              first_raw_int_list_bail_reason;
        if (first_raw_f64_list_bail_reason)
          resolved_local->raw_f64_list_bail_reason =
              first_raw_f64_list_bail_reason;
      }
    ...
    if (resolved_local) {
      ...
      stmt_update_dict_binding_range(resolved_global, rhs_has_dict_int_range,
                                     rhs_dict_min_raw, rhs_dict_max_raw);
      stmt_update_direct_callable_binding(cg, resolved_global, expr_for_check);
      if (first_static_list_object_elided && i == 0)
      ...
      if (first_raw_int_list_mutation && i == 0)
        ...
        if (first_raw_int_list_bail_reason)
          resolved_global->raw_int_list_bail_reason =
              first_raw_int_list_bail_reason;
        if (first_raw_f64_list_bail_reason)
          resolved_global->raw_f64_list_bail_reason =
              first_raw_f64_list_bail_reason;
      }
    } else if (slot) {
      size_t nlen = strlen(n);
...
static void gen_stmt_expr_stmt(codegen_t *cg, scope *scopes, size_t *depth,
  ...
  if (is_tail && !cg->result_store_val && cg->current_fn_ret_type) {
    ensure_expr_type_compatible(cg, scopes, *depth, cg->current_fn_ret_type, e,
                                e->tok, "return");
    if (cg->had_error)
      return;
    if (!cg->current_fn_attr_naked && !cg->result_store_val &&
      ...
      LLVMValueRef v =
          stmt_gen_return_value(cg, scopes, *depth, e, cg->current_fn_ret_type);
      if (!v) {
  ...
  if (is_tail && cg->result_store_val) {
    ...
    /*
     * The enclosing match owns the branch and arm-scope cleanup.
     */
    return;
  }
  ...
  if (e->kind == NY_E_CALL && e->as.call.callee &&
    ...
    (void)stmt_expr_store_back(cg, scopes, *depth, e->as.call.args.data[0].val,
                               v, s->tok);
    ...
    if (target && stored_value && target->kind == NY_E_IDENT &&
      ...
      if (b && b->is_slot && b->is_mut) {
        stmt_ownership_check_live_borrows(cg, scopes, *depth, b, s->tok,
                                          "mutate");
        if (ensure_store_ready(cg, s->tok, v, b->value, "indexed assignment")) {
      ...
      if (b && b->is_list_storage) {
        ...
        stmt_update_list_binding_proof(b, true,
                                       b->is_int_list_storage && val_is_int,
...
case NY_S_WHILE: {
  stmt_preinvalidate_loop_static_indexables(cg, scopes, *depth, s->as.whl.body);
...
case NY_S_GOTO: {
  ...
  if (lb) {
    ...
    if (*depth < target_depth) {
      ny_diag_error(
          ...
          s->as.go.name);
      ny_diag_hint("move the label to the current scope or use structured "
                   "control flow");
      cg->had_error = 1;
      break;
    }
  ...
  ny_diag_error(s->tok, "undefined label \033[1;37m'%s'\033[0m", s->as.go.name);
  cg->had_error = 1;
  break;
}
case NY_S_MACRO: {
  const char *macro_name =
      (s->as.macro.name && *s->as.macro.name) ? s->as.macro.name : "<unknown>";
  for (size_t i = 0; i < s->as.macro.args.len; i++) {
