...
ny_try_compile_time_type_builtin(codegen_t *cg, scope *scopes, size_t depth,
  ...
  if (want_is_nil) {
    /*
     * Integer zero is the tagged integer payload 1, not the nil immediate
     * 0.  Prefer the literal's representation over a conservative/nullable
     * inferred type here; otherwise `is_nil(0)` is folded to true before the
     * runtime predicate ever sees its argument.
     */
    if (ny_expr_literal_i64(arg, NULL))
      return ny_gencall_const_bool(cg, false, "ct_is_nil_integer");
    /*
     * Method/function calls may return nullable aliases (for example
     * `?self`) even when nominal inference currently exposes the underlying
     * object type.  Do not fold those calls as definitely non-nil; preserve
     * the runtime predicate until the call's nullable ABI is explicit.
     */
...
static LLVMValueRef ny_gencall_store_idx_intrinsic(
  ...
  LLVMValueRef raw_v = val_v;
  if (untag_before_store) {
    expr_t *val_expr = c->args.data[val_arg].val;
    int64_t val_lit = 0;
    binding *val_bind = ny_gencall_ident_binding(cg, scopes, depth, val_expr);
    if (val_expr && ny_expr_literal_i64(val_expr, &val_lit)) {
      raw_v = LLVMConstInt(cg->type_i64, (uint64_t)val_lit, true);
    } else if (val_bind && val_bind->raw_int_value &&
               val_bind->is_int_direct) {
      raw_v = val_bind->raw_int_value;
    } else if (val_expr &&
               ny_is_proven_int(cg, scopes, depth, val_expr, val_v)) {
      raw_v = ny_untag_int(cg, val_v);
    } else {
      raw_v = ny_build_untagged_or_raw_i64(cg, val_v, "stx_val_raw");
    }
  }
  LLVMValueRef base_ptr =
...
ny_try_fast_handle_memory_callsite(codegen_t *cg, expr_t *e, scope *scopes,
  ...
  if (want_load) {
    ...
    expr_t *fast_val_expr = (c->args.len >= 2) ? c->args.data[1].val : NULL;
    int64_t fast_val_lit = 0;
    binding *fast_val_bind =
        ny_gencall_ident_binding(cg, scopes, depth, fast_val_expr);
    LLVMValueRef raw = NULL;
    if (fast_val_expr && ny_expr_literal_i64(fast_val_expr, &fast_val_lit)) {
      raw = LLVMConstInt(cg->type_i64, (uint64_t)fast_val_lit, true);
    } else if (fast_val_bind && fast_val_bind->raw_int_value &&
               fast_val_bind->is_int_direct) {
      raw = fast_val_bind->raw_int_value;
    } else if (fast_val_expr &&
               ny_is_proven_int(cg, scopes, depth, fast_val_expr, val_v)) {
      raw = ny_untag_int(cg, val_v);
    } else {
      raw = ny_build_rt_untag_i64(cg, val_v, "fast_store64_h_raw");
    }
    if (is32)
