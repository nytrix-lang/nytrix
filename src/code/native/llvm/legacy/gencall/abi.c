...
#if UINTPTR_MAX == 0xffffffff
  tagged = ny_or(cg, tagged, LLVMConstInt(cg->type_i64, NY_NATIVE_MARK, false),
...
static bool abi_type_is_unsigned_int(const char *type_name) {
  if (!type_name)
    return false;
  type_name = abi_skip_nullable(type_name);
  static const char *const ints[] = {"u8",  "u16",  "u32",
                                     "u64", "u128", "handle"};
  return ny_gencall_str_in(type_name, ints, sizeof(ints) / sizeof(ints[0]));
}
...
static LLVMValueRef ny_try_emit_direct_list_append(codegen_t *cg, scope *scopes,
  ...
  if (target_is_direct) {
    ...
    LLVMValueRef tag_addr = ny_sub(
        cg, args[0], LLVMConstInt(cg->type_i64, 8, false), "append_tag_addr");
    LLVMValueRef tag_ptr = LLVMBuildIntToPtr(
        cg->builder, tag_addr, ny_ptr_i64_ty(cg), "append_tag_ptr");
  ...
  LLVMValueRef target_ptr = LLVMBuildIntToPtr(
      cg->builder, args[0], ny_ptr_i64_ty(cg), "append_ptr_i64");
  ...
  LLVMValueRef cap_addr = ny_add(
      cg, args[0], LLVMConstInt(cg->type_i64, 8, false), "append_cap_addr");
  LLVMValueRef cap_ptr = LLVMBuildIntToPtr(
      cg->builder, cap_addr, ny_ptr_i64_ty(cg), "append_cap_ptr_i64");
  ...
  LLVMValueRef byte_off = LLVMBuildAdd(
      cg->builder, scaled, LLVMConstInt(cg->type_i64, 16, false), "append_off");
  LLVMValueRef elem_addr =
      ny_add(cg, args[0], byte_off, NY_LLVM_NAME(cg, "append_addr"));
  LLVMValueRef elem_ptr = LLVMBuildIntToPtr(
      cg->builder, elem_addr, ny_ptr_i64_ty(cg), "append_elem_ptr_i64");
  ...
  LLVMValueRef next_len_tagged = ny_or(
      cg,
      ny_shl(cg, next_len, ny_c1(cg), NY_LLVM_NAME(cg, "append_next_len_shl")),
...
LLVMValueRef ny_static_int_list_global(codegen_t *cg, binding *b, expr_t *init,
  ...
  bool untagged = ny_fast_path_enabled(cg, "NYTRIX_UNTAGGED_INT_LIST_STORAGE");
...
ny_try_emit_raw_int_list_get(codegen_t *cg, scope *scopes, size_t depth,
  ...
  if (!assume_nonnegative) {
    LLVMValueRef neg = LLVMBuildICmp(cg->builder, LLVMIntSLT, key_raw,
                                     ny_c0(cg), "raw_list_get_neg");
    LLVMValueRef wrapped = ny_add(cg, key_raw, len_raw, "raw_list_get_wrapped");
    adj = ny_select(cg, neg, wrapped, key_raw, "raw_list_get_adj");
  }
  if (assume_in_bounds) {
    LLVMValueRef elem_ptr =
        LLVMBuildInBoundsGEP2(cg->builder, cg->type_i64, b->raw_int_list_ptr,
                              &adj, 1, "raw_int_list_get_inbounds_ptr");
    LLVMValueRef val =
  ...
  LLVMValueRef nonneg = assume_nonnegative
                            ...
                            : LLVMBuildICmp(cg->builder, LLVMIntSGE, adj,
                                            ny_c0(cg), "raw_list_get_nonneg");
  LLVMValueRef in_hi =
      LLVMBuildICmp(cg->builder, LLVMIntULT, adj, len_raw, "raw_list_get_hi");
...
static LLVMValueRef ny_box_raw_f64_list_value(codegen_t *cg, LLVMValueRef value,
                                              token_t tok) {
...
ny_try_emit_raw_f64_list_get(codegen_t *cg, scope *scopes, size_t depth,
  ...
  if (assume_in_bounds) {
    LLVMValueRef ptr =
        LLVMBuildInBoundsGEP2(cg->builder, cg->type_f64, b->raw_f64_list_ptr,
                              &adj, 1, "raw_f64_list_get_inbounds_ptr");
    LLVMValueRef loaded =
  ...
  LLVMValueRef ptr =
      LLVMBuildInBoundsGEP2(cg->builder, cg->type_f64, b->raw_f64_list_ptr,
                            &adj, 1, "raw_f64_list_get_ptr");
  LLVMValueRef loaded = ny_load(cg, ptr, NY_LLVM_NAME(cg, "raw_f64_list_get"));
...
static LLVMValueRef ny_try_emit_raw_f64_list_set(codegen_t *cg, scope *scopes,
                                                 size_t depth, expr_t *target,
  ...
  LLVMValueRef target_v =
      b->value ? b->value : gen_expr(cg, scopes, depth, target);
...
static LLVMValueRef ny_emit_trusted_fast_indexable_get(
  ...
  if (assume_in_bounds) {
    LLVMValueRef key_raw = ny_gencall_index_raw_i64(
        cg, scopes, depth, key, key_v, "trusted_get_key_raw");
  ...
  LLVMValueRef len_raw =
      ny_build_untagged_or_raw_i64(cg, len_tagged, "trusted_get_len_raw");
  LLVMValueRef key_raw = ny_gencall_index_raw_i64(cg, scopes, depth, key, key_v,
                                                  "trusted_get_key_raw");
  ...
  LLVMValueRef elem_idx =
      LLVMBuildAdd(cg->builder, adj_idx, LLVMConstInt(cg->type_i64, 2, false),
                   "trusted_get_gep_idx");
  LLVMValueRef elem_ptr = LLVMBuildGEP2(cg->builder, cg->type_i64, target_ptr,
                                        &elem_idx, 1, "trusted_get_elem_gep");
  LLVMValueRef elem_val =
...
static LLVMValueRef ny_emit_fast_list_set(codegen_t *cg, scope *scopes,
  ...
  LLVMValueRef len_raw =
      ny_build_untagged_or_raw_i64(cg, len_tagged, "fast_set_len_raw");
  LLVMValueRef key_raw = ny_gencall_index_raw_i64(cg, scopes, depth, key, key_v,
                                                  "fast_set_key_raw");
...
static LLVMValueRef ny_emit_trusted_fast_list_set(
  ...
  if (assume_existing_index) {
    LLVMValueRef key_raw = ny_gencall_index_raw_i64(
        cg, scopes, depth, key, key_v, "trusted_set_key_raw");
    LLVMValueRef elem_idx =
        LLVMBuildAdd(cg->builder, key_raw, LLVMConstInt(cg->type_i64, 2, false),
  ...
  LLVMValueRef elem_idx =
      LLVMBuildAdd(cg->builder, adj_idx, LLVMConstInt(cg->type_i64, 2, false),
                   "trusted_set_gep_idx");
  LLVMValueRef elem_ptr = LLVMBuildGEP2(cg->builder, cg->type_i64, target_ptr,
                                        &elem_idx, 1, "trusted_set_elem_gep");
  ny_store(cg, elem_ptr, value_v);
  LLVMBasicBlockRef fast_end_bb = ny_cur_block(cg);
  ny_br(cg, join_bb);
...
static LLVMValueRef ny_try_emit_fast_receiver_get(codegen_t *cg, scope *scopes,
  ...
  if (target_is_known_dict) {
    ...
    call_args[0] = ny_cast_to_i64(cg, gen_expr(cg, scopes, depth, target),
                                  "dict_get_target");
    call_args[1] =
    ...
    ny_dbg_loc(cg, e->tok);
    return LLVMBuildCall2(cg->builder, dict_get_sig->type, dict_get_sig->value,
                          call_args, 3,
  ...
  bool target_is_dynamic = !target_is_direct_list &&
                           !target_is_raw_int_list &&
                           !target_is_raw_f64_list &&
                           !target_is_known_list_like &&
                           (ny_gencall_type_is(target_type, "any") ||
                            ny_gencall_type_is(target_type, "unknown") ||
                            !target_type);
  /*
   * Inferred list values produced by a dynamic call (for example
   * `node.get("args")`) may use raw elem-8 slots even though the receiver is
   * consumed by an any-valued callback. Decode that boundary before the
  ...
  /*
   * Native typed buffers are exposed at the data pointer, so the generic
   * trusted GEP (which assumes a managed-list header) turns a negative index
   * into a read from the tbuf header.  Keep the safe negative path on the
   * runtime tbuf accessor; it performs the length-relative wrap correctly.
   */
  if (!assume_nonnegative &&
      (target_is_direct_list || target_is_raw_int_list ||
       target_is_raw_f64_list || target_is_known_list_like)) {
    LLVMValueRef target_v = ny_cast_to_i64(
        cg, gen_expr(cg, scopes, depth, target), "negative_get_target");
    LLVMValueRef key_v = ny_cast_to_i64(cg, gen_expr(cg, scopes, depth, key),
                                        "negative_get_key");
    LLVMValueRef default_v =
        default_expr
            ? ny_cast_to_i64(cg, gen_expr(cg, scopes, depth, default_expr),
                             "negative_get_default")
            : ny_c1(cg);
    LLVMValueRef key_raw = ny_gencall_index_raw_i64(
        cg, scopes, depth, key, key_v, "negative_get_key_raw");
    LLVMTypeRef args_ty[3] = {cg->type_i64, cg->type_i64, cg->type_i64};
    LLVMTypeRef fn_ty = LLVMFunctionType(cg->type_i64, args_ty, 3, 0);
    LLVMValueRef fn = ny_get_named_fn(cg, "rt_tbuf_get");
    if (!fn)
      fn = LLVMAddFunction(cg->module, "rt_tbuf_get", fn_ty);
    if (target_v && key_raw && default_v && fn)
      return LLVMBuildCall2(cg->builder, fn_ty, fn,
                            (LLVMValueRef[]){target_v, key_raw, default_v}, 3,
                            NY_LLVM_NAME(cg, "negative_tbuf_get"));
  }
  ...
  if (target_is_direct_list && !ny_env_enabled("NYTRIX_INDEX_READ_PARITY") &&
    ...
    return ny_emit_trusted_fast_indexable_get(
        cg, scopes, depth, target, key, default_expr, e->tok,
        assume_nonnegative, assume_in_bounds);
  }
...
static LLVMValueRef ny_try_emit_fast_receiver_set(codegen_t *cg, scope *scopes,
  ...
  if (ny_expr_has_known_dict_type(cg, scopes, depth, target)) {
    ...
    call_args[2] = ny_cast_to_i64(cg, gen_expr(cg, scopes, depth, value),
                                  "dict_set_value");
    if (!call_args[0] || !call_args[1] || !call_args[2]) {
    ...
    ny_dbg_loc(cg, e->tok);
    return LLVMBuildCall2(cg->builder, dict_set_sig->type, dict_set_sig->value,
                          call_args, 3,
...
ny_emit_fast_indexable_get(codegen_t *cg, scope *scopes, size_t depth,
  ...
  if (assume_in_bounds) {
    ...
    if (!assume_nonnegative) {
      ...
      LLVMValueRef wrapped = ny_add(cg, key_raw, len_raw,
                                    NY_LLVM_NAME(cg, "fast_get_wrapped_idx"));
      adj_idx = ny_select(cg, is_neg, wrapped, key_raw,
  ...
  if (!load_fn)
    load_fn = LLVMAddFunction(cg->module, "rt_load_item_any", load_fn_ty);
  LLVMValueRef elem_val =
      (load_fn && adj_idx)
          ? LLVMBuildCall2(cg->builder, load_fn_ty, load_fn,
                           (LLVMValueRef[]){target_v, adj_idx}, 2,
                           NY_LLVM_NAME(cg, "fast_get_elem_any"))
          : ny_c0(cg);
  if (probe_sig) {
    ny_dbg_loc(cg, tok);
...
static LLVMValueRef ny_emit_fast_int_dict_get(codegen_t *cg, scope *scopes,
  ...
  LLVMValueRef off =
      LLVMBuildAdd(cg->builder, idx_times_24,
                   LLVMConstInt(cg->type_i64, 0, false), "fast_dict_off");
  LLVMValueRef state_addr =
      LLVMBuildAdd(cg->builder, off, LLVMConstInt(cg->type_i64, 16, false),
                   "fast_dict_state_off");
  LLVMValueRef state_abs =
      ny_add(cg, table, state_addr, NY_LLVM_NAME(cg, "fast_dict_state_addr"));
  ...
  LLVMValueRef state_filled = LLVMBuildICmp(
      cg->builder, LLVMIntEQ, state_v, LLVMConstInt(cg->type_i64, 1, false),
      NY_LLVM_NAME(cg, "fast_dict_filled"));
  LLVMValueRef key_eq = LLVMBuildICmp(cg->builder, LLVMIntEQ, slot_key, key_v,
...
static LLVMValueRef ny_try_fast_len_builtin(codegen_t *cg, scope *scopes,
  ...
  if (static_init && static_init->kind == NY_E_LITERAL &&
    ...
    return LLVMConstInt(cg->type_i64,
                        ((uint64_t)static_init->as.literal.as.s.len << 1) | 1u,
                        false);
  }
  if (static_init &&
...
LLVMValueRef ny_box_abi_result(codegen_t *cg, LLVMValueRef v,
  ...
  if (strcmp(type_name, "cstr") == 0) {
    ...
    LLVMValueRef is_null =
        LLVMBuildIsNull(cg->builder, v, NY_LLVM_NAME(cg, "cstr_isnull"));
    LLVMValueRef native = abi_encode_native_i64(cg, raw, "ret_cstr_native");
    LLVMValueRef res =
        LLVMBuildCall2(cg->builder, box->type, box->value, &native, 1,
                       NY_LLVM_NAME(cg, "cstr_to_str"));
    return LLVMBuildSelect(cg->builder, is_null,
                           LLVMConstInt(cg->type_i64, 0, false), res,
                           NY_LLVM_NAME(cg, "cstr_select"));
  }
  if (abi_type_is_ptr(type_name)) {
  ...
  if (want_align) {
    ...
    if (align == 0) {
      ...
      for (size_t i = 0; i < def->fields.len; ++i) {
        size_t field_align = def->fields.data[i].align;
        if (def->pack > 0 && field_align > def->pack)
          field_align = def->pack;
        if (field_align > align)
          align = field_align;
      }
      if (def->align_override > 0)
        align = def->align_override;
    }
    val = align;
  }
  if (want_offset) {
