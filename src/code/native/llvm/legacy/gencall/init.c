 ...
 * pattern in src/code/native/llvm/legacy/binary.c: a guard block checks type
 * invariants, then branches to either the inline fast path or the generic slow
 * fallback.
 *
 * See also: src/code/native/llvm/legacy/binary.c (tagged-int fast/slow binary
 * ops) src/code/native/llvm/legacy/gencall/mono.c (type-driven monomorphization
 * logic)
 */
#include "base/util.h"
#include "code/parse/json.h"
...
/*
 * Assert condition text extraction: walk expression tree to find the source
 * span.
 */
...
LLVMValueRef ny_try_native_call_as_f64(codegen_t *cg, scope *scopes,
  ...
  if (!ret_type ||
      !(ny_type_is(ret_type, "f64") || ny_type_is(ret_type, "float")))
    return NULL;
  const char *abi_ret_type =
      sig->abi_return_type ? sig->abi_return_type : ret_type;
  if (!abi_ret_type ||
  ...
  for (size_t i = 0; i < argc; ++i) {
    ...
    if (abi_sig_param_needs_native_coerce(sig, ptype) &&
      ...
      args[i] =
          LLVMBuildFPTrunc(cg->builder, gen_expr_as_f64(cg, scopes, depth, arg),
...
LLVMValueRef gen_call_expr(codegen_t *cg, scope *scopes, size_t depth,
  ...
  if (mc && mc->name && strcmp(mc->name, "get") == 0 &&
    ...
    LLVMValueRef fast_get = ny_try_emit_fast_receiver_get(
        cg, scopes, depth, e, mc->target, &mc->args);
    if (fast_get)
      return fast_get;
  }
  if (mc && mc->name &&
      ...
      mc->args.len == 2) {
    LLVMValueRef fast_set = ny_try_emit_fast_receiver_set(
        cg, scopes, depth, e, mc->target, &mc->args);
    if (fast_set)
      return fast_set;
  }
  ...
  if (c && c->callee && c->callee->kind == NY_E_IDENT &&
    ...
    if (!ny_gencall_precheck_safe_raw_memory_api(cg, e, scopes, depth, n,
    ...
    if (!builtin_name_shadowed && strcmp(builtin_name, "assert") == 0 &&
      ...
      if (cond_expr) {
        ...
        if (start && end && end > start) {
          ...
          while (cond_len > 0 &&
                 (start[cond_len - 1] == ' ' || start[cond_len - 1] == '\t'))
            cond_len--;
          if (cond_len > 0 && cond_len < 512) {
             ...
             * Evaluate condition
             */
            LLVMValueRef cond_val = gen_expr(cg, scopes, depth, cond_expr);
            /*
            ...
            LLVMBasicBlockRef panic_bb = LLVMAppendBasicBlockInContext(
                cg->ctx, parent_fn, "assert_panic");
            ...
            if (panic_sig && panic_sig->value && panic_sig->type) {
              LLVMValueRef msg_global = const_string_ptr(cg, msg_buf, msg_len);
              LLVMValueRef msg_ptr =
                  ny_load(cg, msg_global, "assert_panic_msg");
    ...
    bool want_builtin_to_str =
        ny_gencall_builtin_name_is(builtin_name, "to_str",
                                   builtin_name_shadowed) ||
        ny_gencall_builtin_name_is(builtin_name, "str", builtin_name_shadowed);
    ...
    if (want_builtin_get && (c->args.len == 2 || c->args.len == 3) &&
      ...
      return ny_emit_fast_indexable_get(
          cg, scopes, depth, c->args.data[0].val, c->args.data[1].val,
          default_expr, e->tok, assume_nonnegative, assume_in_bounds);
    }
    if (get_target_object_elided) {
    ...
    if ((want_builtin_set_idx || want_builtin_set) && c->args.len == 3) {
      ...
      bool set_target_is_trusted_f64_list =
          ...
          ny_f64_list_target_binding(cg, scopes, depth, c->args.data[0].val) !=
              NULL;
    ...
    if ((want_load_item_checked || want_load_item_fast) && c->args.len == 2) {
      ...
      if (want_load_item_fast) {
        LLVMValueRef idx_raw = ny_gencall_index_raw_i64(
            cg, scopes, depth, c->args.data[1].val, idx_v, "load_item_idx_raw");
        /*
         * Native typed buffers expose their data pointer directly, unlike
         * legacy managed lists whose first element lives at +16.  The old
         * fast path unconditionally applied the managed-list offset, so a
         * tbuf read skipped two elem-8 values (nested iterator chunks became
         * 0/false).  The runtime accessor handles both layouts and preserves
         * the checked path's negative-index contract.
         */
        LLVMValueRef rt = ny_get_raw_i64_runtime_fn(cg, "rt_tbuf_get", 3);
        if (rt && idx_raw) {
          LLVMTypeRef rt_ty = LLVMGlobalGetValueType(rt);
          return LLVMBuildCall2(
              cg->builder, rt_ty, rt,
              (LLVMValueRef[]){lst_v, idx_raw, ny_c0(cg)}, 3,
              NY_LLVM_NAME(cg, "load_item_fast_runtime"));
        }
        if (!idx_raw)
          return ny_c0(cg);
  ...
  skip_static_handling:;
    ...
    if (name) {
      ...
      /*
       * A call in `p(...)` position resolves a callable, not a container.
       * Imported `#main`/self-test blocks register short variable bindings
       * (notably `p` in std.core.iter) that are visible to user programs.
       * When the call site's own source file defines a function with the same
       * tail name and compatible arity, that definition shadows the foreign
       * variable; otherwise the lazy emitter materializes the foreign
       * initializer (and its out-of-module lambdas) into user code.
       */
      bool foreign_var_shadows_source_fn = false;
      if (b && c->callee) {
        fun_sig *source_fn = ny_gencall_lookup_source_file_fun(
            cg, name, c->callee->tok, c->args.len);
        if (source_fn && b->stmt_t && c->callee->tok.filename &&
            b->stmt_t->tok.filename &&
            strcmp(b->stmt_t->tok.filename, c->callee->tok.filename) != 0)
          foreign_var_shadows_source_fn = true;
      }
      if (b && !foreign_var_shadows_source_fn) {
        b->is_used = true;
  ...
  if (has_sig && sig_meta && sig_meta->stmt_t &&
      ((c && c->args.len > 0) || mc) &&
      !ny_proof_call_params_ok(cg, scopes, depth, sig_meta, sig_meta->stmt_t, c,
                               mc, e->tok)) {
    cg->had_error = 1;
    goto call_fail;
  }
  for (size_t i = 0; i < final_argc; i++) {
    ...
    if (mc && !skip_target && i == 0) {
      ...
      if (!param_type && expr_for_check &&
        ...
        goto call_fail;
      args[i] = NULL;
      if (sig_meta && sig_meta->is_native_abi && param_type &&
          ny_type_is(param_type, "fnptr"))
        args[i] = ny_native_callback_adapter_value(cg, user_args[user_idx].val);
      if (!args[i])
  ...
  if (mono_base_sig && mono_base_ft && mono_base_callee && mono_tagged_args &&
    ...
    if (needs_guard) {
      ...
      if (cur_fn) {
        ...
        const char *slow_ret_type = sig_meta->abi_return_type
                                        ? sig_meta->abi_return_type
  ...
  if (has_sig && sig_meta) {
    ...
    if (LLVMGetTypeKind(ret_ty) == LLVMVoidTypeKind)
      return ny_c0(cg);
    const char *ret_type = sig_meta->abi_return_type ? sig_meta->abi_return_type
                                                     : sig_meta->return_type;
    bool is_extern_layout_return = false;
    if (sig_meta->is_extern && ret_type && *ret_type) {
      layout_def_t *ret_layout = lookup_layout(cg, ret_type);
      if (ret_layout)
        is_extern_layout_return = true;
    }
    if (!is_extern_layout_return && sig_meta->is_extern &&
        LLVMGetTypeKind(ret_ty) == LLVMStructTypeKind) {
      is_extern_layout_return = true;
    }
    if (abi_sig_type_needs_native_coerce(cg, sig_meta, ret_type) ||
        is_extern_layout_return ||
        (ret_ty && (LLVMGetTypeKind(ret_ty) == LLVMDoubleTypeKind ||
                    LLVMGetTypeKind(ret_ty) == LLVMFloatTypeKind))) {
      return ny_box_abi_result(
          cg, res,
          ret_type
              ? ret_type
              : (LLVMGetTypeKind(ret_ty) == LLVMFloatTypeKind ? "f32" : "f64"));
    }
  }
  return res;
