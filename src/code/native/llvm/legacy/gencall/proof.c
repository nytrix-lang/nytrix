...
/*
 * Canonical proof evaluation functions are declared in code/parse/proof.h
 */
...
static LLVMValueRef ny_try_lemma_proof(codegen_t *cg, scope *scopes,
  ...
  /*
   * A proof certificate may depend on literals and compiler-tracked ranges,
   * but an ordinary runtime call is not a compile-time witness.  Without
   * this boundary check a pure-looking helper such as read_value() can be
   * accepted merely because its current body happens to return a constant.
   */
  for (size_t i = 0; i < n; ++i) {
    expr_t *arg = call_args->data[i].val;
    if (arg && (arg->kind == NY_E_CALL || arg->kind == NY_E_MEMCALL)) {
      ny_diag_error(tok, "lemma '%s' could not prove the supplied arguments", name);
      ny_diag_hint("pass a compile-time value or a binding with a proven integer range");
      cg->had_error = 1;
      return ny_gencall_const_bool(cg, false, "lemma_runtime_argument");
    }
  }
