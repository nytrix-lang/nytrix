...
static inline LLVMTypeRef ny_i1_ty(codegen_t *cg) {
  return LLVMInt1TypeInContext(cg->ctx);
}
static inline LLVMTypeRef ny_i8_ty(codegen_t *cg) {
  return LLVMInt8TypeInContext(cg->ctx);
}
static inline LLVMTypeRef ny_ptr_i64_ty(codegen_t *cg) {
  return LLVMPointerType(cg->type_i64, 0);
}
...
static inline LLVMValueRef ny_cond_br(codegen_t *cg, LLVMValueRef cond,
                                      LLVMBasicBlockRef tb,
                                      LLVMBasicBlockRef fb) {
  return LLVMBuildCondBr(cg->builder, cond, tb, fb);
}

/* ── Loads & Stores ─────────────────────────────────────────── */
static inline LLVMValueRef ny_load(codegen_t *cg, LLVMValueRef ptr,
                                   const char *name) {
  return LLVMBuildLoad2(cg->builder, cg->type_i64, ptr,
                        (cg && cg->llvm_value_names && name) ? name : "");
}
static inline LLVMValueRef ny_load_type(codegen_t *cg, LLVMTypeRef ty,
                                        LLVMValueRef ptr, const char *name) {
  return LLVMBuildLoad2(cg->builder, ty, ptr,
                        (cg && cg->llvm_value_names && name) ? name : "");
}
static inline LLVMValueRef ny_store(codegen_t *cg, LLVMValueRef ptr,
                                    LLVMValueRef val) {
...
/* ── Constants ──────────────────────────────────────────────── */
static inline LLVMValueRef ny_c0(codegen_t *cg) {
  return LLVMConstInt(cg->type_i64, 0, false);
}
static inline LLVMValueRef ny_c1(codegen_t *cg) {
  return LLVMConstInt(cg->type_i64, 1, false);
}
static inline LLVMValueRef ny_cnil(codegen_t *cg) {
  return LLVMConstInt(cg->type_i64, NY_IMM_NIL, false);
}
...
/* ── Type helpers ───────────────────────────────────────────── */
static inline int ny_is_i64(codegen_t *cg, LLVMValueRef v) {
  return LLVMTypeOf(v) == cg->type_i64;
}
...
static inline LLVMTypeRef ny_ptr_i64(codegen_t *cg) {
  return LLVMPointerType(cg->type_i64, 0);
}

/* ── Comparisons & Arithmetic ────────────────────────────────── */
static inline LLVMValueRef ny_icmp(codegen_t *cg, LLVMIntPredicate pred,
                                   LLVMValueRef lhs, LLVMValueRef rhs,
                                   const char *name) {
  return LLVMBuildICmp(cg->builder, pred, lhs, rhs,
                       (cg && cg->llvm_value_names && name) ? name : "");
}
...
static inline LLVMValueRef ny_add(codegen_t *cg, LLVMValueRef a, LLVMValueRef b,
                                  const char *n) {
  return LLVMBuildAdd(cg->builder, a, b,
                      (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_sub(codegen_t *cg, LLVMValueRef a, LLVMValueRef b,
                                  const char *n) {
  return LLVMBuildSub(cg->builder, a, b,
                      (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_mul(codegen_t *cg, LLVMValueRef a, LLVMValueRef b,
                                  const char *n) {
  return LLVMBuildMul(cg->builder, a, b,
                      (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_shl(codegen_t *cg, LLVMValueRef a, LLVMValueRef b,
                                  const char *n) {
  return LLVMBuildShl(cg->builder, a, b,
                      (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_ashr(codegen_t *cg, LLVMValueRef a,
                                   LLVMValueRef b, const char *n) {
  return LLVMBuildAShr(cg->builder, a, b,
                       (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_and(codegen_t *cg, LLVMValueRef a, LLVMValueRef b,
                                  const char *n) {
  return LLVMBuildAnd(cg->builder, a, b,
                      (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_or(codegen_t *cg, LLVMValueRef a, LLVMValueRef b,
                                 const char *n) {
  return LLVMBuildOr(cg->builder, a, b,
                     (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_xor(codegen_t *cg, LLVMValueRef a, LLVMValueRef b,
                                  const char *n) {
  return LLVMBuildXor(cg->builder, a, b,
                      (cg && cg->llvm_value_names && n) ? n : "");
}
static inline LLVMValueRef ny_select(codegen_t *cg, LLVMValueRef cond,
                                     LLVMValueRef t, LLVMValueRef f,
                                     const char *n) {
  return LLVMBuildSelect(cg->builder, cond, t, f,
                         (cg && cg->llvm_value_names && n) ? n : "");
}

/* ── PHI nodes ──────────────────────────────────────────────── */
static inline LLVMValueRef ny_phi(codegen_t *cg, LLVMTypeRef t, const char *n) {
  return LLVMBuildPhi(cg->builder, t,
                      (cg && cg->llvm_value_names && n) ? n : "");
}
static inline void ny_phi_add(LLVMValueRef p, LLVMValueRef v,
                              LLVMBasicBlockRef b) {
  LLVMAddIncoming(p, &v, &b, 1);
}

/* ── Calls ──────────────────────────────────────────────────── */
static inline LLVMValueRef ny_call0(codegen_t *cg, LLVMTypeRef ft,
                                    LLVMValueRef fn) {
  return LLVMBuildCall2(cg->builder, ft, fn, NULL, 0, "");
}
static inline LLVMValueRef ny_call1(codegen_t *cg, LLVMTypeRef ft,
                                    LLVMValueRef fn, LLVMValueRef a0) {
  return LLVMBuildCall2(cg->builder, ft, fn, &a0, 1, "");
}
static inline LLVMValueRef ny_call2(codegen_t *cg, LLVMTypeRef ft,
                                    LLVMValueRef fn, LLVMValueRef a0,
                                    LLVMValueRef a1) {
  LLVMValueRef args[2] = {a0, a1};
  return LLVMBuildCall2(cg->builder, ft, fn, args, 2, "");
}
static inline LLVMValueRef ny_call3(codegen_t *cg, LLVMTypeRef ft,
                                    LLVMValueRef fn, LLVMValueRef a0,
...
static inline LLVMBasicBlockRef ny_bb(codegen_t *cg, const char *name) {
  return ny_llvm_append_block(
      LLVMGetBasicBlockParent(LLVMGetInsertBlock(cg->builder)), name);
}
static inline LLVMBasicBlockRef ny_bb_fn(LLVMValueRef fn, const char *name) {
  /* Names are debug labels, not block identities. Reusing a name would
   * merge independent loop/match arms (and fails when names are disabled). */
...
static inline LLVMMetadataRef ny_loop_bool_attr(codegen_t *cg, const char *name,
                                                bool value) {
  LLVMContextRef ctx = cg->ctx;
  LLVMMetadataRef s = LLVMMDStringInContext2(ctx, name, strlen(name));
