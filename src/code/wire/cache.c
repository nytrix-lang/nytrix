...
uint64_t ny_cache_compiler_source_fingerprint(void) {
  ...
  const char *exe_path = ny_get_executable_path();
  ...
  if (root && *root) {
    const char *parts[] = {
        "CMakeLists.txt", "src/base", "src/code", "src/code/parse",
        "src/cmd/ny/repl",       "src/code/runtime",   "src/sema", "src/code/wire",
    };
...
/*
 * Bump after removing process-local runtime addresses from cached bitcode.
 */
enum { NY_JIT_CACHE_VERSION = 29 };
/*
...
bool ny_jit_cache_load(const char *cache_path, LLVMContextRef ctx, LLVMModuleRef *out_module) {
  ...
  bool parsed = false;
  /*
   * LLVMParseIRInContext ADOPTS the memory buffer unconditionally -- it is
   * released by LLVM on parse FAILURE just as on success. Disposing it here
   * as well was a guaranteed double free ("free(): invalid size") whenever a
   * stale/truncated text-format cache entry failed to parse. The bitcode
   * path (ParseBitcodeInContext2) does NOT take ownership, so its buffer is
   * always ours to dispose.
   */
  bool llvm_owns_buffer = false;
  if (ny_cache_path_is_ir(cache_path)) {
    char *err_msg = NULL;
    parsed = LLVMParseIRInContext(ctx, buf, out_module, &err_msg) == 0 && *out_module != NULL;
    if (err_msg)
      LLVMDisposeMessage(err_msg);
    llvm_owns_buffer = true;
  } else {
...
char *ny_jit_stencil_cache_path(const char *source, int opt_level,
  ...
  key = ny_fnv1a64_cstr("ny-stencil-cache-v3", key);
  ...
  /*
   * The JIT backend is compiled into `ny` and changes with every compiler
   * rebuild.  A stale on-disk stencil (encoded machine code) from an older
   * code generator is observably wrong, so pin the cache key to the compiler
   * build identity: the executable's mtime/size plus the version string.  An
   * uncommitted rebuild changes the executable bytes and NYTRIX_VERSION_BUILD,
   * so a stale cached verdict can never outlive its compiler.
   */
  key = ny_fnv1a64_cstr("stencil-compiler-fingerprint-v1", key);
  {
    const char *exe = ny_get_executable_path();
    const char *p = exe ? exe : "";
    if (*p) {
      struct stat st;
      if (stat(p, &st) == 0) {
        key = ny_hash64_u64(key, (uint64_t)st.st_mtime);
        key = ny_hash64_u64(key, ny_stat_mtime_nsec(&st));
        key = ny_hash64_u64(key, (uint64_t)st.st_size);
      }
    }
  }
#ifdef NYTRIX_VERSION
  key = ny_fnv1a64_cstr(NYTRIX_VERSION, key);
#endif
  const char *root = ny_cache_root_dir();
  if (!root || !*root)
    return NULL;
