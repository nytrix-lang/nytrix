...
/*
 * A discovered ccache is not necessarily usable: sandboxed runners and
 * read-only home mounts commonly leave the executable visible while its
 * cache directory cannot be created.  Falling back to the real compiler is
 * deterministic and avoids turning a runtime test into a tool-cache error.
 */
static bool ny_builder_ccache_usable(void) {
  if (!ny_tool_in_path("ccache"))
    return false;
#ifdef _WIN32
  return true;
#else
  const char *dir = getenv("CCACHE_DIR");
  char fallback[PATH_MAX];
  if (!dir || !*dir) {
    const char *xdg = getenv("XDG_CACHE_HOME");
    const char *home = getenv("HOME");
    if (xdg && *xdg)
      snprintf(fallback, sizeof(fallback), "%s/ccache", xdg);
    else if (home && *home)
      snprintf(fallback, sizeof(fallback), "%s/.cache/ccache", home);
    else
      return false;
    dir = fallback;
  }
  if (access(dir, W_OK) == 0)
    return true;
  /*
   * ccache can create a missing cache directory when its parent is writable.
   */
...
static time_t ny_runtime_latest_dep_mtime(const char *root) {
  ...
  static const char *const deps[] = {
      "src/code/runtime/init.c",     "src/code/runtime/ast.c",       "src/code/runtime/bigint.c", "src/code/runtime/core.c",
      "src/code/runtime/ffi.c",      "src/code/runtime/ffigates.c",  "src/code/runtime/gc.c",     "src/code/runtime/math.c",
      "src/code/runtime/memory.c",   "src/code/runtime/os.c",        "src/code/runtime/proof.c",
      "src/code/runtime/simmd.c",    "src/code/runtime/string.c",    "src/code/runtime/bigfloat.c",
      "src/code/runtime/shared.h",   "src/code/runtime/runtime.h",   "src/code/runtime/defs.h",   "src/code/parse/ast.h",
      "src/code/parse/json.h",  "src/code/parse/parser.h", "src/code/parse/lexer.h", "src/code/typing/types.h",
...
static void ny_runtime_cache_path(char *out, size_t out_len, const char *cc, const char *root,
   ...
   */
  const char *cache_rev = "rtcache-v10-native-runtime";
  char dwarf_key[16];
  if (debug)
...
static bool ny_builder_compile_runtime_archive(
  ...
  for (size_t u = 0; u < sizeof(units) / sizeof(units[0]); ++u) {
    snprintf(src[u], sizeof(src[u]), "%s/src/code/runtime/%s.c", root, units[u]);
...
bool ny_builder_compile_runtime(const char *cc, const char *out_runtime, const char *out_ast,
  ...
  if (speed_level > 3)
    speed_level = 3;
  snprintf(runtime_src, sizeof(runtime_src), "%s/src/code/runtime/init.c", root);
  snprintf(ast_src, sizeof(ast_src), "%s/src/code/runtime/ast.c", root);
  ...
  if (!out_ast && (!sanitize_kind || !*sanitize_kind) &&
      ny_try_restore_runtime_cache(cache_obj, out_runtime, root))
    return true;
  bool has_ccache = ny_builder_ccache_usable();
#if !defined(_WIN32)
  if (!out_ast) {
...
#if !defined(__APPLE__) && !defined(_WIN32)
  if (!target_windows) {
    const char *linker = getenv("NYTRIX_LINKER");
    if (linker && strcmp(linker, "mold") == 0) {
      argv[idx++] = "-fuse-ld=mold";
    } else if (linker && strcmp(linker, "lld") == 0) {
      argv[idx++] = "-fuse-ld=lld";
    } else if (linker && *linker && strcmp(linker, "system") != 0) {
      NY_LOG_ERR("NYTRIX_LINKER must be system, lld, or mold (got '%s')\n", linker);
      return false;
    } else if (verbose_enabled >= 2) {
      NY_LOG_V2("Using default system linker.\n");
    }
  }
#endif
#if !defined(_WIN32)
