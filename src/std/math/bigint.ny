;;; bigint.ny --- math bigint module

;; Keywords: math bigint

;;; Commentary:

;; Arbitrary-precision integers using base 1e9 digits.
...
fn _big_add_small(a, v){
	...
	def da = list_clone(_big_digits(a))
	def i = 0
	def carry = v
	while(carry > 0){
...
fn _big_divmod_abs(a, b){
	...
	while(idx >= 0){
		; r = r * base + da[idx]
		def rd = list_clone(_big_digits(r))
		rd = _digits_prepend(rd, get(da, idx))
		r = _big_make(_big_sign(r), rd)
		; find q digit by binary search
		def lo = 0  def hi = 1000000000 - 1
		def best = 0
		while(lo <= hi){
			def mid = (lo + hi) / 2
			def prod = _big_mul_small(b, mid)
		...
		if(best > 0){
			def prod2 = _big_mul_small(b, best)
